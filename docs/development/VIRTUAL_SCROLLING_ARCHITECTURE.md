...
class ViewportCalculator {
    ...
    calculateVisibleRange(scrollTop, containerHeight, bufferSize = 0) {
        ...
        const endIndex = Math.min(this.totalItems - 1,
            Math.ceil((scrollTop + containerHeight + bufferSize * this.itemHeight) / this.itemHeight)
        );
...
class VirtualGalleryServiceWorker {
    ...
    isGalleryRequest(request) {
        return request.url.includes('/api/gallery') || 
    ...
    async preloadGalleryImages(images) {
        ...
        const thumbnailPromises = images.slice(0, 20).map(img => 
            fetch(img.thumbnail_url).then(response => {
                if (response.ok) {
        ...
        const fullImagePromises = images.slice(0, 5).map(img => 
            fetch(img.full_url).then(response => {
                if (response.ok) {
...
class MemoryDiagnostics {
    ...
    static detectLeaks(virtualGallery) {
        ...
        // Check element pool
        if (virtualGallery.recycledElements.length < virtualGallery.poolSize * 0.8) {
            leaks.unreleasedElements = virtualGallery.poolSize - virtualGallery.recycledElements.length;
        }
...
class ImageDiagnostics {
    ...
    static testImageURL(url) {
        return new Promise((resolve) => {
            ...
            img.onerror = () => resolve({ success: false, url, error: 'Load failed' });
            img.src = url;
...
class VirtualGalleryDebugger {
    ...
    drawDebugOverlays() {
        ...
        const existingOverlays = this.gallery.container.querySelectorAll('.debug-overlay');
    ...
    getCurrentFPS() {
        ...
        return this.gallery.performanceMonitor ? 
               this.gallery.performanceMonitor.getCurrentFPS() : 'N/A';
    }
}
```
