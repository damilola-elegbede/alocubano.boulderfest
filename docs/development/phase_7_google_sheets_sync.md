...
class GoogleSheetsSync {
  ...
  validateInput(data) {
    ...
    if (!data || !Array.isArray(data)) {
      throw new GoogleSheetsSyncError('Invalid input: data must be an array', 'ValidationError');
    }
    ...
    if (data.length > 10000) {
      throw new GoogleSheetsSyncError('Maximum sync batch size exceeded (10,000 records)', 'BatchSizeError');
    }
      ...
      return await this.rateLimiter.schedule(async () => {
        try {
          ...
          if (apiError.code === 403) {
            throw new GoogleSheetsSyncError('Authorization failed', 'AuthorizationError');
          }
          if (apiError.code === 429) {
            throw new GoogleSheetsSyncError('Rate limit exceeded', 'RateLimitError');
          }
          throw apiError;
        }
      });
