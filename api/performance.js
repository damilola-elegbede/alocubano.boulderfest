    ...
    try {
        ...
        console.log('[PERFORMANCE ANALYTICS]:', {
            ...
            coreWebVitals: coreWebVitals ? {
                lcp: coreWebVitals.lcp ? `${coreWebVitals.lcp.toFixed(0)}ms` : 'N/A',
                fid: coreWebVitals.fid ? `${coreWebVitals.fid.toFixed(0)}ms` : 'N/A',
                cls: coreWebVitals.cls ? coreWebVitals.cls.toFixed(3) : 'N/A'
            } : 'N/A',
            customMetrics: customMetrics ? Object.keys(customMetrics).length + ' metrics' : 'N/A',
            memory: memory ? `${(memory.used / 1024 / 1024).toFixed(1)}MB used` : 'N/A'
        });
        ...
        if (customMetrics && typeof customMetrics === 'object') {
            ...
            Object.entries(customMetrics).forEach(([key, value]) => {
                if (key.includes('virtual_scroll') && value > 16) {
                    interestingMetrics.push(`${key}: ${value.toFixed(1)}ms (slow scroll)`);
                }
...
function generatePerformanceInsights(data) {
    ...
    try {
        ...
        if (data.coreWebVitals) {
            ...
            if (lcp && lcp < 1200) {
                insights.push({ type: 'positive', message: 'Excellent Largest Contentful Paint' });
            } else if (lcp && lcp > 2500) {
                insights.push({ type: 'warning', message: 'LCP optimization needed - consider image optimization' });
            }
            
            if (fid && fid < 50) {
                insights.push({ type: 'positive', message: 'Excellent First Input Delay' });
            } else if (fid && fid > 100) {
                insights.push({ type: 'warning', message: 'FID optimization needed - reduce JavaScript execution time' });
            }
            
            if (cls && cls < 0.05) {
                insights.push({ type: 'positive', message: 'Excellent Cumulative Layout Shift' });
            } else if (cls && cls > 0.1) {
                insights.push({ type: 'warning', message: 'CLS optimization needed - avoid layout shifts' });
            }
        }
        ...
        if (data.memory && data.memory.utilization) {
            if (data.memory.utilization < 50) {
                ...
                insights.push({ type: 'warning', message: 'High memory usage - consider optimizing' });
            }
        }
        ...
        if (data.network && data.network.cacheHitRate) {
            ...
            if (hitRate > 85) {
                insights.push({ type: 'positive', message: 'Excellent cache performance' });
            } else if (hitRate < 60) {
                insights.push({ type: 'warning', message: 'Cache optimization opportunity' });
            }
        }
