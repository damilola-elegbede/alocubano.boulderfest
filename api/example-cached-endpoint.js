...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (endpoint === 'analytics') {
      ...
      const analyticsData = {
        totalVisitors: await cacheService.getCounter('visitors:total') || 15432,
        ticketsSold: await cacheService.getCounter('tickets:sold') || 892,
        newsletterSignups: await cacheService.getCounter('newsletter:signups') || 2341,
        pageViews: {
          home: 8765,
          tickets: 4321,
          gallery: 3456,
    ...
    if (endpoint === 'dashboard') {
      const keys = ['event:info', 'tickets:current-availability', 'analytics:dashboard'];
      const results = {};

      // Try to get all data from cache first
      for (const key of keys) {
        const namespace = key.startsWith('event:') ? 'static' :
                         key.startsWith('tickets:') ? 'tickets' :
