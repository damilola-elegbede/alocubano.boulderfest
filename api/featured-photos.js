...
export default async function handler(req, res) {
  ...
  try {
    ...
    const featuredPhotosFolderId = process.env.GOOGLE_DRIVE_FEATURED_PHOTOS_FOLDER_ID || process.env.GOOGLE_DRIVE_FOLDER_ID;
    
    if (!featuredPhotosFolderId) {
      return res.status(400).json({ 
        error: 'Featured photos folder ID is required. Please set GOOGLE_DRIVE_FEATURED_PHOTOS_FOLDER_ID or GOOGLE_DRIVE_FOLDER_ID.' 
      });
    }
    ...
    if (error.code === 404) {
      return res.status(404).json({ error: 'Featured photos folder not found' });
    }
    
    if (error.code === 403) {
      return res.status(403).json({ error: 'Access denied. Please check featured photos folder permissions.' });
    }
