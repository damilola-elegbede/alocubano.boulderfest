  ...
  case 'system':
    return {
      memory: {
        ...
        usage_percent:
            (metrics.system['gauge.system.memory.heap_total'] || 0) === 0
              ? 0
              : (metrics.system['gauge.system.memory.heap_used'] /
                  metrics.system['gauge.system.memory.heap_total']) *
                100
      },
      cpu: {
  ...
  case 'business':
    return {
      ...
      revenue: {
        ...
        average_transaction:
            metrics.business.payments.attempts > 0
              ? metrics.business.payments.revenue /
                metrics.business.payments.successes
              : 0
      }
    };
...
function formatPrometheus(metrics) {
  ...
  if (metrics.business) {
    ...
    if (business.payments) {
      ...
      lines.push(
        `alocubano_payments_total{status="success"} ${business.payments.successes}`
      );
      lines.push(
        `alocubano_payments_total{status="failure"} ${business.payments.failures}`
      );
    ...
    if (business.tickets) {
      ...
      Object.entries(business.tickets).forEach(([operation, count]) => {
        lines.push(
          `alocubano_tickets_total{operation="${operation}"} ${count}`
        );
      });
    }
  }
  ...
  if (metrics.performance && metrics.performance.percentiles) {
    ...
    lines.push(
      `alocubano_response_time_milliseconds{quantile="0.5"} ${perf.p50 || 0}`
    );
    lines.push(
      `alocubano_response_time_milliseconds{quantile="0.95"} ${perf.p95 || 0}`
    );
    lines.push(
      `alocubano_response_time_milliseconds{quantile="0.99"} ${perf.p99 || 0}`
    );
  }
...
function formatDatadog(metrics) {
  ...
  if (metrics.business) {
    ...
    if (business.payments) {
      series.push(
        createMetric('payments.success', business.payments.successes, 'count')
      );
      series.push(
        createMetric('payments.failure', business.payments.failures, 'count')
      );
      series.push(
        createMetric('revenue.total', business.payments.revenue, 'count')
      );
    }

    if (business.users) {
      series.push(
        createMetric('users.active', business.users.activeCount, 'gauge')
      );
      series.push(
        createMetric('users.registered', business.users.registrations, 'count')
      );
    }
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const formattedMetrics = formatMetricsForSystem(metrics, format);
