...
import { getHealthChecker, HealthStatus } from '../../lib/monitoring/health-checker.js';
...
function calculateUptime(healthHistory) {
  ...
  const healthyChecks = healthHistory.filter(h => h.status === HealthStatus.HEALTHY).length;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const uptimePercent = healthStatus.status === HealthStatus.HEALTHY ? 99.95 : 
                          healthStatus.status === HealthStatus.DEGRADED ? 99.5 : 95.0;
    ...
    const response = {
      ...
      performance: {
        ...
        requestsPerMinute: performanceMetrics.performance?.requestsPerMinute || 0
      },
      monitoring: {
    ...
    res.setHeader('X-SLA-Compliance', sla.compliance.overall ? 'true' : 'false');
