...
class MigrationRunner {
  ...
  async runMigration(migrationFile, options = {}) {
    ...
    try {
      ...
      const isApplied = status.appliedMigrations.some(m => m.version === version);
      
      if (isApplied && !force) {
        await this.log(`Migration ${version} already applied. Use force option to reapply.`, 'WARN');
        return {
          success: false,
      ...
      if (!skipBackup && !dryRun) {
        ...
        backupMetadata = await this.backupManager.createBackup(`pre_migration_${version}`);
      ...
      if (dryRun) {
        ...
        await this.log(`Would execute migration from: ${migrationPath}`, 'INFO');
  ...
  async rollbackMigration(migrationVersion) {
    await this.log(`Starting rollback for migration ${migrationVersion}`, 'INFO');
    ...
    try {
      ...
      const relevantBackup = backups.find(b => 
        b.description && b.description.includes(`pre_migration_${migrationVersion}`)
      );
      ...
      const integrity = await this.backupManager.verifyBackupIntegrity(relevantBackup.path);
      ...
      // Restore from backup
      await this.log(`Restoring from backup: ${relevantBackup.filename}`, 'INFO');
      const restoreResult = await this.backupManager.restoreFromBackup(relevantBackup.path);
      ...
      await this.log(`Rollback completed for migration ${migrationVersion}`, 'INFO');
      ...
      throw new Error(`Rollback failed for migration ${migrationVersion}: ${error.message}`);
    }
  }
  ...
  async validateMigration(migrationVersion) {
    ...
    try {
      ...
      const status = await this.getMigrationStatus();
      const isRecorded = status.appliedMigrations.some(m => m.version === migrationVersion);
      
      validationResults.checks.push({
        check: 'migration_recorded',
        passed: isRecorded,
        message: isRecorded ? 'Migration recorded in schema_migrations' : 'Migration not recorded'
      });
      ...
      if (migrationVersion === '009') {
        ...
        validationResults.checks.push({
          ...
          message: walletSourceExists ? 'wallet_source column exists' : 'wallet_source column missing'
        });
      ...
      if (validationResults.valid) {
        await this.log(`Migration ${migrationVersion} validated successfully`, 'INFO');
      } else {
        await this.log(`Migration ${migrationVersion} validation failed`, 'WARN');
      }
  ...
  async recoverFromPartialMigration() {
    ...
    try {
      ...
      // Verify backup integrity
      await this.log('Verifying backup integrity...', 'INFO');
      const integrity = await this.backupManager.verifyBackupIntegrity(mostRecentBackup.path);
      ...
      // Restore from backup
      await this.log(`Recovering from backup: ${mostRecentBackup.filename}`, 'INFO');
      const restoreResult = await this.backupManager.restoreFromBackup(mostRecentBackup.path);
      ...
      throw new Error(`Recovery from partial migration failed: ${error.message}`);
    }
  }
    ...
    try {
      ...
      for (const tableName of criticalTables) {
        if (!integrityResults.tables[tableName]) {
          ...
          integrityResults.issues.push(`Critical table '${tableName}' is missing`);
        }
      }
      ...
      await this.log(`Schema integrity check failed: ${error.message}`, 'ERROR');
      integrityResults.valid = false;
  ...
  async getPendingMigrations() {
    try {
      ...
      await this.log(`Failed to get pending migrations: ${error.message}`, 'ERROR');
      throw error;
    }
  }
}
