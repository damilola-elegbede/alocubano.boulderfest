...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (
      ...
      console.log(
        "CI/Test environment detected without Google credentials, returning mock data",
      );
    ...
    if (eventParamRaw) {
      ...
      if (allowedEvents.includes(sanitizedEvent)) {
        ...
        return res.status(400).json({
          error:
            "Invalid event parameter. Allowed values: " +
            allowedEvents.join(", "),
        });
      }
    }
    ...
    const category =
      categoryParam && allowedCategories.includes(categoryParam.toLowerCase())
    ...
    if (isNaN(parsedLimit) || parsedLimit < 1 || parsedLimit > 100) {
      return res
        .status(400)
        .json({ error: "Invalid limit. Must be integer between 1 and 100." });
    }
    ...
    if (isNaN(parsedOffset) || parsedOffset < 0) {
      return res
        .status(400)
        .json({ error: "Invalid offset. Must be non-negative integer." });
    }
    ...
    console.log(
      `Cache file not found for year ${year}, falling back to Google Drive API`,
    );

    // Use separate gallery folder ID (different from hero images)
    const galleryRootFolderId =
      req.query.folderId || process.env.GOOGLE_DRIVE_GALLERY_FOLDER_ID;

    if (!galleryRootFolderId) {
      return res.status(400).json({
        error:
          "Gallery folder ID is required. Please set GOOGLE_DRIVE_GALLERY_FOLDER_ID.",
      });
    }
