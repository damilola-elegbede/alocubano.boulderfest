...
function calculatePercentile(sortedArray, percentile) {
    ...
    return Math.round((sortedArray[lower] * (1 - weight) + sortedArray[upper] * weight) * 100) / 100;
}
...
function checkAlerts(aggregatedMetrics) {
    ...
    // Check memory usage
    if (aggregatedMetrics.memoryUsage && aggregatedMetrics.memoryUsage.p95 > thresholds.memoryUsage) {
        alerts.push({
            type: 'resource',
            metric: 'memoryUsage',
...
async function storeMetrics(processedData) {
    ...
    console.log('Aggregated Metrics:', JSON.stringify(processedData.aggregatedMetrics, null, 2));
