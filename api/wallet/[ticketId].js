...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      if ((!walletType || walletType === 'apple') && appleWalletService.isConfigured()) {
        try {
      ...
      if ((!walletType || walletType === 'google') && googleWalletService.isConfigured()) {
        try {
      ...
      if ((!walletType || walletType === 'apple') && appleWalletService.isConfigured()) {
        try {
          await appleWalletService.revokePass(ticketId, reason || 'User requested');
          results.apple = { success: true, message: 'Apple Wallet pass revoked' };
        } catch (error) {
      ...
      if ((!walletType || walletType === 'google') && googleWalletService.isConfigured()) {
        try {
          await googleWalletService.revokePass(ticketId, reason || 'User requested');
          results.google = { success: true, message: 'Google Wallet pass revoked' };
        } catch (error) {
