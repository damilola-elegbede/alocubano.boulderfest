...
async function checkAccountInfo() {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Account info fetch failed: ${response.status} - ${error}`);
    }
    ...
    return {
      ...
      quota_usage_percent: credits.sendLimit ? 
        ((credits.used / credits.sendLimit) * 100).toFixed(2) : 0
    };
  } catch (error) {
...
async function checkContactLists() {
  try {
    ...
    if (newsletterListId) {
      newsletterList = lists.find(list => 
...
async function checkEmailTemplates() {
  try {
    ...
    const verificationTemplate = templates.find(t => 
...
async function checkRecentEmailActivity() {
  try {
    ...
    return {
      ...
      delivery_rate: stats.requests > 0 ? 
        ((stats.delivered / stats.requests) * 100).toFixed(2) + '%' : 'N/A',
      open_rate: stats.delivered > 0 ? 
        ((stats.opens / stats.delivered) * 100).toFixed(2) + '%' : 'N/A'
    };
  } catch (error) {
    return {
...
export const checkBrevoHealth = async () => {
  ...
  try {
    ...
    if (recentActivity.requests > 0) {
      const bounceRate = ((recentActivity.hard_bounces + recentActivity.soft_bounces) / recentActivity.requests) * 100;
      if (bounceRate > 5) {
        status = HealthStatus.DEGRADED;
