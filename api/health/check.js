import { getHealthChecker, HealthStatus, formatHealthResponse } from '../../lib/monitoring/health-checker.js';
...
import { initSentry, addBreadcrumb } from '../../lib/monitoring/sentry-config.js';
...
function getHealthRecommendations(health) {
  ...
  } else if (services.stripe?.details?.warnings?.includes('API rate limit approaching')) {
    recommendations.push({
      severity: 'warning',
      service: 'stripe',
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (service) {
      ...
      const openBreakers = Object.entries(circuitStates)
