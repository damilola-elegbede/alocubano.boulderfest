...
export class AdvancedRateLimiter {
  ...
  async applyPenalty(clientId, endpoint) {
    ...
    const penaltyKey = `penalty:${endpoint}:${clientId}`;
    const currentMultiplier = await this.getPenaltyMultiplier(
      clientId,
      endpoint,
    );
    const newMultiplier = Math.min(
      currentMultiplier * 2,
      config.maxPenaltyMultiplier || 32,
    );
  ...
  async checkAbusePattern(endpoint, clientId) {
    ...
    try {
      ...
      if (this.redis) {
        ...
        const record = this.fallbackStore.get(alertKey) || {
          count: 0,
  ...
  async triggerAlert(endpoint, clientId, count) {
    const alertData = {
      ...
      severity:
        count > ENDPOINT_CONFIGS[endpoint].alertThreshold * 2
          ? "high"
          : "medium",
    };
  ...
  async defaultAlertHandler(alertData) {
    console.warn(
      `[RATE LIMIT ALERT] ${alertData.severity.toUpperCase()}: ${alertData.clientId} exceeded limits on ${alertData.endpoint} (${alertData.count} violations)`,
    );
  ...
  async checkRateLimit(req, endpoint, options = {}) {
    ...
    try {
      ...
      const penaltyMultiplier = await this.getPenaltyMultiplier(
        clientId,
        endpoint,
      );
      ...
      if (
        config.ipLimit &&
        (options.clientType === "ip" || !options.clientType)
      ) {
  ...
  async checkFixedWindow(key, limit, windowMs) {
    ...
    if (this.redis) {
      try {
        ...
        return this.checkFallbackFixedWindow(
          key,
          limit,
          windowMs,
          windowStart,
  ...
  cleanupFallbackStore() {
    ...
    if (keysToDelete.length > 0) {
      console.log(
        `Cleaned up ${keysToDelete.length} expired rate limit records`,
      );
    }
  }
