...
class BrevoService {
    ...
    async unsubscribeContact(email) {
        try {
            ...
            if (error.message.includes('contact_not_exist')) {
                return { success: true, message: 'Contact not found (already unsubscribed)' };
            }
            throw error;
        }
    }
    ...
    async getAllListStats() {
        const statPromises = Object.entries(this.lists).map(async ([name, listId]) => {
            try {
                const stats = await this.getListStats(listId);
                return { name, listId, stats };
            } catch (error) {
                console.error(`Failed to get stats for list ${name} (${listId}):`, error);
