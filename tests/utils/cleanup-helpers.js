...
export class EventListenerTracker {
    ...
    cleanup() {
        ...
        this.listeners.forEach(({ target, type, listener, options }) => {
            try {
                if (this.originalRemoveEventListener && target && typeof target.removeEventListener === 'function') {
                    this.originalRemoveEventListener.call(target, type, listener, options);
                }
            } catch (e) {
...
export function logMemoryUsage(label = '') {
    if (process.memoryUsage) {
        ...
        if (heapUsedMB > 500) { // Log if over 500MB
            console.warn(`[Memory Warning] ${label}: Heap ${heapUsedMB}MB/${heapTotalMB}MB, RSS ${rssMB}MB`);
        }
