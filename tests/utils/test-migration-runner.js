...
export class TestMigrationRunner {
  ...
  readMigrationFile(filePath) {
    ...
    try {
      ...
      this.log(
        "debug",
        `Parsed ${validStatements.length} statements from ${filePath}`,
      );
      ...
      throw new Error(
        `Failed to read migration file ${filePath}: ${error.message}`,
      );
    }
  }
  ...
  async getAppliedMigrations(client) {
    ...
    try {
      ...
      this.log(
        "debug",
        `Found ${appliedSet.size} previously applied migrations`,
      );
  ...
  async recordMigrationExecution(
    ...
    try {
      ...
      this.log(
        "warn",
        `Failed to record migration execution for ${filename}`,
        error,
      );
  ...
  async executeMigration(client, migrationFile) {
    ...
    try {
      ...
      if (this.options.transactionMode && !hasOwnTransactions) {
        ...
        try {
          for (const statement of statements) {
            ...
            this.log(
              "debug",
              `Executed statement ${executedStatements}/${statements.length}`,
            );
          }
          ...
          this.log(
            "info",
            `Migration ${migrationFile.name} completed successfully (${executedStatements} statements)`,
          );
        } catch (error) {
        ...
        if (hasOwnTransactions) {
          this.log(
            "debug",
            `Migration ${migrationFile.name} has own transaction management`,
          );
        }
      ...
      if (!this.options.continueOnError) {
        throw new Error(
          `Migration ${migrationFile.name} failed: ${error.message}`,
        );
      }
    } finally {
  ...
  async _executeStatementWithFallback(client, statement) {
    ...
    const alterTableMatch = trimmedStatement.match(
    ...
    if (alterTableMatch) {
      ...
      try {
        ...
        if (
          ...
          error.message.includes('near "EXISTS"')
        ) {
          this.log(
            "debug",
            `Falling back to column existence check for ${tableName}.${columnName}`,
          );

          // Check if column already exists
          const columnExists = await this._checkColumnExists(
            client,
            tableName,
            columnName,
          );
          ...
          if (!columnExists) {
            ...
            if (/\bNOT\s+NULL\b/i.test(columnDefinition)) {
              ...
              this.log(
                "warn",
                `Removing NOT NULL constraint from ${tableName}.${columnName} - add data and constraints separately`,
              );
            }
            ...
            this.log(
              "debug",
              `Executing fallback statement: ${fallbackStatement}`,
            );
            ...
            if (hasUniqueConstraint) {
              ...
              try {
                ...
                this.log(
                  "warn",
                  `Failed to create unique index for ${tableName}.${columnName}`,
                  indexError,
                );
    ...
    const createIndexMatch = statement
      .trim()
      .match(
    ...
    if (createIndexMatch) {
      ...
      try {
        ...
        if (
          ...
          this.log(
            "warn",
            `Skipping index creation due to missing column: ${statement.trim()}`,
          );
          return;
        }
    ...
    if (statement.trim().toUpperCase().includes("RAISE(")) {
      try {
        ...
        if (
          ...
          this.log(
            "warn",
            `Skipping statement with invalid RAISE() usage: ${statement.trim().substring(0, 100)}...`,
          );
          return;
        }
        throw error;
  ...
  async _checkColumnExists(client, tableName, columnName) {
    try {
      ...
      return result.rows.some(
        (row) => row.name.toLowerCase() === columnName.toLowerCase(),
      );
    } catch (error) {
      // If table doesn't exist, column doesn't exist
      this.log(
        "debug",
        `Table ${tableName} doesn't exist or cannot check column ${columnName}`,
      );
      return false;
    }
  }
    ...
    try {
      ...
      for (const migrationFile of migrationFiles) {
        if (appliedMigrations.has(migrationFile.name)) {
          this.log(
            "debug",
            `Skipping already applied migration: ${migrationFile.name}`,
          );
