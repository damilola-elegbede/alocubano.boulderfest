...
class TestReporter {
    ...
    updateProgressDisplay() {
        ...
        const percentage = this.progressTotal > 0 ? 
            Math.round((this.progressCurrent / this.progressTotal) * 100) : 0;
    ...
    printSummaryReport(results, duration) {
        ...
        console.log(`${this.colorize('Total Links Tested:', 'cyan')} ${results.total}`);
        console.log(`${this.colorize('Broken Links:', results.broken > 0 ? 'red' : 'green')} ${results.broken}`);
        console.log(`${this.colorize('Success Rate:', successRate >= 95 ? 'green' : successRate >= 80 ? 'yellow' : 'red')} ${successRate}%`);
        console.log(`${this.colorize('Test Duration:', 'blue')} ${(duration / 1000).toFixed(2)}s`);
    ...
    printBrokenLinksReport(brokenLinks) {
        ...
        brokenLinks.forEach((link, index) => {
            ...
            console.log(`   ${this.colorize('File:', 'cyan')} ${link.file}:${link.lineNumber}`);
            console.log(`   ${this.colorize('Type:', 'cyan')} ${link.type}`);
            console.log(`   ${this.colorize('Error:', 'red')} ${link.error}`);
            
            if (link.suggestion) {
                console.log(`   ${this.colorize('Suggestion:', 'green')} ${link.suggestion}`);
            }
    ...
    printDetailedBreakdown(breakdown) {
        ...
        Object.entries(breakdown.byType).forEach(([type, stats]) => {
            ...
            console.log(`  ${this.colorize(type.padEnd(12), 'white')}: ${stats.total} total, ${this.colorize(stats.broken + ' broken', stats.broken > 0 ? 'red' : 'green')} (${this.colorize(successRate + '%', color)})`);
        });
        ...
        Object.entries(breakdown.byStatus).forEach(([status, count]) => {
            ...
            console.log(`  ${this.colorize(status.padEnd(12), 'white')}: ${this.colorize(count, color)}`);
        });
    }
    ...
    printFooter(success, duration) {
        ...
        if (success) {
            console.log(this.colorize('🎉 All tests passed! Your site links are working perfectly.', 'green'));
        } else {
            console.log(this.colorize('❌ Tests failed. Please review and fix the broken links above.', 'red'));
        }
        
        console.log(this.colorize(`⏱ Total execution time: ${(duration / 1000).toFixed(2)}s`, 'blue'));
        console.log(this.colorize(footerLine, 'cyan'));
    }
    ...
    generateReportFiles(results, report) {
        try {
            ...
            if (results.broken > 0) {
                ...
                const textPath = path.join(this.options.outputDir, `${baseFileName}.txt`);
    ...
    generateTextReport(results, report) {
        ...
        if (report.suggestions.length > 0) {
            ...
            report.suggestions.forEach((suggestion, index) => {
                lines.push(`${index + 1}. [${suggestion.priority.toUpperCase()}] ${suggestion.category}`);
