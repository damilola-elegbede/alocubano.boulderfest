...
class CILinkChecker {
    ...
    async run() {
        ...
        try {
            ...
            if (result.success) {
                ...
                console.error(`❌ Link validation failed with ${result.results.broken} broken links`);
                process.exit(1);
            }
        ...
        try {
            ...
            if (results.broken > 0 && report.brokenLinks) {
                report.brokenLinks.forEach(link => {
                    console.log(`::error file=${link.file},line=${link.lineNumber}::Broken link: ${link.url} - ${link.error}`);
                });
            }
    ...
    generateMarkdownSummary(results, report) {
        ...
        if (results.broken === 0) {
            ...
            lines.push(`Found ${results.broken} broken link${results.broken === 1 ? '' : 's'} that need attention.`);
        }
        lines.push('');
        ...
        Object.entries(report.breakdown.byType).forEach(([type, stats]) => {
            const successRate = stats.total > 0 ? 
                ...
                '100%';
            lines.push(`| ${type} | ${stats.total} | ${stats.broken} | ${successRate} |`);
        });
        lines.push('');
    ...
    async generateJUnitReport(results, report) {
        try {
            ...
            lines.push(`<testsuites name="LinkValidation" tests="${results.total}" failures="${results.broken}" time="${duration.toFixed(3)}" timestamp="${timestamp}">`);
            lines.push(`  <testsuite name="A Lo Cubano Boulder Fest Link Validation" tests="${results.total}" failures="${results.broken}" time="${duration.toFixed(3)}">`);
            ...
            results.details.forEach((detail, index) => {
                ...
                if (detail.status === 'ok') {
                    lines.push(`    <testcase name="${testName}" classname="LinkValidation" time="0"/>`);
                } else {
                    lines.push(`    <testcase name="${testName}" classname="LinkValidation" time="0">`);
                    lines.push(`      <failure message="${detail.error}" type="${detail.status}">`);
                    lines.push(`        <![CDATA[`);
            ...
            const junitPath = path.join(this.options.outputDir, 'junit-link-validation.xml');
    ...
    async generateSummaryReport(results, report) {
        try {
            ...
            const summaryPath = path.join(this.options.outputDir, 'link-validation-summary.json');
    ...
    async generateErrorReport(error) {
        try {
            ...
            const errorPath = path.join(this.options.outputDir, 'link-validation-error.json');
