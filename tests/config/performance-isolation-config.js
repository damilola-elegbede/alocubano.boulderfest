...
export const performanceIsolationConfig = {
  ...
  validation: {
    ...
    failOnViolations:
      process.env.TEST_STRICT_MODE === "true" || process.env.CI === "true",
 ...
 */
export function getPerformanceIsolationLevel(
  testFilePath,
  testName = "",
  options = {},
) {
  ...
  if (performanceIsolationConfig.performance.caching.decisionCache.enabled) {
    ...
    if (
      ...
      Date.now() - cached.timestamp <
        performanceIsolationConfig.performance.caching.decisionCache.ttl
    ) {
...
// Initialize caches
const decisionCache = new SimpleCache(
  performanceIsolationConfig.performance.caching.decisionCache.maxSize,
);
const componentCache = new SimpleCache(
  performanceIsolationConfig.performance.caching.componentCache.maxSize,
);
...
export const performanceMonitor = {
  ...
  endTimer(testPath, operation) {
    ...
    if (duration > budget) {
      ...
      if (
        isolationInfo.performance?.critical ||
...
export const batchOperations = {
  ...
  queueSingletonOperation(operation) {
    ...
    if (
      this.singletonQueue.length >=
      performanceIsolationConfig.performance.batching.singletonOperations
        .batchSize
    ) {
  ...
  async flushSingletonQueue() {
    ...
    try {
      ...
      if (performanceIsolationConfig.debug) {
        console.log(
          `[BatchOperations] Processed ${operations.length} singleton operations in ${duration.toFixed(2)}ms`,
        );
      }
    } catch (error) {
      ...
      for (const operation of operations) {
        try {
          ...
          console.warn(
            "[BatchOperations] Individual operation failed:",
            err.message,
          );
        }
      }
...
export const componentPreloader = {
    ...
    try {
      switch (componentName) {
        ...
        case "TestSingletonManager":
          console.warn(
            "[ComponentPreloader] TestSingletonManager has been removed. Use simple helpers instead.",
          );
          return;

        case "TestMockManager":
          const { setupTestMockManager } = await import(
            "../utils/test-mock-manager.js"
          );
        ...
        case "TestEnvironmentManager":
          ...
          componentCache.set("TestEnvironmentManager", {
            backupEnv,
            restoreEnv,
          });
          break;
        ...
        default:
          console.warn(
            `[ComponentPreloader] Unknown component: ${componentName}`,
          );
          return;
      }
      ...
      if (performanceIsolationConfig.debug) {
        console.log(
          `[ComponentPreloader] Preloaded ${componentName} in ${duration.toFixed(2)}ms`,
        );
      }
    } catch (error) {
      console.warn(
        `[ComponentPreloader] Failed to preload ${componentName}:`,
        error.message,
      );
    }
  },
  ...
  async preloadAll() {
    const componentsToPreload =
      performanceIsolationConfig.performance.caching.componentCache.preload ||
      [];
