...
test("essential APIs respond", async () => {
  ...
  for (const path of criticalEndpoints) {
    ...
    if (response.status === 0)
      throw new Error(`Critical endpoint failure for GET ${path}`);
  ...
  for (const path of nonCriticalEndpoints) {
    ...
    if (response.status === 0) {
      ...
      expect([200, 400, 403, 404, 500, 503].includes(response.status)).toBe(
        true,
      );
    }
  }
  ...
  if (nonCriticalFailures > 0) {
    console.warn(
      `Test completed with ${nonCriticalFailures}/${nonCriticalEndpoints.length} non-critical endpoint failures`,
    );
  }
});
...
test("stripe webhook endpoint accessible", async () => {
  ...
  if (response.status === 0)
    throw new Error(
      `Network connectivity failure for POST /api/payments/stripe-webhook`,
    );
