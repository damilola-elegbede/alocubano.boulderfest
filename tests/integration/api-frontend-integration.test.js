...
try {
  galleryAPISource = fs.readFileSync(
    path.join(__dirname, "../../api/gallery.js"),
    "utf8",
  );
} catch (error) {
...
describe("Gallery API to Frontend Integration", () => {
          ...
          workshops: [
            {
              ...
              name: "Workshop Photo 1.jpg",
              thumbnailUrl:
                "/api/image-proxy/file1?size=thumbnail&name=Workshop%20Photo%201.jpg",
              viewUrl:
                "/api/image-proxy/file1?size=view&name=Workshop%20Photo%201.jpg",
              downloadUrl:
                "/api/image-proxy/file1?size=original&name=Workshop%20Photo%201.jpg",
            },
          ],
          socials: [
            {
              ...
              name: "Social Dance.jpg",
              thumbnailUrl:
                "/api/image-proxy/file2?size=thumbnail&name=Social%20Dance.jpg",
              viewUrl:
                "/api/image-proxy/file2?size=view&name=Social%20Dance.jpg",
              downloadUrl:
                "/api/image-proxy/file2?size=original&name=Social%20Dance.jpg",
            },
          ],
        },
        totalCount: 2,
  ...
  test("API rate limiting affects frontend behavior", async () => {
    ...
    const hasRateLimitHeaders = setHeaderCalls.some(
      (call) =>
  ...
  test("API parameter validation prevents frontend errors", async () => {
    ...
    const mockValidationHandler = async (req, res) => {
      ...
      res
        .status(200)
        .json({ categories: {}, totalCount: 0, timestamp: Date.now() });
    };
          ...
          workshops: [
            {
              ...
              thumbnailUrl:
                "/api/image-proxy/cached1?size=thumbnail&name=Cached%20Workshop.jpg",
              viewUrl:
                "/api/image-proxy/cached1?size=view&name=Cached%20Workshop.jpg",
              downloadUrl:
                "/api/image-proxy/cached1?size=original&name=Cached%20Workshop.jpg",
            },
          ],
          socials: [],
        },
  ...
  test("API image URL generation works with frontend expectations", async () => {
    ...
    expect(item.thumbnailUrl).toBe(
      "/api/image-proxy/test-file-123?size=thumbnail&name=Test%20Image.jpg",
    );
    expect(item.viewUrl).toBe(
      "/api/image-proxy/test-file-123?size=view&name=Test%20Image.jpg",
    );
    expect(item.downloadUrl).toBe(
      "/api/image-proxy/test-file-123?size=original&name=Test%20Image.jpg",
    );
  });
