...
describe('Email Subscription Integration Tests', () => {
    ...
    describe('POST /api/email/subscribe', () => {
        ...
        it('should handle duplicate email subscription', async () => {
            ...
            const responseData = JSON.parse(res._getData());
            expect(responseData.error).toBe('This email address is already subscribed to our newsletter');
        });
        ...
        it('should handle Brevo API errors', async () => {
            ...
            const responseData = JSON.parse(res._getData());
            expect(responseData.error).toBe('Email service temporarily unavailable. Please try again later.');
        });
        ...
        it('should handle verification email flow', async () => {
            ...
            const responseData = JSON.parse(res._getData());
            expect(responseData.message).toBe('Please check your email to verify your subscription');
            expect(responseData.subscriber.requiresVerification).toBe(true);
            
            expect(mockEmailService.brevoService.sendVerificationEmail).toHaveBeenCalled();
        });
    });
    ...
    describe('GET/POST /api/email/unsubscribe', () => {
        ...
        it('should unsubscribe via GET request with valid token', async () => {
            ...
            expect(mockEmailService.validateUnsubscribeToken).toHaveBeenCalledWith('test@example.com', 'valid-token');
            expect(mockEmailService.unsubscribeSubscriber).toHaveBeenCalledWith('test@example.com', 'user_request');
        });
        ...
        it('should unsubscribe via POST request with valid token', async () => {
            ...
            const responseData = JSON.parse(res._getData());
            expect(responseData.success).toBe(true);
            expect(responseData.message).toBe('Successfully unsubscribed from newsletter');
        ...
        it('should handle subscriber not found error', async () => {
            ...
            const responseData = JSON.parse(res._getData());
            expect(responseData.error).toBe('Email address not found or already unsubscribed');
        });
