...
import { setupIntegrationTests } from "../utils/enhanced-test-setup.js";
import { testInit } from "../utils/test-initialization-helpers.js";

describe("Comprehensive API Integration", () => {
  const testSetup = setupIntegrationTests({
    enableDatabase: true,
    enableMocks: false, // Test with real services
    timeout: 40000,
    cleanDatabase: true,
  });
  ...
  beforeAll(async () => {
    services = testSetup.getServices();
    ...
    // Wait for services to be ready before adding routes
    await testInit.waitForService("database", 20000);
