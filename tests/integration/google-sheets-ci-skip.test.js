...
describe("Google Sheets Analytics Integration (CI)", () => {
  beforeEach(() => {
    ...
    if (process.env.CI === "true") {
      console.log(
        "⏭️ Skipping Google Sheets integration tests in CI environment",
      );
      console.log(
        "   Reason: Prevents database locking and resource conflicts",
      );
  ...
  it("should skip Google Sheets tests in CI environment", () => {
    ...
    console.log(
      "🔧 In development: Run full Google Sheets tests from google-sheets.test.js",
    );
    expect(true).toBe(true);
  });
