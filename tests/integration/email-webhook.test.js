...
describe("Brevo Webhook Integration Tests", () => {
  ...
  describe("POST /api/email/brevo-webhook", () => {
    it("should process valid webhook event", async () => {
      ...
      expect(mockEmailService.processWebhookEvent).toHaveBeenCalledWith(
        webhookData,
      );
    });
    ...
    it("should reject webhook with invalid signature", async () => {
      ...
      const { req, res } = createWebhookRequest(
        webhookData,
        "invalid-signature",
      );
    ...
    it("should handle different event types with appropriate messages", async () => {
      const eventTests = [
        ...
        {
          event: "hard_bounce",
          expectedMessage: "Hard bounce processed, contact marked as bounced",
        },
        {
          event: "spam",
          expectedMessage:
        ...
        {
          event: "invalid_email",
          expectedMessage: "Invalid email processed, contact marked as bounced",
        },
        {
          event: "unknown_event",
          expectedMessage: "Unknown event type processed: unknown_event",
        },
      ];
    ...
    it("should reject webhook missing required fields", async () => {
      ...
      expect(responseData.error).toBe(
        "Missing required webhook fields (event, email)",
      );
    });
