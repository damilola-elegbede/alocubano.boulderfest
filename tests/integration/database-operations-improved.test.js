...
describe.skipIf(shouldSkipInCI)(
  "Database Operations Integration - HTTP Testing",
  () => {
    let app;
    let server;
    ...
    beforeAll(async () => {
      ...
      try {
        ...
        await fs.promises
          .unlink("./test-integration-shared.db")
          .catch(() => {});
    ...
    describe("Database Health via HTTP", () => {
      beforeEach(async () => {
        ...
        `);
          ...
          console.warn(
            "⚠️ Database setup failed in beforeEach:",
            error.message,
          );
        }
      });
    ...
    describe("Performance Testing via HTTP", () => {
      it("should handle multiple health check requests efficiently", async () => {
        ...
        const requests = Array(10)
          .fill()
          .map(() => request(app).get("/api/health/database"));
