...
class LinkChecker {
  ...
  shouldSkipUrl(url, context = "") {
    ...
    if (
      context.includes('rel="dns-prefetch"') ||
    ...
    try {
      ...
      if (!resolvedPath) {
        return {
          ...
          suggestion:
            "Check if the path is correct relative to the current file",
        };
      }
  ...
  checkServerRouting(url, visitedRoutes = new Set(), depth = 0) {
    ...
    if (url.startsWith("/api/")) {
      ...
      const isValidApi =
        apiEndpoints.includes(url) || url.startsWith("/api/image-proxy/");
    ...
    if (vercelRedirects[url]) {
      ...
      if (redirectTarget.startsWith("/pages/")) {
        ...
        return this.checkServerRouting(
          redirectTarget,
          visitedRoutes,
          depth + 1,
        );
  ...
  suggestFixWithRouting(url, resolvedPath) {
    ...
    if (url.startsWith("/")) {
      ...
      if (!this.hasFileExtension(url)) {
        ...
        if (fs.existsSync(htmlPath)) {
          suggestions.push(
            `Server will route ${url} to ${url}.html automatically`,
          );
        }
      }
      ...
      if (fs.existsSync(pagesPath)) {
        suggestions.push(
          `File exists in pages directory - server routing should handle this`,
        );
      }
    }
    ...
    return suggestions.length > 0
      ? suggestions[0]
      : "Check file path and server routing configuration";
  }
  ...
  async generateReport() {
    ...
    const successRate =
      this.results.total > 0
        ? (
            ((this.results.total - this.results.broken) / this.results.total) *
            100
          ).toFixed(1)
        : 100;
  ...
  getBrokenLinksReport() {
    return this.results.details
      .filter(
        (detail) => detail.status === "broken" || detail.status === "error",
      )
      .map((detail) => ({
        url: detail.url,
        type: detail.type,
  ...
  calculateHealthScore() {
    ...
    const baseScore =
      ((this.results.total - this.results.broken) / this.results.total) * 100;
