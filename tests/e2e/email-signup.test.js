...
test.describe('Email Newsletter Signup E2E', () => {
    ...
    test('should display newsletter signup form', async ({ page }) => {
        ...
        await expect(page.locator('#newsletter-email')).toHaveAttribute('placeholder', 'YOUR EMAIL ADDRESS');
        await expect(page.locator('.newsletter-submit .button-text')).toHaveText('SUBSCRIBE');
        await expect(page.locator('.checkbox-label')).toContainText('I agree to receive marketing emails');
    });
    
    test('should successfully subscribe to newsletter with valid data', async ({ page }) => {
        ...
        await expect(page.locator('.newsletter-success .success-text')).toHaveText('Welcome to the A Lo Cubano family!');
    ...
    test('should validate email format', async ({ page }) => {
        ...
        await expect(page.locator('.newsletter-input-wrapper')).toHaveClass(/error/);
    ...
    test('should require email address', async ({ page }) => {
        ...
        // Check for error message
        await expect(page.locator('#newsletter-error')).toBeVisible();
        await expect(page.locator('#newsletter-error')).toContainText('Please enter your email address');
    });
    ...
    test('should clear errors when user corrects input', async ({ page }) => {
        ...
        await expect(page.locator('.newsletter-input-wrapper')).not.toHaveClass(/error/);
        await expect(page.locator('#newsletter-email')).toHaveAttribute('aria-invalid', 'false');
    });
    ...
    test('should show loading state during submission', async ({ page }) => {
        ...
        // Check loading state
        await expect(page.locator('.newsletter-submit')).toHaveAttribute('aria-busy', 'true');
        ...
        await expect(page.locator('.newsletter-submit')).toHaveAttribute('aria-busy', 'false');
    ...
    test('should handle server errors gracefully', async ({ page }) => {
        ...
        // Check for error message
        await expect(page.locator('#newsletter-error')).toBeVisible();
        await expect(page.locator('#newsletter-error')).toContainText('error occurred');
    });
    ...
    test('should auto-hide success message after 10 seconds', async ({ page }) => {
        ...
        await expect(page.locator('.newsletter-success')).toHaveAttribute('aria-hidden', 'true');
    });
    ...
    test('should track Google Analytics event on successful signup', async ({ page }) => {
        // Mock gtag function
    ...
    test('should be mobile responsive', async ({ page }) => {
        ...
        const flexDirection = await inputWrapper.evaluate(el => 
