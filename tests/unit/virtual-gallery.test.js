...
class VirtualGalleryManager {
    ...
    recycleItems() {
        this.visibleItems.forEach((item, index) => {
            const itemTop = Math.floor(index / this.state.itemsPerRow) * this.config.itemHeight;
            const itemBottom = itemTop + this.config.itemHeight;
            
            if (itemBottom < this.state.viewportStart || itemTop > this.state.viewportEnd) {
    ...
    renderItems() {
        const startRow = Math.floor(this.state.viewportStart / this.config.itemHeight);
    ...
    destroy() {
        if (this.observers.intersection && typeof this.observers.intersection.disconnect === 'function') {
            this.observers.intersection.disconnect();
        }
        if (this.observers.resize && typeof this.observers.resize.disconnect === 'function') {
...
describe('Virtual Gallery Manager', () => {
    ...
    describe('Layout Calculations', () => {
        ...
        test('should update content height when photos are loaded', () => {
            ...
            const expectedRows = Math.ceil(mockPhotos.length / gallery.state.itemsPerRow);
    ...
    describe('Virtual Scrolling and DOM Recycling', () => {
        beforeEach(() => {
            gallery = new VirtualGalleryManager(container, { itemHeight: 200, bufferSize: 2 });
            gallery.loadPhotos(mockPhotos);
        });
        ...
        test('should update visible range on scroll', () => {
            ...
            expect(gallery.state.viewportEnd).toBeGreaterThan(gallery.state.viewportStart);
        });
        ...
        test('should recycle items outside viewport', () => {
            ...
            expect(gallery.getPooledItemsCount() + gallery.getVisibleItemsCount()).toBeGreaterThan(0);
        });
    });
