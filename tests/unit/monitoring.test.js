...
describe('Health Checker', () => {
  ...
  describe('Health Check Registration', () => {
    ...
    test('should register critical health check', () => {
      ...
      healthChecker.registerCheck('critical-service', checkFn, { critical: true });
  ...
  describe('Health Check Execution', () => {
    ...
    test('should handle health check failure', async () => {
      const checkFn = vi.fn().mockRejectedValue(new Error('Service unavailable'));
  ...
  describe('Overall Health Status', () => {
    test('should aggregate health status correctly', async () => {
      const healthyCheck = vi.fn().mockResolvedValue({ status: HealthStatus.HEALTHY });
      const degradedCheck = vi.fn().mockResolvedValue({ status: HealthStatus.DEGRADED });
      const unhealthyCheck = vi.fn().mockResolvedValue({ status: HealthStatus.UNHEALTHY });
      
      healthChecker.registerCheck('service1', healthyCheck);
      healthChecker.registerCheck('service2', degradedCheck);
      healthChecker.registerCheck('service3', unhealthyCheck, { critical: false });
    ...
    test('should mark overall status as unhealthy if critical service fails', async () => {
      const healthyCheck = vi.fn().mockResolvedValue({ status: HealthStatus.HEALTHY });
      const criticalFailure = vi.fn().mockResolvedValue({ status: HealthStatus.UNHEALTHY });
      
      healthChecker.registerCheck('service1', healthyCheck);
      healthChecker.registerCheck('critical-service', criticalFailure, { critical: true });
...
describe('Performance Tracker', () => {
  ...
  describe('Metrics Collection', () => {
    ...
    test('should calculate percentiles correctly', async () => {
      // Add 100 samples
      for (let i = 1; i <= 100; i++) {
        await performanceTracker.trackEndpointPerformance('/api/test', i * 10, 200);
      }
  ...
  describe('Baseline Tracking', () => {
    test('should establish baseline after sufficient samples', async () => {
      // Add 100 samples
      for (let i = 0; i < 100; i++) {
        await performanceTracker.trackEndpointPerformance('/api/test', 100 + i, 200);
      }
    ...
    test('should detect performance regression', async () => {
      ...
      // Add slow responses
      for (let i = 0; i < 10; i++) {
        await performanceTracker.trackEndpointPerformance('/api/test', 500, 200);
      }
      
      const stats = performanceTracker.metricsStore.getEndpointStats('/api/test');
      const regressions = performanceTracker.baselineTracker.checkRegression('/api/test', stats);
  ...
  describe('Performance Reports', () => {
    ...
    test('should generate recommendations for slow endpoints', async () => {
      ...
      const perfRecommendation = report.recommendations.find(r => 
        r.type === 'performance' && r.endpoint === '/api/slow'
      );
...
describe('Sentry Configuration', () => {
  test('should sanitize sensitive data in error events', async () => {
    const { sanitizeEvent } = await import('../../lib/monitoring/sentry-config.js');
