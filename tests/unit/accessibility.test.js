...
describe("Lightbox Accessibility", () => {
  ...
  test("lightbox has proper ARIA labels", () => {
    ...
    expect(lightbox.getAttribute("aria-describedby")).toBe(
  ...
  test("keyboard navigation works correctly", () => {
    if (global.window.Lightbox) {
      ...
      lightbox.openAdvanced(mockItems, 0, ["accessibility"], {
        accessibility: 2,
      });
  ...
  test("focus trap functions properly", () => {
    ...
    const handleTabNavigation = (event) => {
      if (event.key === "Tab") {
        if (event.shiftKey) {
          currentFocus =
            currentFocus > 0 ? currentFocus - 1 : focusableElements.length - 1;
        } else {
          currentFocus =
            currentFocus < focusableElements.length - 1 ? currentFocus + 1 : 0;
        }
    ...
    if (global.window.Lightbox) {
      const lightbox = new global.window.Lightbox();
      const mockItems = [
        { id: "focus-test", viewUrl: "focus.jpg", name: "Focus Test" },
      ];
  ...
  test("screen reader announcements work correctly", () => {
    if (global.window.Lightbox) {
      ...
      lightbox.openAdvanced(mockItems, 0, ["screenreader"], {
        screenreader: 2,
      });
...
describe("Gallery Accessibility", () => {
  ...
  test("gallery images have proper alt text", () => {
    ...
    images.forEach((img, index) => {
      ...
      const hasDescriptiveWords = /workshop|dance|salsa|students|couples/i.test(
        altText,
      );
