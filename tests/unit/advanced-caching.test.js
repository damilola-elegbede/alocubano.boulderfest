...
describe('Advanced Caching System - Phase 2', () => {
  ...
  describe('Service Worker Cache Strategies', () => {
    test('should implement cache-first strategy for images', async () => {
      ...
      mockFetch.mockResolvedValueOnce(createMockResponse('image-data', {
    ...
    test('should handle cache metadata correctly', async () => {
      ...
      const cachedResponse = await cache.match(request);
      expect(cachedResponse.headers.get('sw-cached-at')).toBe(metadata.cachedAt.toString());
  ...
  describe('Prefetch Manager Connection Constraints', () => {
    test('should detect connection capabilities correctly', () => {
      const detectConnectionCapabilities = () => {
        ...
        if (connection) {
          ...
          capabilities.qualityTier = score < 2 ? 'low' : score < 6 ? 'medium' : 'high';
    ...
    test('should calculate resource budget based on connection and device', () => {
      ...
      const budget = calculateResourceBudget(connectionInfo, deviceCapabilities);
        ...
        return {
          ...
          getNext: () => {
            ...
            for (const priority of priorities) {
              ...
              if (resources.size > 0) {
                const resourceInfo = JSON.parse(resources.values().next().value);
  ...
  describe('Cache Warming Queue Prioritization', () => {
    test('should determine warming strategy based on connection', () => {
      ...
      expect(determineStrategy({ effectiveType: '4g', downlink: 10, saveData: false })).toBe('aggressive');
      expect(determineStrategy({ effectiveType: '3g', downlink: 3, saveData: false })).toBe('conservative');
      expect(determineStrategy({ effectiveType: '2g', downlink: 1, saveData: false })).toBe('minimal');
      expect(determineStrategy({ effectiveType: '4g', downlink: 10, saveData: true })).toBe('minimal');
    });
    ...
    test('should implement progressive warming phases', () => {
      ...
      expect(aggressiveConfig.phases).toEqual(['critical', 'essential', 'predictive']);
  ...
  describe('Hero Image API Functionality', () => {
    test('should validate hero image API parameters', () => {
      const validateHeroImageParams = (query) => {
      ...
      const validParams = validateHeroImageParams('w=800&h=600&format=webp&q=90');
    ...
    test('should generate responsive image URLs', () => {
      const generateResponsiveUrls = (baseUrl, breakpoints) => {
        return breakpoints.map(bp => ({
          width: bp.width,
          url: `${baseUrl}?w=${bp.width}&q=${bp.quality || 85}&format=${bp.format || 'webp'}`,
          descriptor: `${bp.width}w`
        }));
      };
      ...
      const breakpoints = [
        ...
        { width: 1200, quality: 90, format: 'webp' }
      ];
      
      const urls = generateResponsiveUrls('/api/hero-image', breakpoints);
      
      expect(urls).toHaveLength(3);
      expect(urls[0].url).toBe('/api/hero-image?w=320&q=75&format=webp');
      expect(urls[1].url).toBe('/api/hero-image?w=768&q=85&format=webp');
      expect(urls[2].url).toBe('/api/hero-image?w=1200&q=90&format=webp');
    });
    
    test('should handle hero image caching with proper headers', () => {
      const createHeroImageResponse = (imageData, params) => {
        const headers = {
          'Content-Type': `image/${params.format}`,
          'Cache-Control': 'public, max-age=86400', // 24 hours
          'ETag': `"hero-${params.width}x${params.height}-${params.quality}"`,
          'Vary': 'Accept'
        };
        
        if (params.format === 'webp') {
          headers['Content-Encoding'] = 'webp';
        }
        
        return createMockResponse(imageData, { headers });
      };
      
      const params = { width: 800, height: 600, format: 'webp', quality: 90 };
      const response = createHeroImageResponse('image-binary-data', params);
      
      expect(response.headers.get('Content-Type')).toBe('image/webp');
      expect(response.headers.get('Cache-Control')).toBe('public, max-age=86400');
      expect(response.headers.get('ETag')).toContain('hero-800x600-90');
    });
  });
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    test('should handle connection changes during operations', () => {
      const handleConnectionChange = (oldConnection, newConnection, operations) => {
      ...
      const filteredOps = handleConnectionChange(highConnection, lowConnection, operations);
      ...
      const manageCacheSize = async (cacheName, maxSize) => {
        ...
        for (const key of keys) {
          ...
          if (response) {
            ...
            const cachedAt = parseInt(response.headers.get('sw-cached-at') || Date.now().toString());
      ...
      const createMetricsCollector = () => {
        ...
        return {
          recordCacheHit: (hit) => {
            ...
            metrics.cacheHitRate = samples.cacheHits.filter(Boolean).length / samples.cacheHits.length;
          },
          
          recordPrefetchResult: (success) => {
            samples.prefetchResults.push(success);
            metrics.prefetchSuccessRate = samples.prefetchResults.filter(Boolean).length / samples.prefetchResults.length;
          },
          
          recordLoadTime: (time) => {
            samples.loadTimes.push(time);
            metrics.averageLoadTime = samples.loadTimes.reduce((a, b) => a + b, 0) / samples.loadTimes.length;
          },
          
          recordBandwidth: (bytes) => {
            samples.bandwidthSamples.push(bytes);
            metrics.bandwidthUsage = samples.bandwidthSamples.reduce((a, b) => a + b, 0);
          },
    ...
    test('should handle cross-component error propagation', () => {
      const createErrorHandler = () => {
        ...
        return {
          ...
          getErrors: (component) => {
            return component ? errors.filter(e => e.component === component) : errors;
          }
        };
      };
      ...
      errorHandler.reportError('service-worker', new Error('Cache failed'), { url: '/test.js' });
      errorHandler.reportError('prefetch-manager', new Error('Network error'), { priority: 'high' });
    ...
    test('should monitor cache performance metrics', () => {
      const createPerformanceMonitor = () => {
        ...
        return {
          ...
          getStats: () => ({
            ...metrics,
            hitRate: metrics.requests > 0 ? metrics.cacheHits / metrics.requests : 0,
            averageLoadTime: metrics.requests > 0 ? metrics.totalLoadTime / metrics.requests : 0,
      ...
      const createAdaptiveStrategy = () => {
        ...
        return {
          updatePerformance: function(hitRate, loadTime, errorRate) {
            performanceHistory.push({ hitRate, loadTime, errorRate, timestamp: Date.now() });
          ...
          adaptStrategy: function() {
            ...
            const avgHitRate = recent.reduce((sum, p) => sum + p.hitRate, 0) / recent.length;
          ...
          getStrategyConfig: () => {
            const configs = {
              conservative: { maxConcurrent: 2, cacheSize: '5MB', prefetchDistance: 1 },
              balanced: { maxConcurrent: 4, cacheSize: '10MB', prefetchDistance: 3 },
              aggressive: { maxConcurrent: 8, cacheSize: '25MB', prefetchDistance: 5 }
            };
