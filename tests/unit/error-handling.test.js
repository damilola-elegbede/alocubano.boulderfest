...
describe('Lightbox Error Scenarios', () => {
  ...
  test('handles corrupted image data', async () => {
    ...
    if (global.window.Lightbox) {
      ...
      const corruptedItems = [
        { id: 'corrupt1', viewUrl: 'data:image/jpeg;base64,invalid-data', name: 'Corrupted Image' }
      ];
      
      // Test opening lightbox with corrupted image
      expect(() => {
        lightbox.openAdvanced(corruptedItems, 0, ['corrupted'], { corrupted: 1 });
      }).not.toThrow();
      ...
      const testKeyboardRobustness = () => {
        ...
        keyEvents.forEach(key => {
          ...
          try {
            ...
            const isValidKey = ['ArrowRight', 'ArrowLeft', 'Enter', 'Escape'].includes(key);
  ...
  test('handles empty or invalid item arrays', () => {
    if (global.window.Lightbox) {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(Error);
        console.warn('Lightbox empty array handling needs improvement:', error.message);
      }
      
      // Test valid items work correctly
      const validItems = [{ id: 'valid1', viewUrl: 'valid1.jpg', name: 'Valid' }];
  ...
  test('handles concurrent lightbox operations', async () => {
    if (global.window.Lightbox) {
      ...
      const items1 = [{ id: 'conc1', viewUrl: 'conc1.jpg', name: 'Concurrent 1' }];
      const items2 = [{ id: 'conc2', viewUrl: 'conc2.jpg', name: 'Concurrent 2' }];
