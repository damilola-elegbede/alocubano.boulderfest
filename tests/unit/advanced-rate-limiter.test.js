...
import {
  AdvancedRateLimiter,
  getRateLimiter,
} from "../../api/lib/security/rate-limiter.js";
import {
  withRateLimit,
  paymentRateLimit,
  authRateLimit,
...
describe("Advanced Rate Limiter", () => {
  ...
  describe("Performance Requirements", () => {
    it("should process requests within 5ms performance target", async () => {
      ...
      console.log(
        `Performance stats: avg=${avgTime}ms, max=${maxTime}ms, p95=${p95Time}ms`,
      );
    ...
    it("should handle concurrent requests efficiently", async () => {
      ...
      console.log(
        `Concurrent processing: ${totalTime}ms total, ${avgTimePerRequest}ms per request`,
      );

      // Should process all requests and most should be allowed
      expect(results).toHaveLength(concurrentRequests);
      expect(results.filter((r) => r.allowed).length).toBeGreaterThan(
  ...
  describe("Progressive Penalties", () => {
    it("should apply penalties for repeated violations", async () => {
      ...
      let lastResult;
      for (let i = 0; i <= 5; i++) {
        // 6 requests to exceed the 5 req/min limit
      ...
      if (!lastResult.allowed && lastResult.reason === "rate_limit_exceeded") {
        let multiplier = await rateLimiter.getPenaltyMultiplier(
          clientId,
          "auth",
        );
        ...
        const newMultiplier = await rateLimiter.getPenaltyMultiplier(
          clientId,
          "auth",
        );
  ...
  describe("Endpoint-Specific Configurations", () => {
    it("should apply different limits for different endpoints", async () => {
      ...
      const paymentBlocked = await rateLimiter.checkRateLimit(
        testReq,
        "payment",
      );
      ...
      const generalAllowed = await rateLimiter.checkRateLimit(
        testReq,
        "general",
      );
  ...
  describe("Error Handling", () => {
    it("should fail open on errors", async () => {
      ...
      const originalCheckSlidingWindow =
        rateLimiter.checkSlidingWindow.bind(rateLimiter);
      rateLimiter.checkSlidingWindow = vi
        .fn()
