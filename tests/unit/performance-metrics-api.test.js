...
describe("Performance Metrics API", () => {
  ...
  describe("Request Validation", () => {
    ...
    test("should validate Core Web Vitals ranges", () => {
      ...
      // Valid ranges
      expect(
        validateCoreWebVitals({ lcp: 2500, fid: 100, cls: 0.1 }).isValid,
      ).toBe(true);
      ...
      const testMetrics = [
        {
          timestamp: Date.now(),
          page: "/test",
          metrics: { lcp: 2000, fid: 50 },
        },
        {
          timestamp: Date.now(),
          page: "/test",
          metrics: { lcp: 3000, fid: 75 },
        },
        {
          timestamp: Date.now(),
          page: "/test",
          metrics: { lcp: 2500, fid: 100 },
        },
      ];
  ...
  describe("Error Handling", () => {
    ...
    test("should validate request size limits", () => {
      const validateRequestSize = (body, maxSize = 1024 * 1024) => {
        // 1MB default
      ...
      const generateInsights = (metrics) => {
        ...
        if (metrics.cls > 0.25) {
          insights.push({
            ...
            suggestion:
              "Add size attributes to images and reserve space for content",
          });
        }
    ...
    test("should track trends over time", () => {
      const trackTrends = (historicalData) => {
        ...
        Object.keys(latest.metrics).forEach((key) => {
          if (
            typeof latest.metrics[key] === "number" &&
