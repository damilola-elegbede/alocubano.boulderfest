...
describe('CORS Configuration Performance Optimizations', () => {
  ...
  it('should cache CORS configuration to avoid repeated disk reads', async () => {
    ...
    const { getCorsConfig, clearConfigCache } = await import('../../api/lib/cors-config.js');
    ...
    expect(config1.allowedOrigins).toEqual(["http://localhost:3000", "https://test.example.com"]);
  });
  ...
  it('should invalidate cache when environment variable changes', async () => {
    ...
    process.env.CORS_ALLOWED_ORIGINS = "https://new.example.com,https://another.example.com";
    const config2 = getCorsConfig();
    ...
    expect(config2.allowedOrigins).toEqual(["https://new.example.com", "https://another.example.com"]);
