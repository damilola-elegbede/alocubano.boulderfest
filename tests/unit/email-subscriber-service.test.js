...
describe('EmailSubscriberService', () => {
    ...
    describe('createSubscriber', () => {
        ...
        it('should handle duplicate email error', async () => {
            ...
            await expect(emailService.createSubscriber(subscriberData))
        ...
        it('should log email event and audit trail', async () => {
            ...
            const logEmailEventSpy = vi.spyOn(emailService, 'logEmailEvent')
                .mockResolvedValue({ id: 1 });
            const auditLogSpy = vi.spyOn(emailService, 'auditLog')
                .mockResolvedValue({ id: 1 });
    ...
    describe('getSubscriberByEmail', () => {
        it('should return subscriber data', async () => {
            const result = await emailService.getSubscriberByEmail('test@example.com');
    ...
    describe('updateSubscriber', () => {
        it('should update subscriber with provided data', async () => {
            ...
            const result = await emailService.updateSubscriber('test@example.com', updateData);
        ...
        it('should throw error when no update data provided', async () => {
            await expect(emailService.updateSubscriber('test@example.com', {}))
    ...
    describe('unsubscribeSubscriber', () => {
        it('should unsubscribe user in Brevo and database', async () => {
            ...
            const result = await emailService.unsubscribeSubscriber('test@example.com');
            
            expect(mockBrevoService.unsubscribeContact).toHaveBeenCalledWith('test@example.com');
            expect(logEmailEventSpy).toHaveBeenCalledWith(
                expect.any(Number),
                'unsubscribed',
                { reason: 'user_request' }
    ...
    describe('verifySubscriber', () => {
        it('should verify subscriber with valid token', async () => {
            ...
            vi.spyOn(emailService, 'getSubscriberByEmail')
                .mockResolvedValue(mockSubscriber);
            
            const logEmailEventSpy = vi.spyOn(emailService, 'logEmailEvent')
                .mockResolvedValue({ id: 1 });
            
            const result = await emailService.verifySubscriber('test@example.com', 'valid-token');
        ...
        it('should reject invalid verification token', async () => {
            ...
            await expect(emailService.verifySubscriber('test@example.com', 'invalid-token'))
        ...
        it('should reject already verified email', async () => {
            ...
            await expect(emailService.verifySubscriber('test@example.com', 'valid-token'))
    ...
    describe('processWebhookEvent', () => {
        it('should process webhook event and log it', async () => {
            ...
            expect(mockBrevoService.processWebhookEvent).toHaveBeenCalledWith(webhookData);
            expect(logEmailEventSpy).toHaveBeenCalledWith(
                expect.any(Number),
                'opened',
                {},
        ...
        it('should update subscriber status for bounce events', async () => {
            ...
            const updateSubscriberSpy = vi.spyOn(emailService, 'updateSubscriber')
                .mockResolvedValue({});
