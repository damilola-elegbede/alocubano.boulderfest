...
describe("API Input Validation", () => {
  ...
  test("validates gallery request parameters", () => {
    ...
    // Test invalid inputs
    expect(validateYear("invalid")).toEqual({
      valid: false,
      error: "Year must be a valid number",
    });
    expect(validateYear("1999")).toEqual({
      valid: false,
      error: "Year must be between 2020 and 2026",
    });
    expect(validateLimit("0")).toEqual({
      valid: false,
      error: "Limit must be between 1 and 100",
    });
    expect(validateLimit("200")).toEqual({
      valid: false,
      error: "Limit must be between 1 and 100",
    });
    expect(validateOffset("-1")).toEqual({
      valid: false,
  ...
  test("prevents SQL injection attempts", () => {
    ...
    maliciousInputs.forEach((input) => {
      expect(() => preventSQLInjection(input)).toThrow(
...
describe("Data Integrity and Validation", () => {
  test("validates API response data structure", () => {
    ...
    const validateGalleryResponse = (response) => {
      ...
      if (response.timestamp !== undefined) {
        if (
          ...
          isNaN(new Date(response.timestamp))
        ) {
    ...
    const invalidResponses = [
      ...
      {
        categories: { workshops: "not-an-array" },
        totalCount: 0,
        timestamp: Date.now(),
      },
      {
        categories: { workshops: [{ name: "test.jpg" }] },
        totalCount: 0,
        timestamp: Date.now(),
      }, // Missing id
