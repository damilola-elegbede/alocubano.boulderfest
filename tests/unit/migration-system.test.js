...
describe('MigrationSystem', () => {
  ...
  describe('initializeMigrationsTable', () => {
    it('should create migrations table successfully', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith('✅ Migrations table initialized');
    });
    ...
    it('should handle database errors when creating migrations table', async () => {
      ...
      await expect(migrationSystem.initializeMigrationsTable()).rejects.toThrow(error);
  ...
  describe('getExecutedMigrations', () => {
    ...
    it('should handle database errors when getting executed migrations', async () => {
      ...
      await expect(migrationSystem.getExecutedMigrations()).rejects.toThrow(error);
      expect(console.error).toHaveBeenCalledWith(
  ...
  describe('getAvailableMigrations', () => {
    it('should return sorted list of SQL migration files', async () => {
      migrationSystem.addMockFile('002_create_posts.sql', 'CREATE TABLE posts (id INTEGER);');
      migrationSystem.addMockFile('001_create_users.sql', 'CREATE TABLE users (id INTEGER);');
      migrationSystem.addMockFile('not_a_migration.txt', 'This is not a migration');
      migrationSystem.addMockFile('003_create_comments.sql', 'CREATE TABLE comments (id INTEGER);');
    ...
    it('should handle filesystem errors when reading directory', async () => {
      ...
      migrationSystem._mockReaddir = vi.fn().mockRejectedValue(new Error('Permission denied'));

      await expect(migrationSystem.getAvailableMigrations()).rejects.toThrow('Permission denied');
      expect(console.error).toHaveBeenCalledWith(
...
`;
    ...
    it('should handle file read errors', async () => {
      await expect(migrationSystem.readMigrationFile('missing.sql')).rejects.toThrow();
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining('❌ Failed to read migration file missing.sql:'),
        expect.any(String)
      );
    });
  });
...
`;
      ...
      expect(sqlStatements).toHaveLength(2);
      expect(sqlStatements.some(stmt => stmt.includes('CREATE TABLE users'))).toBe(true);
      expect(sqlStatements.some(stmt => stmt.includes('INSERT INTO users'))).toBe(true);
    });
  });
  ...
  describe('executeMigration', () => {
    it('should execute all statements in migration and record it', async () => {
      ...
      expect(mockDatabase.execute).toHaveBeenCalledWith('INSERT INTO test VALUES (1)');
      expect(mockDatabase.execute).toHaveBeenCalledWith(
        'INSERT INTO migrations (filename, checksum) VALUES (?, ?)',
        ['001_test.sql', expect.any(String)]
      );
      expect(console.log).toHaveBeenCalledWith('🔄 Executing migration: 001_test.sql');
      expect(console.log).toHaveBeenCalledWith('✅ Migration completed: 001_test.sql');
    });
    ...
    it('should handle SQL execution errors', async () => {
      ...
      await expect(migrationSystem.executeMigration(migration)).rejects.toThrow(error);
      expect(console.error).toHaveBeenCalledWith(
  ...
  describe('runMigrations', () => {
    it('should execute all pending migrations', async () => {
      ...
      // Set up mock files
      migrationSystem.addMockFile('001_existing.sql', 'CREATE TABLE existing (id INTEGER);');
      migrationSystem.addMockFile('002_new.sql', 'CREATE TABLE new (id INTEGER);');
      migrationSystem.addMockFile('003_another.sql', 'CREATE TABLE another (id INTEGER);');
      ...
      expect(console.log).toHaveBeenCalledWith('🚀 Starting database migrations...');
      expect(console.log).toHaveBeenCalledWith('📋 Found 2 pending migrations:');
      expect(console.log).toHaveBeenCalledWith('🎉 Successfully executed 2 migrations');
    });
    ...
    it('should handle case when no migrations are pending', async () => {
      ...
      // Set up mock files
      migrationSystem.addMockFile('001_existing.sql', 'CREATE TABLE existing1 (id INTEGER);');
      migrationSystem.addMockFile('002_existing.sql', 'CREATE TABLE existing2 (id INTEGER);');

      const result = await migrationSystem.runMigrations();

      expect(result).toEqual({ executed: 0, skipped: 2 });
      expect(console.log).toHaveBeenCalledWith('✨ No pending migrations found');
    });
  ...
  describe('verifyMigrations', () => {
    it('should verify migration integrity successfully', async () => {
      ...
      migrationSystem.addMockFile('001_test.sql', 'CREATE TABLE test (id INTEGER);');
      
      // Mock checksum query
      const expectedChecksum = await migrationSystem.generateChecksum('CREATE TABLE test (id INTEGER);');
      mockDatabase.execute.mockResolvedValueOnce({ 
      ...
      expect(console.log).toHaveBeenCalledWith('✅ All migrations verified successfully');
    });
    ...
    it('should detect missing migration files', async () => {
      ...
      expect(console.warn).toHaveBeenCalledWith('⚠️  Warning: Some executed migrations no longer exist:');
    ...
    it('should detect checksum mismatches', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('❌ Checksum mismatch for 001_test.sql');
    });
  ...
  describe('status', () => {
    it('should display migration status correctly', async () => {
      ...
      // Set up mock files
      migrationSystem.addMockFile('001_test.sql', 'CREATE TABLE test (id INTEGER);');
      migrationSystem.addMockFile('002_pending.sql', 'CREATE TABLE pending (id INTEGER);');
      migrationSystem.addMockFile('003_another_pending.sql', 'CREATE TABLE another (id INTEGER);');
  ...
  describe('Error Handling', () => {
    ...
    it('should handle database connection timeouts', async () => {
      ...
      await expect(migrationSystem.initializeMigrationsTable()).rejects.toThrow('Connection timeout');
    });
    ...
    it('should handle disk space errors', async () => {
      ...
      await expect(migrationSystem.runMigrations()).rejects.toThrow('No space left on device');
    });
  });
...
`;
    ...
    it('should handle migration files with mixed line endings', async () => {
      const mixedLineEndings = 'CREATE TABLE test (id INTEGER);\r\nINSERT INTO test VALUES (1);\nSELECT * FROM test;\r';
