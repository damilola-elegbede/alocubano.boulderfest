  ...
  test('handles partial IntersectionObserver support', () => {
    ...
    expect(() => {
      if (observer.disconnect) {
        ...
        console.log('IntersectionObserver.disconnect not available, using manual cleanup');
      }
    }).not.toThrow();
  });
...
describe('Service Worker Compatibility', () => {
  ...
  test('handles service worker message communication', () => {
    ...
    expect(global.navigator.serviceWorker.postMessage).toHaveBeenCalledWith(testMessage);
  });
});
