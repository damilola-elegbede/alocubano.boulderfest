...
describe("UI/UX Integration Tests", () => {
  ...
  describe("Gallery Page Festival Year Cards", () => {
    test("should have festival year card styles defined in CSS", () => {
      const componentsCSS = fs.readFileSync(
        path.join(projectRoot, "css", "components.css"),
        "utf8",
      );
    ...
    test("boulder-fest-2025-gallery.html should contain properly structured festival year cards", () => {
      const galleryHTML = fs.readFileSync(
        path.join(projectRoot, "pages", "boulder-fest-2025-gallery.html"),
        "utf8",
      );

      // Check for gallery detail section
      expect(galleryHTML).toMatch(
        /class=["'][^"']*gallery-detail-section[^"']*["']/,
      );
    ...
    test("should have mobile responsive styles for festival year cards", () => {
      ...
      expect(componentsCSS).toMatch(
        /\.festival-years-grid\s*{\s*grid-template-columns:\s*1fr/,
      );
      expect(componentsCSS).toMatch(
        /\.year-number\s*{\s*font-size:\s*var\(--font-size-5xl\)/,
      );
    });
  });
  ...
  describe("Gallery Data Build System", () => {
    test("should have gallery data generation scripts available", () => {
      ...
      const featuredPhotosScript = path.join(
        projectRoot,
        "scripts",
    ...
    test("vercel.json should have proper JSON headers for static files", () => {
      ...
      const jsonHeaders = vercelConfig.headers.find((h) =>
        h.source.includes("json"),
      );
  ...
  describe("JavaScript Error Prevention", () => {
    test("gallery-detail.js should handle undefined apiUrl gracefully", () => {
      const galleryDetailJS = fs.readFileSync(
        path.join(projectRoot, "js", "gallery-detail.js"),
        "utf8",
      );

      // Check for proper error handling of apiUrl
      expect(galleryDetailJS).toMatch(
        /const\s+errorUrl\s*=\s*apiUrl\s*\|\|\s*['"`]Unknown URL['"`]/,
      );
    ...
    test("gallery-detail.js should have proper try-catch error handling", () => {
      ...
      expect(galleryDetailJS).toMatch(
        /staticEl\.style\.display\s*=\s*['"`]block['"`]/,
      );
    });
  });
  ...
  describe("Form Security - Mixed Content Prevention", () => {
    test("about.html should not have mailto form action", () => {
      const aboutHTML = fs.readFileSync(
        path.join(projectRoot, "pages", "about.html"),
        "utf8",
      );
      ...
      expect(aboutHTML).toMatch(
        /onsubmit\s*=\s*["']handleVolunteerForm\(event\)["']/,
      );
    ...
    test("tickets.html should have secure interactive ticket selection", () => {
      const ticketsHTML = fs.readFileSync(
        path.join(projectRoot, "pages", "tickets.html"),
        "utf8",
      );
    ...
    test("donations.html should have secure card-based donation selection", () => {
      const donationsHTML = fs.readFileSync(
        path.join(projectRoot, "pages", "donations.html"),
        "utf8",
      );
    ...
    test("donation selection JavaScript should handle cart integration", () => {
      const donationJS = fs.readFileSync(
    ...
    test("components.css should define all required CSS variables", () => {
      ...
      requiredVars.forEach((varName) => {
        expect(baseCSS).toMatch(
          new RegExp(`${varName.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&")}:`),
        );
      });
    });
  });
  ...
  describe("Asset Loading and Performance", () => {
    ...
    test("vercel.json should optimize static asset caching", () => {
      ...
      const imageHeaders = vercelConfig.headers.find((h) =>
        h.source.includes("jpg|jpeg|png"),
      );
      const cssJSHeaders = vercelConfig.headers.find((h) =>
        h.source.includes("css|js"),
      );
      const jsonHeaders = vercelConfig.headers.find((h) =>
        h.source.includes("json"),
      );
