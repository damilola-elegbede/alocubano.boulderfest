...
describe("Static Hero Image System", () => {
  ...
  describe("Hero Image Path Mapping", () => {
    test("should map page paths to correct static hero images", () => {
      ...
      expect(HERO_IMAGES["artists"]).toBe(
        "/images/hero/boulder-fest-2026-hero.jpg",
      );
      expect(HERO_IMAGES["schedule"]).toBe(
        "/images/hero/boulder-fest-2025-hero.jpg",
      );
      expect(HERO_IMAGES["gallery"]).toBe(
    ...
    test("should handle fallback to default hero image", () => {
      ...
      expect(getHeroImagePath("unknown-page")).toBe(
  ...
  describe("Hero Image Loading", () => {
    test("should load hero image with correct src and alt attributes", () => {
      const loadHeroImage = (pageId) => {
        ...
        const imagePath =
          HERO_IMAGES[pageId] || "/images/hero/hero-default.jpg";
  ...
  describe("CSS Object Position", () => {
    test("should maintain consistent object-position styling", () => {
      ...
      const testObjectPosition = (element) => {
        return (
          element.style.objectPosition === "top center" ||
          element
            .getAttribute("style")
            ?.includes("object-position: top center !important")
        );
      };
      ...
      const mockElement = {
        ...
        getAttribute: vi
          .fn()
          .mockReturnValue("object-position: top center !important;"),
      };
  ...
  describe("Performance Characteristics", () => {
    ...
    test("should eliminate ImageCacheManager dependency", () => {
      ...
      expect(loadWithoutCacheManager("unknown")).toBe(
        "/images/hero/hero-default.jpg",
      );
    });
  });
