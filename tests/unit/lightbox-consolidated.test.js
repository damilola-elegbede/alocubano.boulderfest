  ...
  return {
    ...
    classList: {
      ...
      contains: vi.fn((className) => {
        if (className === "lightbox" || className === "gallery-lightbox")
          return true;
    ...
    querySelector: vi.fn((selector) => {
      ...
      if (selector.includes("lightbox-close"))
        return { addEventListener: vi.fn() };
      if (selector.includes("lightbox-prev"))
        return { addEventListener: vi.fn(), style: { display: "block" } };
      if (selector.includes("lightbox-next"))
        return { addEventListener: vi.fn(), style: { display: "block" } };
      return null;
    }),
  ...
  global.window.Lightbox = class Lightbox {
    ...
    createLightboxHTML() {
      ...
      if (!document.getElementById(this.lightboxId)) {
        ...
        global.document.getElementById = vi.fn((id) => {
          if (id === this.lightboxId || id.includes("lightbox"))
            return mockElement;
          return null;
        });
      }
...
describe("Lightbox Component", () => {
  ...
  afterEach(() => {
    ...
    if (lightbox) {
      const lightboxElement = document.getElementById(
        lightbox.lightboxId || "unified-lightbox",
      );
      if (lightboxElement) {
...
describe("Lightbox Counter", () => {
  ...
  test("should handle category name formatting", () => {
    ...
    expect(
      counterText === "Social: 1 / 10" || counterText === "Socials: 1 / 10",
    ).toBe(true);
  });
