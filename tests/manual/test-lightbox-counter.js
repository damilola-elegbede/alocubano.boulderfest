...
(function () {
  ...
  if (window.galleryDebug) {
    ...
    if (categoryIndices.workshops.length > 0) {
      const workshopsSorted = [...categoryIndices.workshops].sort(
        (a, b) => a - b,
      );
      const workshopsExpected = Array.from(
        { length: workshopsSorted.length },
        (_, i) => i,
      );
      const workshopsCorrect =
        JSON.stringify(workshopsSorted) === JSON.stringify(workshopsExpected);
      console.log(
        "Workshop indices sequential:",
        workshopsCorrect ? "✅" : "❌",
      );
      if (!workshopsCorrect) {
    ...
    if (categoryIndices.socials.length > 0) {
      ...
      const socialsExpected = Array.from(
        { length: socialsSorted.length },
        (_, i) => i,
      );
      const socialsCorrect =
        JSON.stringify(socialsSorted) === JSON.stringify(socialsExpected);
    ...
    console.log(
      "\nOverall category indexing:",
      indexingCorrect ? "✅ PASS" : "❌ FAIL",
    );
  } else {
  ...
  function testLightboxCounter(
    itemIndex,
    expectedCategory,
    expectedCategoryIndex,
    ...
    if (galleryItems.length > itemIndex) {
      ...
      console.log(
        `Expected: ${expectedCategory} ${expectedCategoryIndex + 1}/${expectedCategoryTotal}`,
      );
      ...
      if (window.galleryDebug) {
        ...
        if (item) {
          ...
          const totalCorrect =
            state.categoryCounts[item.category] === expectedCategoryTotal;
  ...
  // Test last workshop
  const test2 = testLightboxCounter(
    workshopCount - 1,
    "workshops",
    workshopCount - 1,
    workshopCount,
  ...
  const savedState = sessionStorage.getItem(
    `gallery_${window.location.pathname.includes("2024") ? "2024" : "2025"}_state`,
  );
  if (savedState) {
    const parsed = JSON.parse(savedState);
    console.log("Saved categoryItemCounts:", parsed.categoryItemCounts);
    console.log(
      "CategoryItemCounts in session storage:",
      parsed.categoryItemCounts ? "✅" : "❌",
    );

    // Check if displayOrder items have categoryIndex
    const hasCategoryIndices =
      parsed.displayOrder &&
      parsed.displayOrder.length > 0 &&
      parsed.displayOrder[0].categoryIndex !== undefined;
    console.log(
      "Display order items have categoryIndex:",
      hasCategoryIndices ? "✅" : "❌",
    );
  } else {
    console.log(
      "No saved state yet - navigate away and back to test persistence",
    );
  }
  console.groupEnd();
