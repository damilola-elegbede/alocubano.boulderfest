...
(function () {
  ...
  if (state && state.displayOrder.length > 0) {
    ...
    if (firstSocialIndex === -1 || lastWorkshopIndex < firstSocialIndex) {
      console.log(
        "✅ Workshops load before socials (sequential loading works)",
      );
    } else {
      console.error(
        "❌ Sequential loading not working - socials appear before all workshops",
      );
    }
  }
  console.groupEnd();
  ...
  if (perfStats) {
    console.log(
      "Cache hit ratio:",
      (perfStats.cacheHitRatio * 100).toFixed(2) + "%",
    );
