...
(function() {
  ...
  lightboxes.forEach((lightbox, index) => {
    ...
    if (computedStyle.display === 'none' && !lightbox.classList.contains('is-open')) {
  ...
  console.log('Elements at page center:', elementsAtCenter.map(el => ({
    tagName: el.tagName,
  ...
  if (galleryItems.length > 0) {
    ...
    const hasClickHandler = firstItem.onclick !== null || 
                          firstItem.hasAttribute('data-loaded');
  ...
  if (window.galleryDebug && window.galleryDebug.getState) {
    ...
    if (state.lightbox && state.lightboxItems.length > 0) {
      ...
      state.lightbox.openAdvanced(state.lightboxItems, 0, state.lightboxCategories, state.categoryCounts);
      ...
      setTimeout(() => {
        ...
        if (lightbox) {
          ...
          setTimeout(() => {
            const isHidden = window.getComputedStyle(lightbox).display === 'none';
  ...
  // Summary
  console.log('\n📊 Fix Summary:');
  console.log('1. Lightbox display blocking - Fixed with display:none and proper show/close methods');
  console.log('2. Z-index conflicts - Gallery items have proper z-index layering');
