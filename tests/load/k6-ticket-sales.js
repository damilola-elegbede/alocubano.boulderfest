...
const testData = {
  ticketTypes: [
    { id: "general", name: "General Admission", price: 45.0 },
    { id: "vip", name: "VIP Pass", price: 125.0 },
    { id: "workshop", name: "Workshop Pass", price: 75.0 },
    { id: "full", name: "Full Festival Pass", price: 200.0 },
  ],
    ...
    group("Browse Tickets", () => {
      ...
      const pageLoadSuccess = check(response, {
        "tickets page loaded": (r) => {
          if (r.status !== 200) {
            console.warn(
              `Tickets page failed to load: ${r.status} - ${r.statusText}`,
            );
            return false;
          }
          return true;
        ...
        "page load time acceptable": (r) => {
          if (r.timings.duration >= 2000) {
            console.warn(
              `Slow page load: ${r.timings.duration}ms (expected < 2000ms)`,
            );
            return false;
          }
          return true;
      ...
      const availabilitySuccess = check(response, {
        "ticket availability fetched": (r) => {
          if (r.status !== 200) {
            console.warn(
              `Availability check failed: ${r.status} - ${r.statusText || "Unknown error"}`,
            );
            return false;
          }
          return true;
    ...
    group("Cart Management", () => {
      ...
      const cartCreationSuccess = check(response, {
        "cart created": (r) => {
          if (r.status !== 201) {
            console.warn(
              `Cart creation failed: ${r.status} - ${r.statusText || "Unknown error"}`,
            );
        ...
        "cart ID received": (r) => {
          try {
            ...
            if (!data || !data.cartId) {
              console.warn(
                `Cart ID missing from response: ${JSON.stringify(data)}`,
              );
              return false;
            }
            return true;
            ...
            console.warn(
              `Invalid JSON in cart creation response: ${e.message}`,
            );
            return false;
          }
        },
      ...
      for (const ticket of selectedTickets) {
        ...
        const addTicketSuccess = check(response, {
          "ticket added to cart": (r) => {
            if (r.status !== 200) {
              console.warn(
                `Failed to add ${ticket.id} to cart ${cartId}: ${r.status} - ${r.statusText || "Unknown error"}`,
              );
    ...
    group("Checkout Process", () => {
      ...
      const checkoutSuccess = check(response, {
        "checkout initialized": (r) => {
          if (r.status !== 200) {
            console.warn(
              `Checkout initialization failed: ${r.status} - ${r.statusText || "Unknown error"}`,
            );
            if (r.body) console.warn(`Response: ${r.body}`);
            return false;
          }
        ...
        "payment intent created": (r) => {
          try {
            ...
            if (!data || !data.paymentIntentId) {
              console.warn(
                `Payment intent ID missing: ${JSON.stringify(data)}`,
              );
              return false;
            }
            return true;
        ...
        "checkout response time acceptable": (r) => {
          if (r.timings.duration > 5000) {
            console.warn(
              `Slow checkout initialization: ${r.timings.duration}ms`,
            );
          }
      ...
      const paymentSuccess = check(response, {
        "payment processed": (r) => {
          if (r.status !== 200) {
            console.warn(
              `Payment processing failed: ${r.status} - ${r.statusText || "Unknown error"}`,
            );
        ...
        "order ID received": (r) => {
          try {
            ...
            if (!data || !data.orderId) {
              console.warn(
                `Order ID missing from payment response: ${JSON.stringify(data)}`,
              );
              return false;
            }
            return true;
    ...
    group("Order Confirmation", () => {
      ...
      const confirmationSuccess = check(response, {
        "confirmation received": (r) => {
          if (r.status !== 200) {
            console.warn(
              `Order confirmation failed for ${confirmationId}: ${r.status} - ${r.statusText || "Unknown error"}`,
            );
        ...
        "tickets attached": (r) => {
          try {
            ...
            if (
              ...
              console.warn(
                `No tickets in confirmation response: ${JSON.stringify(data)}`,
              );
              return false;
            }
            return true;
        ...
        "QR codes generated": (r) => {
          try {
            ...
            if (
              ...
              !data.tickets ||
              !data.tickets[0] ||
              !data.tickets[0].qrCode
            ) {
              console.warn(
                `QR codes missing from tickets: ${JSON.stringify(data?.tickets?.[0])}`,
              );
              return false;
            }
            return true;
...
export function handleSummary(data) {
  ...
  return {
    ...
    [`reports/load-test-results/ticket-sales-${timestamp}.json`]:
      JSON.stringify(data, null, 2),

    // HTML report for visualization
    [`reports/load-test-results/ticket-sales-${timestamp}.html`]:
      htmlReport(data),
  };
}
