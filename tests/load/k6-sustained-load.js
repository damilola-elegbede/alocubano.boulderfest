...
function purchaseOperation() {
  ...
  if (!success) {
    console.warn(
      `Purchase completion failed for cart ${cartId}: ${response.status}`,
    );
  }
...
function checkinOperation() {
  ...
  if (!success) {
    console.warn(
      `Check-in failed for device ${deviceId}, QR ${qrCode}: ${response.status}`,
    );
  }
...
export function teardown(data) {
  ...
  if (response.status === 200) {
    ...
    console.log(
      `- DB Connections: ${metrics.database?.activeConnections || "N/A"}`,
    );
  }
}
...
export function handleSummary(data) {
  ...
  return {
    ...
    [`reports/load-test-results/sustained-load-${timestamp}.json`]:
      JSON.stringify(
        {
          ...data,
          stabilityMetrics,
    ...
    [`reports/load-test-results/sustained-load-${timestamp}.html`]:
      generateStabilityReport(data, stabilityMetrics),
  };
}
