...
describe("Configuration Validation", () => {
  ...
  test("no environment detection in config", () => {
    ...
    // Should not have conditional configurations
    expect(config).not.toContain("if (");

    // Should not have ternary operators for configuration
    const ternaryCount = (config.match(/\?/g) || []).length;
    expect(ternaryCount).toBe(0); // No conditional configuration

    // Should use simple, static configuration
