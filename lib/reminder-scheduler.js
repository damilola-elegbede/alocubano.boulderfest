 ...
 *   - 24+ hours: 4 reminders (1hr post, 12hr post, 12hr before, 6hr before)
 *   - 6-24 hours: 3 reminders (initial, midpoint, final)
 *   - 1-6 hours: 2 reminders (initial, urgent halfway)
...
export class ReminderScheduler {
  constructor() {
    // Production reminder schedule (optimized for 24-hour deadline)
    // - 1 hour after purchase (initial confirmation)
    // - 12 hours after purchase (mid-point nudge)
    // - 12 hours before deadline (urgency reminder)
    // - 6 hours before deadline (final warning)
    this.productionReminderTypes = [
      { type: 'initial', hoursAfterPurchase: 1 },
      { type: '12hr-post-purchase', hoursAfterPurchase: 12 },
      { type: '12hr-before-deadline', hoursBeforeDeadline: 12 },
      { type: '6hr-before-deadline', hoursBeforeDeadline: 6 }
    ];
  ...
  computeAdaptiveReminders(purchaseTime, deadline, isTestTransaction) {
    ...
    if (totalHours >= 24) {
      ...
      // Use existing schedule: 12hr post-purchase, 12hr before, 6hr before
      reminders.push(
        { type: '12hr-post-purchase', hoursAfterPurchase: 12 },
        { type: '12hr-before-deadline', hoursBeforeDeadline: 12 },
        { type: '6hr-before-deadline', hoursBeforeDeadline: 6 }
      ...
      // Two reminders: midpoint + final
      reminders.push(
        { type: 'midpoint-reminder', hoursAfterPurchase: totalHours / 2 },
        { type: 'final-reminder', hoursBeforeDeadline: Math.min(2, totalHours * 0.2) }
      );
    } else if (totalHours >= 1) {
      // Very late purchase: 1-6 hours until deadline
      // One urgent reminder at halfway point
      reminders.push(
        { type: 'urgent-reminder', hoursBeforeDeadline: totalHours / 2 }
      );
    }
  ...
  async scheduleRemindersForTransaction(transactionId, registrationDeadline, isTestTransaction = false) {
    ...
    for (const reminder of reminderSchedule) {
      ...
      // Minimum spacing guard: Don't schedule reminders less than 5 minutes apart
      const timeSinceNow = (scheduledAt.getTime() - now.getTime()) / (1000 * 60);
      const timeUntilDeadline = (deadline.getTime() - scheduledAt.getTime()) / (1000 * 60);
      
      // Only schedule if:
      // 1. Scheduled time is in the future (after now + 5min minimum)
      // 2. Scheduled time is before or at the registration deadline
      // 3. Maintains minimum 5-minute spacing from now
      if (scheduledAt > now && scheduledAt <= deadline && timeSinceNow >= 0) {
        reminders.push({
          transactionId,
          type: reminder.type,
