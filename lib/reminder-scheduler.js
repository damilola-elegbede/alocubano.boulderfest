...
export class ReminderScheduler {
  ...
  async getPendingReminders(limit = 100) {
    ...
    // BIGINT FIX: Sanitize database results to convert BigInt values
    const sanitizedRows = processDatabaseResult(result.rows);

    // Only return reminders where there are still pending tickets
    const filtered = sanitizedRows.filter(row => row.pending_tickets > 0);

    console.log(`[ReminderScheduler] After filtering: ${filtered.length} reminders with pending tickets`);

    if (sanitizedRows.length > filtered.length) {
      const skipped = sanitizedRows.filter(row => row.pending_tickets === 0);
