...
export class StripeOrderMapper {
  ...
  static buildItemMetadata(item) {
    ...
    switch (item.type) {
      case 'ticket':
        // No silent defaults for ticket type - fail loudly
        if (!item.ticketType) {
          throw new Error(`Ticket type is required for ticket item: ${item.name || 'Unknown'}`);
        }
        metadata.ticket_type = item.ticketType;
