...
async function createTicketsForTransaction(fullSession, transaction) {
  ...
  // Track if any tickets need registration (for skipping reminders if all are pre-registered)
  let ticketsNeedingRegistration = 0;
  ...
  for (const item of lineItems) {
    ...
    const attendeeFromMeta = {
      firstName: meta.attendee_first_name || null,
      lastName: meta.attendee_last_name || null,
      email: meta.attendee_email || null
    };
  ...
  for (const lineData of lineItemData) {
    ...
    for (let i = 0; i < quantity; i++) {
      ...
      // Determine attendee info: use inline registration data if present, fallback to purchaser
      // This supports both inline checkout registration and legacy post-purchase registration
      const attendeeFirstName = attendee?.firstName || firstName;
      const attendeeLastName = attendee?.lastName || lastName;
      ...
      // Track tickets that still need registration (for reminder scheduling decision)
      if (!hasInlineRegistration) {
        ticketsNeedingRegistration++;
      }
      ...
      batchOperations.push({
        ...
        args: [
          ...
          ticketEventTime,
          priceForThisTicket,
          isTestTransaction ? `TEST-${attendeeFirstName}` : attendeeFirstName,
          isTestTransaction ? `TEST-${attendeeLastName}` : attendeeLastName,
  ...
  const sideEffectsPromise = (async () => {
    ...
    try {
      ...
      // Schedule reminders for each unique deadline (non-blocking fire-and-forget)
      // OPTIMIZATION: Don't await reminder scheduling to achieve 200-500ms faster checkout
      // INLINE REGISTRATION: Skip reminders entirely if all tickets are pre-registered
      sideEffectStep = 'reminder_scheduling';
      if (ticketsNeedingRegistration === 0) {
        console.log(`✅ All ${tickets.length} tickets pre-registered via inline checkout - skipping reminder scheduling`);
      } else if (deadlineTracker.size > 0) {
        console.log(`Scheduling reminders for transaction ${transaction.id} with ${deadlineTracker.size} unique deadline(s) (${ticketsNeedingRegistration} tickets need registration)`);

        // Fire-and-forget: Schedule reminders asynchronously without blocking
        for (const [deadlineKey, deadlineInfo] of deadlineTracker) {
          scheduleRegistrationReminders(
            ...
            console.log(
              `✅ Scheduled ${reminderCount} reminders for transaction ${transaction.id} ` +
              `(deadline ${deadlineInfo.deadline.toISOString()}, ${deadlineInfo.ticketCount} tickets, isTest: ${isTestTransaction})`
            );
            ...
            console.error('❌ Reminder scheduling failed for transaction:', transaction.id, reminderError);
            ...
            // Log to monitoring system but don't block checkout
          });
        }
      } else {
