...
async function createTicketsForTransaction(fullSession, transaction) {
  ...
  for (const item of lineItems) {
    ...
    const eventDateObj = new Date(authoritativeEventDate);
  ...
  try {
    ...
    // Schedule reminders for each unique deadline (handles multi-date purchases)
    ...
    console.error('❌ [TicketCreation] Non-critical side effect failed:', {
      step: sideEffectStep,
      ...
      errorName: sideEffectError.name
    });

    // Log specific guidance based on which step failed
    if (sideEffectStep === 'email_sending') {
    ...
    } else if (sideEffectStep === 'reminder_scheduling') {
    ...
    // Continue - tickets are created successfully, but side effects failed
  }
