...
async function createTicketsForTransaction(fullSession, transaction) {
    ...
    for (let i = 0; i < quantity; i++) {
      ...
      batchOperations.push({
        ...
        args: [
          ...
          parseInt(process.env.QR_CODE_MAX_SCANS || "3"),
  ...
  // Post-creation side effects (fire-and-forget, non-blocking)
  // Run asynchronously to avoid blocking payment success response
  (async () => {
    ...
    console.error('Background side effects failed:', error);
  });
