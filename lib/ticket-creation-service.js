...
async function createTicketsForTransaction(fullSession, transaction) {
  ...
  // SECURITY FIX: Separate test transaction marking from validation bypass
  // - isTestTransaction: For marking tickets as test data (cleanup, display) - ONLY based on Stripe session
  // - shouldBypassValidation: ONLY bypass if Stripe session is in test mode
  ...
  const shouldBypassValidation = testModeInfo.is_test === 1; // Only bypass for actual Stripe test mode
  
  console.log(`Transaction mode: isTest=${isTestTransaction}, bypassValidation=${shouldBypassValidation}, INTEGRATION_TEST_MODE=${process.env.INTEGRATION_TEST_MODE}`);
    ...
    try {
      ...
      await auditService.logDataChange({
        ...
        metadata: {
          ...
          validation_bypassed: shouldBypassValidation
        },
