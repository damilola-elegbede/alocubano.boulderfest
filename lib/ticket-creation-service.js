...
async function createTicketsForTransaction(fullSession, transaction) {
  ...
  for (const item of lineItems) {
    ...
    try {
      ...
      if (!ticketTypeResult.rows || ticketTypeResult.rows.length === 0) {
        ...
        // Validation 3: Verify event exists and is active
        if (!ticketTypeRecord.event_status || ticketTypeRecord.event_status !== 'active') {
          validationErrors.push(`Event for ticket type '${ticketType}' is not active: ${ticketTypeRecord.event_status || 'null'}`);
          validationFailed = true;
          console.error('SECURITY ALERT: Event not active:', {
            ticket_type: ticketType,
            event_id: eventId,
    ...
    let registrationDeadline = new Date(eventDateObj.getTime() - (7 * 24 * 60 * 60 * 1000));

    // Edge case: If deadline is in the past, use 24 hours from now as fallback
    if (registrationDeadline <= now) {
      console.warn(
        `Registration deadline for event ${eventDate} would be in the past. ` +
        `Using 24 hours from now as fallback.`
      );
      registrationDeadline = new Date(now.getTime() + (24 * 60 * 60 * 1000));
    }

    console.log(
      `Event date: ${eventDate}, Registration deadline: ${registrationDeadline.toISOString()}`
    );
    ...
    for (let i = 0; i < quantity; i++) {
      ...
      ticketsToSchedule.push({
        ticketId,
        registrationDeadline,
        isTestTicket: isTestTransaction
      });
  ...
  try {
    ...
    // TODO: Fix reminder scheduler to use transaction_id instead of ticket_id
    // Schedule reminders for each ticket
    // for (const { ticketId, registrationDeadline, isTestTicket } of ticketsToSchedule) {
    //   await scheduleRegistrationReminders(ticketId, registrationDeadline, isTestTicket);
    // }
    console.log(`Skipped reminder scheduling - needs schema update to support ticket_id`);
