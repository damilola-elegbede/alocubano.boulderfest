...
async function createTicketsForTransaction(fullSession, transaction) {
  ...
  for (const lineData of lineItemData) {
    const { item, quantity, ticketType, eventId, eventDate, priceInCents, meta } = lineData;
  ...
  const sideEffectsPromise = (async () => {
    ...
    try {
      ...
      if (deadlineTracker.size > 0) {
        console.log(`Scheduling reminders for transaction ${transaction.id} with ${deadlineTracker.size} unique deadline(s) (non-blocking)`);
