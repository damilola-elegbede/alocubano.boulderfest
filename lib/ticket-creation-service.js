...
async function createTicketsForTransaction(fullSession, transaction) {
  ...
  const sideEffectsPromise = (async () => {
    ...
    try {
      ...
      // Send ticket confirmation email asynchronously (non-blocking for performance)
      // This achieves 1-2 second performance improvement by not blocking checkout
      sideEffectStep = 'email_sending';
      console.log(`📧 [TicketCreation] Initiating async ticket confirmation email for transaction ${transaction.uuid}`);
      const ticketEmailService = getTicketEmailService();

      // Fire-and-forget: Send email asynchronously without blocking checkout
      ticketEmailService.sendTicketConfirmation(transaction)
        .then(() => {
