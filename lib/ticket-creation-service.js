...
// NOTE: reminder-scheduler.js was removed in commit 00cae43f (inline checkout registration)
// Reminders are no longer needed since attendee info is captured during checkout
...
import { buildQRCodeUrl, buildViewTicketsUrl, buildWalletPassUrls, getBaseUrl } from "./url-utils.js";
...
/**
 * Mask attendee data for logging (PII protection)
 * @param {object} attendee - Attendee object with name/email
 * @returns {object} Masked attendee data
 */
function maskAttendee(attendee) {
  if (!attendee) return null;
  return {
    ...attendee,
    email: attendee.email ? maskEmail(attendee.email) : undefined,
    name: attendee.name ? `${attendee.name.substring(0, 1)}***` : undefined
  };
}
 ...
 * @returns {{transaction: object, ticketCount: number, isTestTransaction: boolean, created: boolean}} An object containing the transaction, the number of tickets present or created, whether this is a test transaction (derived from Stripe session test mode), and `created` which is `true` when tickets were created during this call and `false` when existing tickets were returned.
 */
    ...
    if (existingTransaction) {
      ...
      if (ticketCount > 0) {
        ...
        return {
          ...
          isTestTransaction: existingTransaction.is_test === 1,
          created: false
        };
      }
...
async function createTicketsForTransaction(fullSession, transaction) {
  ...
  // Mark transaction as test data (for cleanup, display) - ONLY based on Stripe session
  // CRITICAL: Do NOT use INTEGRATION_TEST_MODE for isTestTransaction - it prevents testing production sold_count
  //           Integration tests should be able to create both test AND production transactions
  const isTestTransaction = testModeInfo.is_test === 1;

  console.log(`Transaction mode: isTest=${isTestTransaction}, INTEGRATION_TEST_MODE=${process.env.INTEGRATION_TEST_MODE}`);
  ...
  // =============================================================================
  // PERFORMANCE OPTIMIZATION: Batch ticket validation with single query
  // =============================================================================
  // Step 1: Extract all ticket types and build line item metadata
  // Step 2: Single batch query with JOINs to fetch all validation data
  // Step 3: Create validation map for O(1) lookups
  // Step 4: Validate all line items using cached data
  // Result: Replaces N+1 queries with 1 batch query (200-300ms faster)
  // =============================================================================

  const lineItemData = [];
  const ticketTypeIds = [];

  // Phase 1: Extract ticket type IDs and build line item metadata
  for (const item of lineItems) {
    ...
    // Convert event_id to INTEGER (Stripe metadata is always strings)
    ...
    // Parse event_id as INTEGER for database foreign key constraint
    // SQLite FK constraint requires exact type match (INTEGER vs TEXT)
    // Note: Negative IDs are valid for test events (e.g., -1, -2)
    const eventId = parseInt(eventIdRaw, 10);
    if (!Number.isInteger(eventId) || eventId === 0) {
      console.error('CRITICAL: Invalid event ID (must be non-zero integer):', {
        item_id: item.id,
        product_id: product?.id,
        event_id_raw: eventIdRaw,
        event_id_parsed: eventId,
        description: item.description
      });
      throw new Error(`Invalid event ID '${eventIdRaw}' for item: ${item.description || 'Unknown'}`);
    }
    ...
    // Extract attendee info from metadata if present (inline checkout registration)
    // This allows pre-registration during checkout instead of post-purchase
    // Use .trim() to handle empty strings and whitespace-only values
    const attendeeFromMeta = {
      firstName: meta.attendee_first_name?.trim() || null,
      lastName: meta.attendee_last_name?.trim() || null,
      email: meta.attendee_email?.trim() || null
    };

    // DEBUG: Log attendee data retrieved from Stripe metadata
    console.log('🔍 [ticket-creation] Extracted attendee from Stripe metadata:', {
      ticketType,
      rawMetaFirstName: meta.attendee_first_name,
      rawMetaLastName: meta.attendee_last_name,
      rawMetaEmail: meta.attendee_email,
      parsedAttendee: attendeeFromMeta
    });

    // Store line item data for batch validation
    lineItemData.push({
      item,
      quantity,
      ticketType,
      ...
      priceInCents,
      meta,
      attendee: attendeeFromMeta
    });

    // Collect unique ticket type IDs for batch query
    if (!ticketTypeIds.includes(ticketType)) {
      ticketTypeIds.push(ticketType);
    }
  }

  // Phase 2: Single batch query to fetch all ticket type validation data
  // =============================================================================
  // PERFORMANCE: Single query replaces N individual queries (200-300ms faster)
  // =============================================================================
  ...
  if (ticketTypeIds.length > 0) {
    ...
    const validationResult = await db.execute({
      sql: `
        SELECT
          ...
          (tt.max_quantity - COALESCE(tt.sold_count, 0)) as available_quantity
    ...
    // Build validation map for O(1) lookups
    for (const row of validationResult.rows) {
      validationMap.set(row.ticket_type_id, row);
    }

    console.log(`Batch validation query: Fetched ${validationResult.rows.length} ticket types in single query (expected ${ticketTypeIds.length})`);
  }
  ...
  // Webhook timing analysis with defensive null safety (shared across all line items)
  // FIX: Guard against null/undefined fullSession.created to prevent Invalid Date and NaN
  const validationTime = new Date();
  const createdSec = Number(fullSession?.created);
  const hasCreated = Number.isFinite(createdSec) && createdSec > 0;
  const sessionCreatedAt = hasCreated ? new Date(createdSec * 1000) : null;
  const webhookDelaySeconds = hasCreated ? (validationTime - sessionCreatedAt) / 1000 : 0;
  const isDelayedWebhook = hasCreated && webhookDelaySeconds > 300; // 5 minutes threshold

  if (isDelayedWebhook && sessionCreatedAt) {
    console.warn(`⚠️ Delayed webhook detected: ${webhookDelaySeconds.toFixed(0)}s delay (threshold: 300s)`, {
      ...
      delay_seconds: Number.isFinite(webhookDelaySeconds) ? webhookDelaySeconds.toFixed(0) : 'n/a',
      mitigation: 'Using lenient validation rules to prevent false positives'
    });
  } else if (!hasCreated) {
    console.warn('⚠️ Stripe session missing created timestamp - using lenient validation', {
      stripe_session_id: fullSession.id,
      validation_at: validationTime.toISOString()
    });
  }

  // Phase 3: Validate all line items using cached validation data
  for (const lineData of lineItemData) {
    const { item, quantity, ticketType, eventId, eventDate, priceInCents, meta, attendee } = lineData;
    ...
    try {
      // Validation 1: Verify ticket type exists in validation map (was individual query)
      ticketTypeRecord = validationMap.get(ticketType);

      if (!ticketTypeRecord) {
        // SECURITY FIX: Log audit entry BEFORE validation failure handling
        // This ensures audit logs are created even when validation fails
        try {
          const auditService = (await import('./audit-service.js')).default;
          await auditService.logDataChange({
            action: 'WEBHOOK_METADATA_VALIDATION_FAILED',
            targetType: 'stripe_webhook_validation',
            targetId: fullSession.id,
            metadata: {
              ticket_type: ticketType,
              error: `Ticket type '${ticketType}' does not exist in database`,
              stripe_session_id: fullSession.id
            },
            severity: 'critical'
          });
        } catch (auditError) {
          console.error('Failed to log audit entry for invalid ticket type:', auditError);
        }
        ...
        if (!isDelayedWebhook) {
          if (ticketTypeRecord.ticket_type_status !== 'available' && ticketTypeRecord.ticket_type_status !== 'test') {
            validationErrors.push(`Ticket type '${ticketType}' has invalid status: ${ticketTypeRecord.ticket_type_status}`);
            validationFailed = true;
            console.error('SECURITY ALERT: Ticket type not available:', {
              ticket_type: ticketType,
              status: ticketTypeRecord.ticket_type_status,
              stripe_session_id: fullSession.id,
              webhook_delay_seconds: webhookDelaySeconds
            });
          }
          ...
          console.log(`⏭️  Skipping ticket status validation due to delayed webhook (${webhookDelaySeconds.toFixed(0)}s)`, {
            ticket_type: ticketType,
            current_status: ticketTypeRecord.ticket_type_status,
        ...
        const dbEventId = Number(ticketTypeRecord.tt_event_id);
        ...
        if (stripeEventId !== null && !isNaN(stripeEventId) && !isNaN(dbEventId)) {
          if (dbEventId !== stripeEventId) {
            ...
            console.error('SECURITY ALERT: Event ID mismatch in metadata:', {
              ...
              webhook_delay_seconds: webhookDelaySeconds,
              raw_values: {
                db_raw: ticketTypeRecord.tt_event_id,
                stripe_raw: eventId
              }
            });
          } else {
        ...
        if (!isDelayedWebhook && ticketTypeRecord.max_quantity && ticketTypeRecord.max_quantity > 0) {
          const availableQuantity = ticketTypeRecord.available_quantity;
        ...
        // Validation 6: Verify price matches
        // FIX: Increased tolerance from 1% to 2% for multi-item rounding edge cases
        // FIX: Guard against divide-by-zero for free tickets
        // Convert BigInt to Number to avoid "Cannot mix BigInt and other types" error
        const expectedPriceCents = Number(ticketTypeRecord.price_cents) * quantity;

        // Handle free tickets (price = 0) separately to avoid divide-by-zero
        if (expectedPriceCents === 0) {
          if (priceInCents !== 0) {
            validationErrors.push(`Price mismatch: expected 0 cents, got ${priceInCents} cents (free ticket)`);
            validationFailed = true;
            console.error('SECURITY ALERT: Price mismatch on free ticket:', {
              ticket_type: ticketType,
            ...
            console.log('✓ Price validation passed for free ticket: 0 cents');
          }
        } else {
          // Non-zero price: calculate variance percentage
      ...
      await auditService.logDataChange({
        ...
        metadata: {
          ...
          webhook_timing: {
            created_at: sessionCreatedAt ? sessionCreatedAt.toISOString() : null,
          ...
          ticket_type_record: ticketTypeRecord ? {
            ...
            event_id: ticketTypeRecord.tt_event_id
          } : null,
          stripe_session_id: fullSession.id,
    ...
    for (let i = 0; i < quantity; i++) {
      ...
      // Determine attendee info: STRICT MODE - require explicit attendee data
      // No fallback to Stripe customer name - error if attendee data is missing
      // This ensures all tickets have proper attendee registration from checkout
      if (!attendee?.firstName || !attendee?.lastName) {
        console.error('❌ CRITICAL: Attendee data missing for ticket creation!', {
          ticketType,
          eventId,
          attendee: attendee,
          stripeMetadata: meta,
        ...
        throw new Error(
          `Attendee name is required for ticket "${ticketType}". ` +
          `Got firstName="${attendee?.firstName}", lastName="${attendee?.lastName}". ` +
          `This indicates data was lost somewhere in the checkout flow.`
        );
      }
      const attendeeFirstName = attendee.firstName;
      const attendeeLastName = attendee.lastName;
      ...
      batchOperations.push({
        ...
        args: [
          ...
          ticketTypeRecord ? ticketType : null,  // Only set FK if ticket type exists in database
          eventId,
          ticketEventDate,
          ticketEventTime,
          priceForThisTicket,
          attendeeFirstName,
          attendeeLastName,
          attendeeEmail,  // NEW: Store attendee email for inline registration
      ...
      // Store full ticket data for per-attendee email sending
      tickets.push({
        id: ticketId,
        type: ticketType,
        ticket_id: ticketId,
        ...
        event_time: ticketEventTime,
        ticket_type_name: ticketTypeRecord?.name || ticketType,
        event_name: ticketTypeRecord?.event_name || 'A Lo Cubano Boulder Fest 2026',
        venue_name: ticketTypeRecord?.venue_name || 'Avalon Ballroom',
        venue_city: ticketTypeRecord?.venue_city || 'Boulder',
        venue_state: ticketTypeRecord?.venue_state || 'CO'
      });
    }
  ...
  const sideEffectsPromise = (async () => {
    ...
    try {
      ...
      // Send ticket confirmation email (MUST await to ensure completion in serverless)
      // CRITICAL: Must await in serverless environments to prevent function termination before email sends
      sideEffectStep = 'email_sending';
      console.log(`📧 [TicketCreation] Initiating ticket confirmation email for transaction ${transaction.uuid}`);
      const ticketEmailService = getTicketEmailService();

      // AWAIT email sending to ensure completion before serverless function terminates
      try {
        await ticketEmailService.sendTicketConfirmation(transaction);
        console.log(`✅ [TicketCreation] Ticket confirmation email sent successfully to: ${maskEmail(transaction.customer_email)} (transaction: ${transaction.uuid})`);
      } catch (emailError) {
        // CRITICAL: Log full error details to Vercel logs (errors were being swallowed)
        console.error('❌ [TicketCreation] Email sending failed, queuing for retry:', {
          transactionId: transaction.uuid,
          transactionInternalId: transaction.id,
          customerEmail: maskEmail(transaction.customer_email),
        ...
        try {
          ...
          await db.execute({
            ...
            args: [
              ...
              emailError.message,
              transaction.is_test ? 1 : 0
            ]
          });
          console.log(`📝 [TicketCreation] Email queued for retry: transaction ${transaction.uuid} (will retry in 5 minutes)`);
        } catch (queueError) {
          console.error('🚨 [TicketCreation] CRITICAL: Failed to queue email for retry:', {
            transactionId: transaction.uuid,
            customerEmail: maskEmail(transaction.customer_email),
            queueError: queueError.message,
            queueErrorStack: queueError.stack,
            originalEmailError: emailError.message,
            action: 'Manual intervention required - customer may not receive confirmation email'
          });
        }
      }

      // NOTE: Reminder scheduling removed in commit 00cae43f
      // With inline checkout registration, attendee info is captured during checkout
      // so registration reminders are no longer needed.

      // ========================================================================
      // STEP: Send per-attendee confirmation emails with QR codes
      // Each attendee gets their own email with their specific ticket QR code
      // ========================================================================
      sideEffectStep = 'per_attendee_emails';
      console.log(`📧 [TicketCreation] Sending ${tickets.length} per-attendee confirmation emails...`);
      ...
      for (const ticket of tickets) {
        // Skip if no attendee email (shouldn't happen with inline checkout, but be safe)
        if (!ticket.attendee?.email) {
          console.warn(`⚠️ [TicketCreation] Skipping email for ticket ${ticket.ticket_id} - no attendee email`);
          continue;
        }

        try {
          // Generate QR token for this ticket
          const qrToken = await qrService.getOrCreateToken(String(ticket.ticket_id));

          // Format event date
          const eventDate = ticket.event_date
            ? new Date(ticket.event_date + 'T00:00:00').toLocaleDateString('en-US', {
                month: 'long',
                day: 'numeric',
                year: 'numeric',
          ...
          const walletUrls = buildWalletPassUrls(ticket.ticket_id);
          ...
          const htmlContent = generateAttendeeConfirmationEmail({
            ...
            orderNumber: transaction.order_number || transaction.uuid,
          ...
          await brevo.makeRequest('/smtp/email', {
            ...
            body: JSON.stringify({
              ...
              subject: isTestTransaction
                ? `[TEST] Your Ticket is Ready - ${ticket.event_name}`
                : `Your Ticket is Ready - ${ticket.event_name}`,
              htmlContent: htmlContent,
              headers: {
                'X-Mailin-Tag': isTestTransaction ? 'attendee-confirmation-test' : 'attendee-confirmation',
                'X-Ticket-ID': ticket.ticket_id,
                'X-Transaction-ID': transaction.uuid,
                'X-Test-Mode': isTestTransaction ? 'true' : 'false'
              }
            })
          });

          sentCount++;
          console.log(`✅ [TicketCreation] Sent attendee confirmation for ticket ${ticket.ticket_id} to ${maskEmail(ticket.attendee.email)}`);

        } catch (attendeeConfirmError) {
          failedCount++;
          console.error(`❌ [TicketCreation] Failed to send attendee confirmation for ticket ${ticket.ticket_id}:`, {
            recipient: maskEmail(ticket.attendee.email),
            error: attendeeConfirmError.message
          });
          // Continue with other tickets - don't fail the whole batch
        }
      }

      console.log(`📧 [TicketCreation] Attendee confirmations complete: sent=${sentCount}, failed=${failedCount}`);
      console.log(`✅ [TicketCreation] ${tickets.length} tickets created with inline registration`);
