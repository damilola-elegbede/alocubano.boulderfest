  ...
  async verifyPassword(password) {
    ...
    try {
      ...
      if (this.isCIOrTest) {
        ...
        // Fast path for CI/test environments - plain text comparison
        if (password === testAdminPassword) {
          this.recordSuccess();
          return true;
        }
        
        // If plain text doesn't match in CI/test, don't fall through to bcrypt
        // This prevents unnecessary bcrypt operations in CI
        this.recordSuccess(); // Not a failure, just wrong password
        return false;
      }

      // For production and development, use bcrypt hashed ADMIN_PASSWORD with timeout protection
