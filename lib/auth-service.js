...
export class AuthService {
  ...
  async verifyPassword(password) {
    ...
    try {
      // Detect E2E test environment first (highest priority)
      const isE2ETest =
        process.env.E2E_TEST_MODE === 'true' ||
        process.env.CI === 'true';

      // Production ONLY when NOT in E2E test mode AND both env vars are production
      const isProduction = !isE2ETest &&
