...
export class AuthService {
  ...
  async _performInitialization() {
    try {
      ...
      console.log("[Auth] ADMIN_SECRET configured:", this.sessionSecret ? 'SET' : 'NOT_SET');
      ...
      if (this.sessionSecret.length < 32) {
        console.error("[Auth] ADMIN_SECRET too short: must be at least 32 characters");
  ...
  async verifyPassword(password) {
    ...
    try {
      // Production ONLY when both NODE_ENV and VERCEL_ENV are explicitly production
      // This takes precedence over CI/E2E_TEST_MODE flags
      const isProduction = process.env.NODE_ENV === "production" &&
                          process.env.VERCEL_ENV === "production";

      // E2E/CI test mode only applies when NOT explicitly in production
      const isE2ETest = !isProduction && (
  ...
  requireAuth(handler) {
    ...
    return async (req, res) => {
      ...
      try {
        ...
        const token = this.getSessionFromRequest(req);
        console.log(`🔐 [AUTH] Session token present:`, !!token); // Only log presence, not token value
        ...
        // SECURITY: Verify admin scope/role in JWT payload
        if (!session.admin || session.admin.role !== 'admin') {
          console.log(`🔐 [AUTH] Missing admin role in token, returning 403`);
