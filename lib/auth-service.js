...
export class AuthService {
  constructor() {
    ...
    this.isTest = process.env.NODE_ENV === 'test' || process.env.E2E_TEST_MODE === 'true';
  ...
  initialize() {
    ...
    try {
      ...
      // Default: 3600000ms = 1 hour, but shorter for CI/test environments
      const defaultDuration = this.isCIOrTest ? "600000" : "3600000"; // 10 minutes for CI/test, 1 hour for production
  ...
  async verifyPassword(password) {
    ...
    try {
      // For E2E testing, support plain text TEST_ADMIN_PASSWORD if available
      if (this.isCIOrTest && testAdminPassword) {
        // Fast path for CI/test environments when TEST_ADMIN_PASSWORD is available
        if (password === testAdminPassword) {
          this.recordSuccess();
          return true;
        }
        
        // If plain text doesn't match but TEST_ADMIN_PASSWORD is configured,
        // fall through to bcrypt to allow both test and real passwords
      }

      // For production, development, and CI without TEST_ADMIN_PASSWORD,
      // use bcrypt hashed ADMIN_PASSWORD with timeout protection
