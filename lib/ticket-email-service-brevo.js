...
import { buildRegistrationUrl, buildQRCodeUrl, buildViewTicketsUrl } from "./url-utils.js";
...
export class TicketEmailService {
  ...
  async sendTicketConfirmation(transaction) {
    ...
    try {
      ...
      const htmlContent = generateOrderConfirmationEmail({
        ...
        registrationUrl: transaction.registration_token ? buildRegistrationUrl(transaction.registration_token) : buildViewTicketsUrl(accessToken),
  ...
  async sendRegistrationInvitation(options) {
    ...
    try {
      ...
      // Log test mode operation
      logTestModeOperation(
        `Sending registration invitation email to ${this.maskEmail(customerEmail)}`,
        isTest,
        {
          transactionId,
          ticketCount,
      ...
      const registrationUrl = buildRegistrationUrl(registrationToken);
  ...
  async sendRegistrationReminder(options) {
    ...
    try {
      ...
      // Log test mode operation
      logTestModeOperation(
        `Sending registration reminder (${reminderType}) email to ${this.maskEmail(customerEmail)}`,
        isTest,
        {
  ...
  async sendTransferNotification(params) {
    ...
    try {
      ...
      // Use registration token if available, otherwise generate access token for view-tickets
      let registrationUrl;
      if (transaction?.registration_token) {
        // Use proper registration token for registration flow
        registrationUrl = buildRegistrationUrl(transaction.registration_token);
      } else {
        // Fallback: generate access token for view-tickets URL
        const accessToken = await ticketService.generateAccessToken(
          transactionId,
          newOwnerEmail
        );
        registrationUrl = buildViewTicketsUrl(accessToken);
      }

      // Format registration deadline (72 hours from now)
      const registrationDeadline = this.formatRegistrationDeadline();
      ...
      const emailHtml = generateTransferNotificationEmail({
        ...
        eventLocation: this.venueName,
        eventDate: this.eventDatesDisplay
      });
      ...
      console.log('✅ [TicketEmail] Transfer notification sent successfully to:', this.maskEmail(newOwnerEmail));
      return true;
    } catch (error) {
  ...
  async sendTransferConfirmation(params) {
    ...
    try {
      ...
      console.log('✅ [TicketEmail] Transfer confirmation sent successfully to:', this.maskEmail(originalOwnerEmail));
      return true;
    } catch (error) {
