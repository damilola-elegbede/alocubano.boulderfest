...
`;
  ...
  async sendTicketConfirmation(transaction) {
    try {
      ...
      const fullTickets = await this.getTicketsByTransactionId(transaction.id);

      if (!fullTickets || fullTickets.length === 0) {
        throw new Error(`No tickets found for transaction ${transaction.uuid || transaction.id}`);
      }

      // Detect test mode from transaction and tickets
      const isTest = this.isTestTransaction(transaction) || fullTickets.some(ticket => this.isTestTicket(ticket));
      ...
      const ticketDetails = await this.formatTicketsForEmail(fullTickets);

      // Generate QR code for the first ticket (main QR for entry)
      const mainQRToken = ticketDetails[0]?.qrToken;
      const firstTicketId = fullTickets[0]?.ticket_id;
      ...
      const emailParams = {
        ...
        params: {
          ...
          APPLE_WALLET_BUTTON_URL: `${this.baseUrl}/images/add-to-wallet-apple.svg`,
  ...
  /**
   * Format purchase date for email
   */
  ...
  /**
   * Format ticket details for email (simple text format for Order Confirmation)
   */
    ...
    try {
      ...
      const result = await db.execute({
        sql: `SELECT
          ...
          attendee_first_name,
          attendee_last_name,
          'A Lo Cubano Boulder Fest 2026' as event_name,
          'Avalon Ballroom, Boulder, CO' as event_location,
          is_test,
          transaction_id,
          created_at
        FROM tickets
  ...
  async sendRegistrationReminder(options) {
    ...
    try {
      ...
      if (ticketId) {
        ...
        const qrService = getQRTokenService();
        qrToken = await qrService.getOrCreateToken(ticketId);
      }
      ...
      const emailParams = {
        ...
        params: {
          ...
          QR_CODE_URL: ticketId && qrToken ? `${this.baseUrl}/api/qr/generate?token=${qrToken}` : '',
          APPLE_WALLET_URL: ticketId ? `${this.baseUrl}/api/tickets/apple-wallet/${ticketId}` : '',
          GOOGLE_WALLET_URL: ticketId ? `${this.baseUrl}/api/tickets/google-wallet/${ticketId}` : '',
          // Wallet button image URLs
          APPLE_WALLET_BUTTON_URL: `${this.baseUrl}/images/add-to-wallet-apple.svg`,
          GOOGLE_WALLET_BUTTON_URL: `${this.baseUrl}/images/add-to-wallet-google.png`,
          // Removed IS_TEST_MODE as requested
