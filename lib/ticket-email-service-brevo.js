  ...
  async sendTicketConfirmation(transaction, tickets) {
    try {
      ...
      const emailParams = {
        ...
        params: {
          ORDER_NUMBER: transaction.order_number || transaction.uuid,  // Add order number
          PURCHASE_DATE: this.formatPurchaseDate(transaction.completed_at || transaction.created_at),
          ...
          AMOUNT: (Number(transaction.total_amount) / 100).toFixed(2),
          ...
          QR_CODE: `${this.baseUrl}/api/qr/generate?token=${mainQRToken}`,
          TICKET_DETAILS: this.formatTicketDetailsForEmail(ticketDetails),
          EVENT_SCHEDULE: this.formatEventSchedule(),
          EVENT_LOCATION: 'Boulder, Colorado',
