  ...
  async getScopedDatabaseClient(scopeId) {
    ...
    if (scopeId) {
      ...
      if (scope.databaseClient) {
        try {
          ...
          logger.warn(`⚠️ Cached client invalid for scope ${scopeId}, creating new one`);
          if (scope.databaseClient) {
    ...
    if (scope.databaseClient) {
      try {
        ...
        logger.warn(`⚠️ Cached client invalid for scope ${this.currentTestId}, creating new one`);
        if (scope.databaseClient) {
          scope.connections.delete(scope.databaseClient);
          this.globalConnectionRegistry.delete(scope.databaseClient);
        }
