...
class TestIsolationManager {
  ...
  async initializeTestMode() {
    ...
    logger.debug('🔬 Test Isolation Manager initialized');
  ...
  async getScopedDatabaseClient(scopeId) {
    ...
    if (scopeId) {
      ...
      try {
        // CRITICAL FIX: For in-memory databases, create unique instance per scope
        // This prevents SQLITE_IOERR_FSTAT errors from shared memory access
        if (process.env.DATABASE_URL === ':memory:' && process.env.INTEGRATION_TEST_MODE === 'true') {
          // Create a completely new in-memory database for this scope
          const { createClient } = await import("@libsql/client");

          // Each scope gets its own in-memory database instance
          const client = createClient({
            url: ':memory:',
            // No auth token needed for in-memory databases
          });

          logger.debug(`📦 Created new in-memory database for scope ${scopeId}`);
          ...
          // Run migrations on this fresh database (each scope needs migrations)
          ...
          migrationSystem.ensureDbClient = async () => client;
          await migrationSystem.runMigrations();
          logger.debug(`✅ Migrations completed for scope ${scopeId}`);
    ...
    // CRITICAL FIX: For in-memory databases in legacy API, create unique instance
    if (process.env.DATABASE_URL === ':memory:' && process.env.INTEGRATION_TEST_MODE === 'true') {
      // Create a completely new in-memory database for this scope
      const { createClient } = await import("@libsql/client");
      ...
      logger.debug(`📦 Created new in-memory database for legacy scope ${this.currentTestId}`);
      ...
      migrationSystem.ensureDbClient = async () => client;
      await migrationSystem.runMigrations();
      logger.debug(`✅ Migrations completed for legacy scope ${this.currentTestId}`);
