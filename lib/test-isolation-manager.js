  ...
  async getScopedDatabaseClient(scopeId) {
    ...
    if (scopeId) {
      ...
      if (scope.databaseClient) {
        logger.debug(`🔗 Returning cached database client for scope ${scopeId}`);
    ...
    // Return cached client if it exists (legacy API caching)
    if (scope.databaseClient) {
      logger.debug(`🔗 Returning cached database client for legacy test scope: ${this.currentTestId}`);
      return scope.databaseClient;
    }
