...
class TestIsolationManager {
  ...
  async initializeWorkerDatabase() {
    ...
    this.workerInitializationPromise = (async () => {
      ...
      // Create a single in-memory database for this worker
      const { createClient } = await import("@libsql/client");
      this.workerDatabase = createClient({
        url: ':memory:',
        // No auth token needed for in-memory databases
      });
  ...
  /**
   * Get the worker database directly for services that need it
   * This is used by getDatabaseClient() when in test isolation mode
   * @returns {Promise<Object>} The worker database client
   */
  async getWorkerDatabase() {
    if (process.env.DATABASE_URL === ':memory:' && process.env.INTEGRATION_TEST_MODE === 'true') {
      // Initialize worker database if not done yet
    ...
    // For non-memory databases, use the standard scoped client approach
    return await this.getScopedDatabaseClient();
  }
}
