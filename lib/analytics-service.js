...
import { isTestMode } from "./test-mode-utils.js";
...
export class AnalyticsService {
  ...
  async getEventStatistics(eventId = "boulder-fest-2026") {
    try {
      ...
      const stats = await client.execute({
        sql: `
        SELECT
          -- Ticket Stats (COUNT all tickets, not DISTINCT - ticket IDs are already unique)
      ...
      return processDatabaseResult(stats.rows[0] || {});
    } catch (error) {
    ...
    try {
      ...
      const trend = await client.execute({
        ...
        SELECT
          ...
          AVG(t.price_cents) / 100.0 as avg_price
        FROM tickets t
      ...
      return processDatabaseResult(trend.rows).map((row) => {
  ...
  async getHourlySalesPattern(eventId = "boulder-fest-2026") {
    ...
    const sanitizedRows = processDatabaseResult(pattern.rows);
    const hourlyData = Array(24)
      .fill(null)
      .map((_, hour) => {
        const hourStr = hour.toString().padStart(2, "0");
        const data = sanitizedRows.find((r) => r.hour === hourStr);
  ...
  async getCustomerAnalytics(eventId = "boulder-fest-2026") {
    ...
    return {
      summary: processDatabaseResult(analytics.rows[0]),
      topCustomers: processDatabaseResult(topCustomers.rows),
    };
  }
  ...
  async getCheckinAnalytics(eventId = "boulder-fest-2026") {
    ...
    return {
      timeline: processDatabaseResult(checkins.rows),
      rates: processDatabaseResult(checkinRates.rows),
    };
  }
  ...
  async getRevenueBreakdown(eventId = "boulder-fest-2026") {
    ...
    const breakdown = await client.execute({
      sql: `
        SELECT
          ...
          ROUND(COALESCE(SUM(t.price_cents), 0) * 100.0 / NULLIF((
            SELECT COALESCE(SUM(t2.price_cents), 0)
            FROM tickets t2
            JOIN transactions tr2 ON t2.transaction_id = tr2.id
            WHERE t2.status = 'valid' AND t2.event_id = ? AND COALESCE(tr2.payment_processor, '') <> 'comp'
          ), 0), 2) as revenue_percentage,
    ...
    return processDatabaseResult(breakdown.rows);
  }
  ...
  async getWalletAnalytics(eventId = "boulder-fest-2026") {
    ...
    const [analytics, summary, roi] = await Promise.all([
      ...
      client.execute({
        sql: `
          SELECT
            COUNT(CASE WHEN (t.apple_pass_serial IS NOT NULL OR t.google_pass_id IS NOT NULL) THEN 1 END) as wallet_sales,
            COUNT(CASE WHEN (t.apple_pass_serial IS NULL AND t.google_pass_id IS NULL) THEN 1 END) as traditional_sales,
            SUM(CASE WHEN (t.apple_pass_serial IS NOT NULL OR t.google_pass_id IS NOT NULL) THEN t.price_cents ELSE 0 END) / 100.0 as wallet_revenue,
            SUM(CASE WHEN (t.apple_pass_serial IS NULL AND t.google_pass_id IS NULL) THEN t.price_cents ELSE 0 END) / 100.0 as traditional_revenue,
            COUNT(CASE WHEN t.apple_pass_serial IS NOT NULL THEN 1 END) as apple_wallet_sales,
            COUNT(CASE WHEN t.google_pass_id IS NOT NULL THEN 1 END) as google_wallet_sales,
            SUM(CASE WHEN t.apple_pass_serial IS NOT NULL THEN t.price_cents ELSE 0 END) / 100.0 as apple_wallet_revenue,
            SUM(CASE WHEN t.google_pass_id IS NOT NULL THEN t.price_cents ELSE 0 END) / 100.0 as google_wallet_revenue
          FROM tickets t
          JOIN transactions tr ON t.transaction_id = tr.id
          WHERE t.status = 'valid'
            AND t.event_id = ?
    ...
    return {
      timeline: processDatabaseResult(analytics.rows),
      summary: processDatabaseResult(summary.rows[0]),
      roi: processDatabaseResult(roi.rows[0]),
    };
  }
  ...
  async getConversionFunnel(days = 30, eventId = "boulder-fest-2026") {
    ...
    const data = processDatabaseResult(funnel.rows[0]);
   ...
   * @param {string} eventId - Event ID to filter by
   */
  async getTestDataAnalytics(eventId = "boulder-fest-2026") {
    try {
      const client = await this.ensureInitialized();
      ...
      const testOverview = await client.execute({
        ...
        SELECT
          ...
          SUM(CASE WHEN COALESCE(tr.payment_processor, '') <> 'comp' THEN t.price_cents ELSE 0 END) / 100.0 as test_revenue,
      ...
      const ageDistribution = await client.execute({
        ...
        SELECT
          CASE
            WHEN julianday('now') - julianday(t.created_at) < 1 THEN 'very_recent'
            WHEN julianday('now') - julianday(t.created_at) < 7 THEN 'recent'
            WHEN julianday('now') - julianday(t.created_at) < 30 THEN 'old'
            ELSE 'very_old'
          END as age_category,
          COUNT(*) as count,
          SUM(CASE WHEN COALESCE(tr.payment_processor, '') <> 'comp' THEN t.price_cents ELSE 0 END) / 100.0 as revenue
        FROM tickets t
        JOIN transactions tr ON t.transaction_id = tr.id
        WHERE t.is_test = 1 AND t.event_id = ?
      ...
      return {
        overview: processDatabaseResult(testOverview.rows[0] || {}),
        age_distribution: processDatabaseResult(ageDistribution.rows),
        status_breakdown: processDatabaseResult(statusBreakdown.rows),
        cleanup_candidates: processDatabaseResult(cleanupCandidates.rows),
        test_mode_info: {
   ...
   * @param {string} eventId - Event ID to filter by
   */
  async getProductionVsTestComparison(eventId = "boulder-fest-2026") {
    try {
      const client = await this.ensureInitialized();
      ...
      const comparison = await client.execute({
        ...
        SELECT
          ...
          AVG(t.price_cents) / 100.0 as avg_ticket_price,
          COUNT(CASE WHEN t.checked_in_at IS NOT NULL THEN 1 END) as checked_in_count
        FROM tickets t
        JOIN transactions tr ON t.transaction_id = tr.id
        WHERE t.is_test = 0 AND t.event_id = ? AND COALESCE(tr.payment_processor, '') <> 'comp'
        ...
        FROM tickets t
        JOIN transactions tr ON t.transaction_id = tr.id
        WHERE t.is_test = 1 AND t.event_id = ? AND COALESCE(tr.payment_processor, '') <> 'comp'
      `,
        args: [eventId, eventId],
      });

      // Calculate ratios and percentages
      const sanitizedRows = processDatabaseResult(comparison.rows);
      const productionData = sanitizedRows.find(r => r.data_type === 'production') || {};
      const testData = sanitizedRows.find(r => r.data_type === 'test') || {};
   ...
   */
  async generateTestAwareExecutiveSummary(eventId = "boulder-fest-2026") {
    const [stats, trend, customers, revenue, funnel, walletAnalytics, testAnalytics, comparison] =
      await Promise.all([
        ...
        this.getConversionFunnel(30, eventId),
        this.getWalletAnalytics(eventId),
        this.getTestDataAnalytics(eventId),
        this.getProductionVsTestComparison(eventId)
      ]);
    ...
    return {
      overview: {
        tickets_sold: stats.total_tickets,  // All tickets in DB = sold tickets
        gross_revenue: stats.gross_revenue,
        unique_customers: customers.summary.unique_customers,
        check_in_rate:
          stats.total_tickets > 0
            ? Math.round((Number(stats.checked_in) / Number(stats.total_tickets)) * 100)
            : 0,
        days_until_event: daysUntilEvent
      },
      performance: {
