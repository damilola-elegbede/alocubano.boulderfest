    ...
    const breakdown = await client.execute({
      sql: `
        SELECT
          ...
          ROUND(SUM(t.price_cents) * 100.0 / (
            SELECT SUM(price_cents)
            FROM tickets
            JOIN transactions ON tickets.transaction_id = transactions.id
            WHERE tickets.status = 'valid' AND tickets.event_id = ? AND transactions.payment_processor NOT IN ('comp')
        ...
        WHERE t.status = 'valid'
          AND t.event_id = ?
          AND tr.payment_processor NOT IN ('comp')
  ...
  async getProductionVsTestComparison(eventId = "boulder-fest-2026") {
    try {
      ...
      const comparison = await client.execute({
        ...
        FROM tickets t
        JOIN transactions tr ON t.transaction_id = tr.id
        WHERE t.is_test = 0 AND t.event_id = ? AND tr.payment_processor NOT IN ('comp')
        ...
        FROM tickets t
        JOIN transactions tr ON t.transaction_id = tr.id
        WHERE t.is_test = 1 AND t.event_id = ? AND tr.payment_processor NOT IN ('comp')
      `,
        args: [eventId, eventId],
      });
