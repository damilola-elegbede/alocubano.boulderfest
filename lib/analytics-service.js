...
export class AnalyticsService {
  ...
  async getProductionVsTestComparison(eventId = "boulder-fest-2026", req = null) {
    try {
      ...
      const totalTickets = (productionData.ticket_count || 0) + (testData.ticket_count || 0);
      const totalRevenue = (productionData.revenue || 0) + (testData.revenue || 0);
      ...
      return {
        ...
        comparison: {
          ...
          test_percentage: totalTickets > 0 ? ((testData.ticket_count || 0) / totalTickets * 100).toFixed(2) : 0,
          test_revenue_percentage: totalRevenue > 0 ? ((testData.revenue || 0) / totalRevenue * 100).toFixed(2) : 0,
          production_percentage: totalTickets > 0 ? ((productionData.ticket_count || 0) / totalTickets * 100).toFixed(2) : 0
        },
        timestamp: new Date().toISOString()
      };
