...
export async function loadConfig(environment, baseDir) {
  ...
  try {
    ...
    // For test environment, fallback to development.json
    if (environment === 'test') {
      const fallbackFile = path.join(baseDir, `../bootstrap/development.json`);
      try {
        await fs.promises.access(fallbackFile, fs.constants.F_OK);
        const logger = createLogger('ConfigLoader');
        logger.warn(`⚠️  ${environment}.json not found, falling back to development.json`);
        logger.info(`Attempted paths: ${configFile}, using: ${fallbackFile}`);

        const configContent = await fs.promises.readFile(fallbackFile, 'utf8');

        let config;
        try {
          config = JSON.parse(configContent);
        } catch (error) {
          throw new Error(`Invalid JSON in configuration file ${fallbackFile}: ${error.message}`);
        }

        // Override environment field to match the requested environment
        config.environment = environment;
        config.metadata = config.metadata || {};
        config.metadata.fallback_from = 'development';
        config.metadata.source_path = fallbackFile;

        return config;
      } catch (fallbackError) {
        // Fallback file doesn't exist or is unreadable
      }
    }

    // Provide detailed error with attempted paths
    const attemptedPaths = [configFile];
    if (environment === 'test') {
      attemptedPaths.push(path.join(baseDir, `../bootstrap/development.json`));
    }

    throw new Error(`Configuration file not found. Attempted paths: ${attemptedPaths.join(', ')}`);
  }

  // Primary config file exists, read and parse it
  ...
  try {
    ...
    throw new Error(`Invalid JSON in configuration file ${configFile}: ${error.message}`);
  }
