...
class TicketTypeCache {
  ...
  async fetchFromDatabase() {
    ...
    try {
      ...
      // Note: Filtering by status is handled at the API layer based on environment (production vs non-production)
      const result = await db.execute({
        sql: `
          SELECT
            tt.*,
  ...
  async updateSoldCount(ticketTypeId, increment = 1) {
    ...
    try {
      ...
      if (this.cache.has(ticketTypeId)) {
        ...
        ticketType.sold_count = Number(ticketType.sold_count || 0) + increment;

        // Recalculate availability
        if (ticketType.max_quantity) {
          ticketType.available_quantity = Math.max(0,
            Number(ticketType.max_quantity) - ticketType.sold_count
          );
