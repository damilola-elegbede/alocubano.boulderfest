  ...
  try {
    ...
    for (const item of validatedItems) {
      ...
      if (ticketType.max_quantity && ticketType.max_quantity > 0) {
        ...
        let currentSoldCount = ticketType.sold_count || 0;
        ...
        while (retryCount < maxRetries && !updateSuccess) {
          ...
          if (newSoldCount > ticketType.max_quantity) {
            ...
            throw new Error(
              `Insufficient tickets: requested ${quantity}, only ${ticketType.max_quantity - currentSoldCount} available for ${ticketType.name}`
            );
          }
    ...
    try {
      ...
      // Use safeStringify to handle BigInt values in debug logging
      console.log(`[DEBUG] tickets array:`, safeStringify(tickets, 2));
      let attendeeEmailsSent = 0;
      for (const ticket of tickets) {
        console.log(`[DEBUG] Processing ticket ${ticket.ticket_id}, has attendee:`, !!ticket.attendee);
        // Use safeStringify to handle BigInt values in debug logging
        console.log(`[DEBUG] Ticket object:`, safeStringify(ticket, 2));
        if (ticket.attendee) {
