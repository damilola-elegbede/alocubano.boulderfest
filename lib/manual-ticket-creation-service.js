...
export async function createManualTickets(params) {
  ...
  try {
    ...
    for (const item of validatedItems) {
      ...
      if (ticketType.max_quantity && ticketType.max_quantity > 0) {
        const expectedSoldCount = ticketType.sold_count || 0;
        ...
        console.log(
          `✅ Atomically updated sold_count for ${ticketType.name}: ` +
          `${expectedSoldCount} -> ${newSoldCount}`
        );
      }
    }
    ...
    try {
      ...
      for (const rollback of soldCountUpdates) {
        try {
          ...
          console.error(
            `Failed to rollback sold_count for ${rollback.ticketTypeId}:`,
            rollbackError
          );
        }
      }
