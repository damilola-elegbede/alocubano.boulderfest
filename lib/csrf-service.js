  ...
  validateCSRF(handler) {
    ...
    return async (req, res) => {
      ...
      try {
        ...
        console.log(`🛡️  [CSRF] Getting CSRF token from request...`);
        ...
        console.log(`🛡️  [CSRF] CSRF token found:`, !!csrfToken);
        ...
        console.log(`🛡️  [CSRF] Getting session ID from request...`);
        // Get session ID from the request (from JWT in cookie or auth header)
        const sessionId = req.admin?.id || "anonymous";
        console.log(`🛡️  [CSRF] Session ID:`, sessionId, `(hasAdmin: ${!!req.admin})`);

        console.log(`🛡️  [CSRF] Verifying CSRF token...`);
        const result = this.verifyToken(csrfToken, sessionId);
        console.log(`🛡️  [CSRF] Token verification result:`, { valid: result.valid, error: result.error });

        if (!result.valid) {
          console.error("🛡️  [CSRF] Invalid CSRF token:", {
            error: result.error,
            sessionId: sessionId,
            hasAdmin: !!req.admin,
        ...
        console.log(`🛡️  [CSRF] CSRF validation passed, calling handler...`);
