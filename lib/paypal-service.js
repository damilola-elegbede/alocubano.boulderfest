...
import { Client, OrdersController, Environment } from '@paypal/paypal-server-sdk';
...
class PayPalService {
  ...
  async _performInitialization(req = null) {
    ...
    try {
      // Determine environment (use SDK's Environment enum)
      const paypalMode = process.env.PAYPAL_MODE || 'sandbox';
      const sdkEnvironment = paypalMode === 'production' ? Environment.Production : Environment.Sandbox;
      this.environment = paypalMode;
      ...
      console.log(`💳 PayPal Service: Initialized in ${paypalMode} environment`);
      this.initialized = true;
      return this.client;
  ...
  async createOrder(orderData, req = null) {
    ...
    try {
      ...
      // Use real PayPal SDK with OrdersController
      const createOrderRequest = {
        body: {
          intent: orderData.intent || 'CAPTURE',
          purchaseUnits: orderData.purchase_units || orderData.purchaseUnits || [],
          applicationContext: orderData.application_context || orderData.applicationContext || {}
        }
      };

      const response = await this.ordersController.createOrder(createOrderRequest);
  ...
  async captureOrder(orderId, captureData = {}, req = null) {
    ...
    try {
      ...
      const response = await this.ordersController.captureOrder(captureOrderRequest);
  ...
  async getOrder(orderId, req = null) {
    ...
    try {
      ...
      const response = await this.ordersController.getOrder(getOrderRequest);
