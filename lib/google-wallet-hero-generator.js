...
export class GoogleWalletHeroGenerator {
  ...
  async generateHeroImage(ticketTypeId, options = {}) {
    ...
    if (circlePosition === "center") {
      ...
      circleX = (this.heroWidth - this.circleSize) / 2;
      circleY = (this.heroHeight - this.circleSize) / 2;
    ...
    try {
      // Load background image for watermark (same as Apple Wallet)
      const backgroundPath = path.join(this.projectRoot, "public", "wallet", "background.png");
