...
class TicketColorService {
   ...
   *
   * Pattern matching uses first-match-wins strategy with priority determined by display_order.
   * Lower display_order values have higher priority. For example:
   * - 'test-' (order 1) matches before 'full' (order 3)
   * - 'friday' (order 5) matches before 'weekender' (order 8)
   *
   * This ensures specific patterns like 'test-' are matched before more general patterns,
   * and single-day tickets are matched before multi-day pass patterns.
   *
  ...
  async getColorForTicketType(ticketTypeId) {
    ...
    // Priority: Patterns are tested in the order they were inserted into the Map,
    // which matches display_order ASC from the DB query (lower = higher priority)
  ...
  async fetchFromDatabase() {
    ...
    try {
      ...
      return; // Exit gracefully - fallback mappings allow continued operation
    }
  }

  /**
   * Load fallback color mappings if database is unavailable
   * NOTE: These mappings must be kept in sync with config/bootstrap.json
   * Update this fallback whenever the bootstrap color configuration changes.
   */
  loadFallbackMappings() {
    ...
    this.cache.set('class', { name: 'Single Class', rgb: 'rgb(34, 139, 34)', emoji: '⬤', display_order: 8 });
    this.cache.set('weekender', { name: 'Weekender', rgb: 'rgb(255, 255, 255)', emoji: '⬤', display_order: 9 });
    this.cache.set('weekend', { name: 'Weekend', rgb: 'rgb(255, 255, 255)', emoji: '⬤', display_order: 10 });
