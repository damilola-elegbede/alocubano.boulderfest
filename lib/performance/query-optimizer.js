...
export class QueryOptimizer extends EventEmitter {
  ...
  async executeWithTracking(queryOrObject, params = [], options = {}) {
    ...
    try {
      ...
      // Execute query using original execute method to prevent infinite recursion
      let result;
      const executeMethod =
        this.originalExecute || this.db.execute.bind(this.db);

      if (prepared && this.dbType === "postgresql") {
        result = await executeMethod(prepared.query, args);
      } else {
        result = await executeMethod(queryOrObject, params);
      }
    ...
    const optimizations = {
      ...
      [QUERY_CATEGORIES.EVENT_STATISTICS]: (query) => {
        ...
        if (
          ...
          !query.includes("TABLESAMPLE")
        ) {
          return query.replace(
            /FROM tickets/i,
            "FROM tickets TABLESAMPLE SYSTEM (10)",
          );
        }
        return query;
      },
...
export function withQueryOptimization(databaseService, options = {}) {
  ...
  databaseService.execute = async function (query, params, opts) {
    return optimizer.executeWithTracking.call(optimizer, query, params, opts);
  };
