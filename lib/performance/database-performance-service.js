...
class DatabasePerformanceService extends EventEmitter {
  ...
  async initialize() {
    ...
    try {
      ...
      this.optimizer = createQueryOptimizer(this.db);
  ...
  wrapDatabaseMethods() {
    ...
    const originalBatch = this.db.batch ? this.db.batch.bind(this.db) : null;
    ...
    // Wrap execute method
    this.db.execute = async (queryOrObject, params = []) => {
      if (this.optimizer) {
    ...
    if (originalBatch) {
      this.db.batch = async (statements) => {
