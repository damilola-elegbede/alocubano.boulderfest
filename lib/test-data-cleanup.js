...
export class TestDataCleanupService {
  ...
  buildCleanupFilter(criteria) {
    // Combined test detection: is_test flag OR name patterns
    // This ensures reliable cleanup even when is_test flag is not set
    const testDetectionConditions = [
      'is_test = 1',
      // Event name patterns (case-insensitive)
      `event_id IN (SELECT id FROM events WHERE LOWER(name) LIKE '%test%')`,
      // Transaction ID patterns (case-insensitive)
      `LOWER(transaction_id) LIKE '%test%'`,
      // Email patterns for test users
      `LOWER(customer_email) LIKE '%test%'`,
      `LOWER(customer_email) LIKE '%+test@%'`
    ];

    const conditions = [`(${testDetectionConditions.join(' OR ')})`];
    const params = [];
  ...
  async identifyCleanupCandidates(client, cleanupFilter) {
    try {
      ...
      if (transactionIds.length > 0) {
        ...
        // Combined test detection for tickets: is_test flag OR name patterns
        const ticketQuery = `
          ...
          WHERE transaction_id IN (${placeholders})
            AND (
              is_test = 1
              OR LOWER(ticket_id) LIKE '%test%'
              OR LOWER(attendee_email) LIKE '%test%'
              OR LOWER(attendee_email) LIKE '%+test@%'
            )
        `;

        // Combined test detection for transaction items
