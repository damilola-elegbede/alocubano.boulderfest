import jwt from 'jsonwebtoken';
import crypto from 'crypto';
import { getDatabaseClient } from './database.js';
import { processDatabaseResult } from './bigint-serializer.js';

export class RegistrationTokenService {
  constructor() {
    this.secret = null;
    // Removed tokenExpiry - now calculated per event (end_date + 7 days)
    this._initPromise = null;
    this.initialized = false;
    this.db = null;
  }

  async ensureInitialized() {
    if (this.initialized && this.secret && this.db) {
      return this;
    }

    if (this._initPromise) {
      return this._initPromise;
    }

    this._initPromise = this._performInitialization();

    try {
      await this._initPromise;
      return this;
    } catch (error) {
      this._initPromise = null;
      this.initialized = false;
      this.secret = null;
      this.db = null;
      throw error;
    }
  }

  async _performInitialization() {
    try {
      // Initialize secret
      const secret = process.env.REGISTRATION_SECRET;
      if (!secret || secret.length < 32) {
        throw new Error('REGISTRATION_SECRET must be set (>=32 chars) for token signing');
      }
      this.secret = secret;

      // Initialize database connection
      // In integration test mode, use the test isolation manager's database
      if (process.env.INTEGRATION_TEST_MODE === 'true') {
        try {
          const { getTestIsolationManager } = await import('./test-isolation-manager.js');
          const isolationManager = getTestIsolationManager();
          this.db = await isolationManager.getScopedDatabaseClient();
        } catch (error) {
          console.warn('[RegistrationTokenService] Failed to get test database, falling back to standard database:', error.message);
          this.db = await getDatabaseClient();
        }
      } else {
        this.db = await getDatabaseClient();
      }

      if (!this.db) {
        throw new Error('Failed to get database client - db is null');
      }

      this.initialized = true;
      return this;
    } catch (error) {
      this.initialized = false;
      this.secret = null;
      this.db = null;
      throw error;
    }
  }

  /**
   * Get latest event end date for tickets in a transaction
   * Uses MAX(end_date) for deterministic results with multi-event transactions
   * @param {string|bigint} transactionId - Transaction ID (may be BigInt from database)
   * @returns {Promise<string|null>} Latest event end date or null
   */
  async getEventEndDateForTransaction(transactionId) {
    await this.ensureInitialized();
    // CRITICAL FIX: Convert BigInt to string for database query compatibility
    const transactionIdStr = typeof transactionId === 'bigint' ? String(transactionId) : transactionId;
    
    const result = await this.db.execute({
      sql: `SELECT MAX(e.end_date) as end_date
            FROM events e
            JOIN tickets t ON t.event_id = e.id
            WHERE t.transaction_id = ?`,
      args: [transactionIdStr]
    });

    // BIGINT FIX: Sanitize database result before accessing properties
    const sanitized = processDatabaseResult(result.rows[0]);
    return sanitized?.end_date;
  }

  async createToken(transactionId) {
    await this.ensureInitialized();
    const tokenId = crypto.randomUUID();
    const now = Date.now();

    // Look up event end date for this transaction
    const eventEndDate = await this.getEventEndDateForTransaction(transactionId);

    let expiresAt;
    if (eventEndDate) {
      // Set expiration to 7 days after event ends
      const eventEnd = new Date(eventEndDate);
      eventEnd.setDate(eventEnd.getDate() + 7);
      expiresAt = eventEnd.getTime();
      console.log(`[RegToken] Token expires 7 days after event (${eventEnd.toISOString()})`);
    } else {
      // Fallback: 1 year if event lookup fails (shouldn't happen in production)
      expiresAt = now + (365 * 24 * 60 * 60 * 1000);
      console.warn('[RegToken] Event lookup failed, using 1-year fallback');
    }

    // Convert BigInt to string to avoid serialization errors
    const txnId = typeof transactionId === 'bigint' ?
      transactionId.toString() :
      String(transactionId);

    // Create JWT with event-based expiration
    const token = jwt.sign({
      tid: tokenId,
      txn: txnId,
      type: 'registration',
      iat: Math.floor(now / 1000),
      exp: Math.floor(expiresAt / 1000)
    }, this.secret, { algorithm: 'HS256' });

    // Store server-side state for tracking
    await this.db.execute({
      sql: `UPDATE transactions
            SET registration_token = ?,
                registration_token_expires = ?,
                registration_initiated_at = ?
            WHERE id = ?`,
      args: [token, new Date(expiresAt).toISOString(), new Date().toISOString(), transactionId]
    });

    await this.logTokenUsage(tokenId, null, 'created');
    return token;
  }

  async validateAndConsumeToken(token, ipAddress) {
    await this.ensureInitialized();
    const startTime = Date.now();

    try {
      // Verify JWT signature and expiration
      const decoded = jwt.verify(token, this.secret);

      // Check server-side state
      const result = await this.db.execute({
        sql: `SELECT * FROM transactions
              WHERE registration_token = ?
              AND datetime(registration_token_expires) > datetime('now')`,
        args: [token]
      });

      if (result.rows.length === 0) {
        throw new Error('Token invalid or expired');
      }

      const transaction = result.rows[0];

      // Check if already used (all tickets registered)
      if (transaction.all_tickets_registered) {
        throw new Error('Registration already completed');
      }

      // Atomically consume the token to enforce single-use
      await this.db.execute({
        sql: `UPDATE transactions
              SET registration_token = NULL,
                  registration_token_expires = NULL
              WHERE id = ?
                AND registration_token = ?`,
        args: [transaction.id, token]
      });

      // Log token usage
      const validationTime = Date.now() - startTime;
      await this.logTokenUsage(decoded.tid, ipAddress, 'validated');

      // Check performance target
      if (validationTime > 100) {
        console.warn(`Token validation exceeded 100ms: ${validationTime}ms`);
      }

      return {
        transactionId: transaction.id,
        customerId: transaction.customer_email,
        expiresAt: transaction.registration_token_expires
      };
    } catch (error) {
      await this.logTokenUsage(null, ipAddress, 'failed', error.message);
      throw error;
    }
  }

  async revokeToken(transactionId) {
    await this.ensureInitialized();
    await this.db.execute({
      sql: `UPDATE transactions
            SET registration_token = NULL,
                registration_token_expires = NULL
            WHERE id = ?`,
      args: [transactionId]
    });
  }

  async logTokenUsage(tokenId, ipAddress, action, errorMessage = null) {
    // Implement audit logging per REQ-SEC-005
    console.log(JSON.stringify({
      event: 'registration_token_usage',
      tokenId: tokenId?.substring(0, 8),
      ip: ipAddress,
      action,
      error: errorMessage,
      timestamp: new Date().toISOString()
    }));
  }
}

// Export singleton instance
export const registrationTokenService = new RegistrationTokenService();

// Singleton instance for getter function
let registrationTokenServiceInstance = null;

/**
 * Get singleton instance of RegistrationTokenService
 * @returns {RegistrationTokenService} Singleton service instance
 */
export function getRegistrationTokenService() {
  if (!registrationTokenServiceInstance) {
    registrationTokenServiceInstance = new RegistrationTokenService();
  }
  return registrationTokenServiceInstance;
}