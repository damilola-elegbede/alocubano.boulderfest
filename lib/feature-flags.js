...
class FeatureFlagContext {
  constructor(options = {}) {
    this.userId = optionalField(options.userId);
    this.sessionId = optionalField(options.sessionId);
    this.requestId = optionalField(options.requestId);
    this.environment = options.environment || EnvironmentDetector.detectEnvironment();
    this.timestamp = options.timestamp || Date.now();
    this.userAgent = optionalField(options.userAgent);
    this.ipAddress = optionalField(options.ipAddress);
  ...
  getStatistics() {
    ...
    for (const [key, flag] of this.flags.entries()) {
      stats.flags[key] = {
        ...
        rolloutPercentage: optionalField(this.rolloutConfig.get(key)?.percentage)
      };
    }
