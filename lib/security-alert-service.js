...
export class SecurityAlertService {
  ...
  /**
   * Record security metrics for analysis and alerting
   */
  async recordMetric(params) {
    try {
      await this.ensureInitialized();
      ...
      return await this._executeWithRetry(async () => {
        const result = await this.db.execute({
          sql: `INSERT INTO security_alert_metrics (
            metric_type, metric_value, timeframe, entity_type, entity_id,
            ip_address, metadata
          ) VALUES (?, ?, ?, ?, ?, ?, ?)`,
          args: [
            metricType,
      ...
      logger.error('[SecurityAlert] Failed to record metric:', error.message);
      return {
        success: false,
        error: error.message
      };
