...
export class BootstrapSchemaValidator {
  constructor() {
    ...
    // Events table: migration 019 CHECK(status IN ('draft', 'upcoming', 'active', 'completed', 'cancelled', 'test'))
    this.allowedEventStatuses = ['draft', 'upcoming', 'active', 'completed', 'cancelled', 'test'];
    this.allowedEventTypes = ['festival', 'weekender', 'workshop', 'social'];
    // Ticket_types table: migration 060 CHECK(status IN ('available', 'sold-out', 'coming-soon', 'closed', 'test', 'unavailable'))
    this.allowedTicketStatuses = ['available', 'sold-out', 'coming-soon', 'closed', 'test', 'unavailable'];
  ...
  validateBusinessRules(data, result) {
    ...
    // Skip this check for completed events or events in the past
    ...
    for (let i = 0; i < eventsArray.length; i++) {
      ...
      if (event.id !== undefined && !eventIdsWithTickets.has(event.id)) {
        // Skip warning for completed events or events with past end dates
