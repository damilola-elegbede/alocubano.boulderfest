    ...
    try {
      ...
      if (process.env.INTEGRATION_TEST_MODE === 'true') {
        try {
          ...
          logger.warn('[FinancialAuditQueries] Failed to get test database, falling back to standard database:', error.message);
      ...
      const result = await this.db.execute({
        sql: `
          ...
          SET reconciliation_status = ?,
              reconciliation_date = CASE WHEN ? = 'reconciled' THEN datetime('now') ELSE reconciliation_date END,
