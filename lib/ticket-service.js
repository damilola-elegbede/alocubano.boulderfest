...
export class TicketService {
  ...
  async createSingleTicket(transaction, item, index, total, req = null) {
    ...
    // Generate test-aware ticket ID
    const ticketId = generateTestAwareTicketName(baseTicketId, req);
  ...
  /**
   * Get tickets by email with test mode filtering
   */
  async getTicketsByEmail(email, req = null) {
    // Apply test mode filtering
    const testFilter = createTestModeFilter('t', null, req);

    const result = await this.db.execute({
      sql: `SELECT t.*, tr.uuid as order_number
            FROM tickets t
            JOIN transactions tr ON t.transaction_id = tr.id
            WHERE (t.attendee_email = ? OR tr.customer_email = ?)${testFilter.sql}
            ORDER BY t.created_at DESC`,
      args: [email, email, ...testFilter.args],
    });
    return result.rows;
  }
   ...
   */
  async getTicketStatistics(req = null) {
    const result = await this.db.execute({
      sql: `SELECT
  ...
  /**
   * Get all tickets with test mode filtering and pagination
   */
  async getAllTickets(options = {}, req = null) {
    const {
      limit = 100,
      offset = 0,
      status = null,
    ...
    let sql = `SELECT * FROM tickets WHERE 1=1${testFilter.sql}`;
