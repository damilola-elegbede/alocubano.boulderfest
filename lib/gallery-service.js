...
class GalleryService {
  ...
  async generateRuntimeData(year, eventId) {
    try {
      ...
      // Re-throw specific errors that should fail fast
      if (error.message.includes('No gallery found for year') ||
          error.message.includes('GOOGLE_DRIVE_ERROR') ||
          error.message.includes('Google Drive credentials not configured')) {
        throw error;
      }

      // Return empty data for other errors (graceful degradation)
      return {
        categories: {
          workshops: [],
          socials: [],
