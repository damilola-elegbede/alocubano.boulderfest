...
import { transformGalleryData } from './url-transformer.js';
    ...
    try {
      ...
      if (shouldUseBuildTimeCache()) {
        ...
        if (cachedData) {
          ...
          const isPlaceholderCache = cachedData.isPlaceholder ||
                                     (cachedData.totalCount === 0 &&
                                      cachedData.message &&
                                      cachedData.message.includes('Empty event gallery cache'));

          if (isPlaceholderCache) {
            console.log('Gallery: Skipping placeholder cache data - proceeding to runtime generation');
            // Skip placeholder cache and proceed to runtime generation for graceful degradation
          } else {
            this.metrics.cacheHits++;
            this.updateResponseTime(startTime);
            console.log('Gallery: Serving from build-time cache');
            // Transform Vercel Blob URLs to image proxy URLs
            const transformedData = transformGalleryData(cachedData);
            return { ...transformedData, source: 'build-time-cache' };
          }
        }
      }
      ...
      if (runtimeCache && !this.isCacheExpired(cacheKey)) {
        ...
        // Consider cache as placeholder if explicitly marked OR if it has no real data
        const isPlaceholderCache = decompressedCache.isPlaceholder ||
                                   (decompressedCache.totalCount === 0 &&
                                    decompressedCache.message &&
                                    decompressedCache.message.includes('Empty event gallery cache'));

        if (isPlaceholderCache) {
          console.log('Gallery: Skipping placeholder runtime cache data - proceeding to fresh generation');
          // Skip placeholder cache and proceed to fresh generation for graceful degradation
        } else {
          this.metrics.cacheHits++;
          this.updateResponseTime(startTime);
          console.log('Gallery: Serving from runtime cache');
          // Transform Vercel Blob URLs to image proxy URLs
          const transformedData = transformGalleryData(decompressedCache);
          return { ...transformedData, source: 'runtime-cache' };
        }
      }
      ...
      // Transform Vercel Blob URLs to image proxy URLs
      const transformedData = transformGalleryData(freshData);
      return { ...transformedData, source: 'runtime-generated' };
      ...
      if (shouldUseBuildTimeCache()) {
        ...
        if (cachedData) {
          ...
          if (cachedData.items && !cachedData.photos) {
            ...
            // Transform Vercel Blob URLs to image proxy URLs
            return transformGalleryData(convertedData);
          }
          // Transform Vercel Blob URLs to image proxy URLs
          return transformGalleryData(cachedData);
        }
      }
