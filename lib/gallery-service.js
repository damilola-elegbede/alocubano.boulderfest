  ...
  async generateRuntimeData(year, eventId) {
    try {
      ...
      if (!hasGoogleDriveConfig) {
        // Fail fast when no credentials are configured and no cache is available
        console.log('Gallery: Google Drive not configured - failing fast');
        throw new Error('FATAL: Google Drive secret not configured. No credentials available for runtime data generation.');
      }
