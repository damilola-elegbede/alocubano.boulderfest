 ...
 * @throws {TypeError} If input is invalid and cannot be safely converted to a price
 */
export function formatPrice(priceCents) {
  // Guard against null/undefined - treat as zero
  if (priceCents == null) {
    return '0.00';
  }
  
  // Convert to number and validate
  const numericValue = Number(priceCents);
  
  // Guard against NaN, Infinity, and invalid conversions
  if (!Number.isFinite(numericValue)) {
    console.warn(`[formatPrice] Invalid price value: ${priceCents}`);
    return '0.00';
  }
  
  // Guard against negative prices (security concern)
  const safePriceCents = Math.max(0, numericValue);
  return (safePriceCents / 100).toFixed(2);
}
