    ...
    switch (type) {
      ...
      default:
        ...
        const shouldSkipRedis = 
          ...
          (process.env.CI === "true" && !process.env.REDIS_URL && !process.env.REDIS_CONNECTION_STRING) ||
          (process.env.NODE_ENV === "test" && !process.env.REDIS_URL && !process.env.REDIS_CONNECTION_STRING);

        if (!shouldSkipRedis && (process.env.REDIS_URL || process.env.REDIS_CONNECTION_STRING)) {
          console.log("Redis detected, using multi-tier cache");
          instance = createMultiTierCache(config || environmentConfig);
        } else {
          console.log("Using memory cache only (Redis skipped or unavailable)");
