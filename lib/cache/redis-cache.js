...
class RedisCache {
  ...
  /**
   * Check if Redis should be skipped in current environment
   */
  shouldSkipRedis() {
    // Skip Redis if explicitly disabled
    if (process.env.SKIP_REDIS === "true" || process.env.DISABLE_REDIS === "true") {
      return true;
    }

    // Skip Redis in CI environments without Redis URL
    ...
    // Skip Redis in test environment by default
    if (process.env.NODE_ENV === "test" && !hasRedisUrl) {
      return true;
    }
  ...
  async init() {
    ...
    // Skip Redis in CI/test environments without Redis
    if (this.shouldSkipRedis()) {
      console.log("Skipping Redis connection (CI/test environment)");
      this.initialized = false;
      this.isConnected = false;
      return false;
    }
    ...
    try {
      ...
      this.client.on("error", (err) => {
        // Reduce log pollution in CI/test environments
        const isCI = process.env.CI === "true" || process.env.GITHUB_ACTIONS === "true";
  ...
  async delPattern(pattern, options = {}) {
    ...
    try {
      ...
      do {
        ...
        // Handle node-redis v4 format: result is {cursor, keys} object
        cursor = result.cursor || 0;
        const scanKeys = Array.isArray(result.keys) ? result.keys : [];
  ...
  async incr(key, options = {}) {
    ...
    try {
      ...
      // Set TTL if this is a new key (check existence first)
      if (result === amount) {
        const ttlSeconds = ttl || this.getTtl(type);
        const keyExists = await this.client.exists(cacheKey);
        if (keyExists === 1) {
          await this.client.expire(cacheKey, ttlSeconds);
        }
      }
