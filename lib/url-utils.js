...
export function buildRegistrationUrl(token, ticketId = null) {
  ...
  const url = `${baseUrl}/register-tickets?token=${encodeURIComponent(token)}`;
  return ticketId ? `${url}&ticketId=${encodeURIComponent(ticketId)}` : url;
}
...
export function buildViewTicketsUrl(token, ticketId = null) {
  const baseUrl = getBaseUrl();
  const url = `${baseUrl}/view-tickets?token=${encodeURIComponent(token)}`;
  return ticketId ? `${url}&ticketId=${encodeURIComponent(ticketId)}` : url;
}
...
export function buildQRCodeUrl(token) {
  const baseUrl = getBaseUrl();
  return `${baseUrl}/api/qr/generate?token=${encodeURIComponent(token)}`;
}
...
export function buildWalletPassUrls(ticketId) {
  const baseUrl = getBaseUrl();
  return {
    apple: `${baseUrl}/api/tickets/apple-wallet/${encodeURIComponent(ticketId)}`,
    google: `${baseUrl}/api/tickets/google-wallet/${encodeURIComponent(ticketId)}`
  };
}
