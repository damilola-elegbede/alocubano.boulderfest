...
export class AppleWalletService {
  ...
  isTestTicket(ticket) {
    ...
    if (!ticket.ticket_id) {
      console.error('[AppleWalletService] Missing ticket_id field, cannot detect test ticket reliably', { ticket });
    }
  ...
  /**
   * Generate a colored circle thumbnail image as PNG buffer
   * @param {string} rgb - RGB color string (e.g., "rgb(255, 20, 147)")
   * @param {number} size - Image size in points (90, 180, or 270 for @1x, @2x, @3x)
   * @returns {Promise<Buffer>} PNG image buffer
   */
  async generateThumbnailCircle(rgb, size = 90) {
    const svgCircle = `
  ...
  async createPassFile(ticket, serialNumber) {
    ...
    const qrToken = await qrTokenService.getOrCreateToken(String(ticket.ticket_id));
    ...
    console.log(`  - preferredStyleSchemes: ${safeStringify(passProps.preferredStyleSchemes)}`);
    console.log(`  - useAutomaticColor: ${passProps.useAutomaticColor}`);

    // Generate colored circle thumbnail images for ticket type indicator
    const colorService = getTicketColorService();
    let ticketColor;
    try {
      ticketColor = await colorService.getColorForTicketType(ticket.ticket_type);
    } catch (error) {
      console.error('[Apple Wallet] Failed to get ticket color:', error);
      ticketColor = { name: 'Default', rgb: 'rgb(255, 255, 255)' };
    }

    console.log(`[Apple Wallet] Generating thumbnail: ${ticketColor.name} (${ticketColor.rgb})`);
    const thumbnail1x = await this.generateThumbnailCircle(ticketColor.rgb, 90);
    const thumbnail2x = await this.generateThumbnailCircle(ticketColor.rgb, 180);
    const thumbnail3x = await this.generateThumbnailCircle(ticketColor.rgb, 270);

    // CORRECT v3 API usage:
    // new PKPass(buffers, certificates, props)
    const pass = new PKPass({
      'thumbnail.png': thumbnail1x,
      'thumbnail@2x.png': thumbnail2x,
      'thumbnail@3x.png': thumbnail3x
    }, certificateConfig, passProps);
    ...
    // Auxiliary fields (ATTENDEE and VENUE)
    console.log(`[Apple Wallet] Adding auxiliary fields`);

    // Attendee name (thumbnail provides color indicator)
    ...
    try {
      ...
      // Background image - full pass watermark (375x466 points, 20% opacity logo)
    ...
    console.log(safeStringify(pass.props, null, 2));
  ...
  async logPassEvent(ticketId, eventType, eventData = {}) {
    ...
    await db.execute({
      ...
      args: [passSerial, eventType, safeStringify(eventData)],
    });
  }
  ...
  buildMountainTimeISO(date, time) {
    ...
    const isoWithoutOffset = `${date}T${time}:00`;

    // Formatter with shortOffset to get actual timezone offset
    ...
    // Try both possible Mountain Time offsets to find the correct one
    for (const guessOffset of ["-07:00", "-06:00"]) {
      const candidate = new Date(`${isoWithoutOffset}${guessOffset}`);
      const parts = formatter.formatToParts(candidate);
      const partsMap = Object.fromEntries(parts.map((part) => [part.type, part.value]));

      // Check if this offset produces the correct date/time components
      if (
        partsMap.year === year &&
        partsMap.month === month &&
        partsMap.day === day &&
        partsMap.hour === hour &&
        partsMap.minute === minute
      ) {
        // Extract actual offset from timeZoneName (e.g., "GMT-7" or "GMT-6")
        const match = partsMap.timeZoneName.match(/GMT([+-]\d{1,2})(?::(\d{2}))?/);
        if (match) {
          // Handle sign separately to avoid padStart issues (e.g., "-7" should become "-07", not "0-7")
          const sign = match[1].startsWith('-') ? '-' : '+';
          const hours = match[1].replace(/[+-]/, '').padStart(2, '0');
          const minutes = match[2] || '00';
          const resolvedOffset = `${sign}${hours}:${minutes}`;
          return `${isoWithoutOffset}${resolvedOffset}`;
        }
        return `${isoWithoutOffset}${guessOffset}`;
      }
    }

    // Fallback if offset resolution fails
    console.warn("Unable to resolve Mountain Time offset accurately, falling back to default");
    return this.eventStartDate || new Date(`${isoWithoutOffset}Z`).toISOString();
  }
}
