...
export class AppleWalletService {
  constructor() {
    ...
    // Critical wallet secrets - store initialization status instead of throwing
    this.initializationError = null;

    if (!process.env.APPLE_PASS_KEY) {
      console.warn("⚠️  APPLE_PASS_KEY not configured - Apple Wallet functionality disabled");
      this.initializationError = new Error("APPLE_PASS_KEY secret not configured");
      return;
    }
    if (!process.env.WALLET_AUTH_SECRET) {
      console.warn("⚠️  WALLET_AUTH_SECRET not configured - Apple Wallet functionality disabled");
      this.initializationError = new Error("WALLET_AUTH_SECRET secret not configured");
      return;
    }
