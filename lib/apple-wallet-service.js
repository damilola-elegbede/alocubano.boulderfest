...
export class AppleWalletService {
  ...
  decodeCertificate(base64String, certType) {
    ...
    try {
      ...
      if (!hasHeaders) {
        // If no headers, assume raw base64 and add appropriate PEM wrapper
        // Clean the content (remove any whitespace/newlines)
        const cleanContent = pemContent.replace(/\s/g, "");

        // Add PEM headers based on certificate type
        const beginHeader = `-----BEGIN ${certType}-----`;
        const endHeader = `-----END ${certType}-----`;
      ...
      // Validate PEM format
      const expectedBegin = `-----BEGIN ${certType}-----`;
      const expectedEnd = `-----END ${certType}-----`;

      if (!pemContent.includes(expectedBegin) || !pemContent.includes(expectedEnd)) {
        throw new Error(
          `Invalid PEM format for ${certType}. ` +
          `Expected headers: ${expectedBegin} and ${expectedEnd}`
        );
      }
