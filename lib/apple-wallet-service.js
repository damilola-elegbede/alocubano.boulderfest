...
export class AppleWalletService {
  ...
  async createPassFile(ticket, serialNumber) {
    ...
    // Get color for ticket type and map to colored emoji
    const colorService = getTicketColorService();
    const ticketColor = await colorService.getColorForTicketType(ticket.ticket_type);

    // Map to colored emoji (testing if colored emojis work in Apple Wallet)
    let colorEmoji = '⬤'; // Default white circle
    if (ticketColor.name === 'Test') {
      colorEmoji = '\u{1F7E3}'; // 🟣 Purple Circle - TEST
    }
    console.log(`[Apple Wallet] Ticket color: ${ticketColor.name} → Emoji: ${colorEmoji}`);

    // Attendee name with colored emoji
    const attendeeName = `${ticket.attendee_first_name || ""} ${ticket.attendee_last_name || ""}`.trim() || "Guest";
    pass.auxiliaryFields.push({
      key: "attendee",
      label: "ATTENDEE",
      value: `${attendeeName} ${colorEmoji}`,
    });
