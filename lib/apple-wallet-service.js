...
export class AppleWalletService {
  ...
  getTestModeWalletStyling(isTest) {
    if (!isTest) {
      return {
        ...
        labelColor: "#ce1126",         // Cuban flag red for labels
      };
    }

    return {
      backgroundColor: "#fff8dc",      // Light yellow background (cornsilk)
      foregroundColor: "#8b4513",      // Dark brown text (saddlebrown)
      labelColor: "#ff8c00",           // Orange labels (darkorange)
    };
  }
  ...
  async createPassFile(ticket, serialNumber) {
    ...
    const passProps = {
      ...
      foregroundColor: "#000000",              // Black text for field values and logo
      backgroundColor: "#ffffff",              // White background
      labelColor: "#ce1126",                   // Cuban red labels only
      logoText: "A Lo Cubano | Boulder Fest",
    ...
    console.log(`  - logoText: "${passProps.logoText}"`);
    console.log(`  - barcode format: ${passProps.barcodes[0].format}`);
    console.log(`  - barcode message length: ${passProps.barcodes[0].message.length} chars`);

    // CORRECT v3 API usage:
    // new PKPass(buffers, certificates, props)
    const pass = new PKPass({}, certificateConfig, passProps);

    // CRITICAL: Set the pass type BEFORE adding fields
    // Setting type initializes empty field arrays
    ...
    // Primary fields
    console.log(`[Apple Wallet] Adding primary field - EVENT: "${ticket.event_name}"`);
    ...
    try {
      ...
      // Background image - 180x220 points (with watermark)
      ...
      const background2xPath = path.join(
        projectRoot,
        "public",
        "wallet",
        "background@2x.png",
      ...
      const background3xPath = path.join(
        projectRoot,
        "public",
        "wallet",
        "background@3x.png",
