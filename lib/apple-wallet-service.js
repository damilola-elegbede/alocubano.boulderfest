...
export class AppleWalletService {
   ...
   * Handles multiple PEM header variants (RSA PRIVATE KEY, PRIVATE KEY, etc.)
   */
  decodeCertificate(base64String, certType) {
    ...
    try {
      // Decode from base64
      let pemContent = Buffer.from(base64String, "base64").toString("utf8");

      console.log(`[Apple Wallet] Decoding ${certType}...`);
      console.log(`[Apple Wallet] Content length: ${pemContent.length} chars`);
      console.log(`[Apple Wallet] First 50 chars: ${pemContent.substring(0, 50)}`);

      // Check if already has PEM headers
      const hasHeaders = pemContent.includes("-----BEGIN") && pemContent.includes("-----END");

      if (hasHeaders) {
        console.log(`[Apple Wallet] ${certType} already has PEM headers`);

        // Extract the actual header type from the content
        const beginMatch = pemContent.match(/-----BEGIN ([^-]+)-----/);
        const endMatch = pemContent.match(/-----END ([^-]+)-----/);
        ...
        if (beginMatch && endMatch) {
          ...
          pemContent = pemContent.trim().replace(/\r\n/g, "\n");
        } else {
          throw new Error("Could not parse PEM headers");
        }
      } else {
        console.log(`[Apple Wallet] ${certType} missing PEM headers, adding them...`);

        // Clean the content (remove any whitespace/newlines)
        const cleanContent = pemContent.replace(/\s/g, "");

        // Determine appropriate header based on certificate type
        let headerType = certType;

        // For private keys, try to detect the key type from the content
        if (certType === "PRIVATE KEY") {
          // Check if content looks like it might be RSA (starts with MII which is common for RSA)
          if (cleanContent.startsWith("MII")) {
            // Could be RSA, but use standard PKCS#8 header for compatibility
            headerType = "PRIVATE KEY";
          }
        }

        const beginHeader = `-----BEGIN ${headerType}-----`;
        const endHeader = `-----END ${headerType}-----`;

        // Format with 64 character line breaks (PEM standard)
        const formattedContent = cleanContent.match(/.{1,64}/g)?.join("\n") || cleanContent;

        pemContent = `${beginHeader}\n${formattedContent}\n${endHeader}`;
        console.log(`[Apple Wallet] Added PEM headers: ${headerType}`);
      }

      // Final validation - just check that we have matching BEGIN/END headers
      const finalBeginMatch = pemContent.match(/-----BEGIN ([^-]+)-----/);
      const finalEndMatch = pemContent.match(/-----END ([^-]+)-----/);

      if (!finalBeginMatch || !finalEndMatch) {
        throw new Error("Invalid PEM format: missing BEGIN or END headers");
      }

      if (finalBeginMatch[1] !== finalEndMatch[1]) {
        throw new Error(
          `Invalid PEM format: header mismatch (${finalBeginMatch[1]} vs ${finalEndMatch[1]})`
        );
      }

      const finalType = finalBeginMatch[1];
      console.log(`[Apple Wallet] Final PEM type: ${finalType}, length: ${pemContent.length} chars`);

      // Return as Buffer for passkit-generator
      ...
      throw new Error(
        ...
        `Ensure the environment variable contains valid base64-encoded PEM format.`
      );
    }
  }
  ...
  async createPassFile(ticket, serialNumber) {
    ...
    const pass = new PKPass({ type: "eventTicket" }, certificateConfig);
    ...
    // Explicitly set type property after initialization to ensure it persists
    ...
    if (process.env.WALLET_ENABLE_UPDATES === "true") {
      pass.webServiceURL = `${this.baseUrl}/api/tickets/apple-wallet`;
