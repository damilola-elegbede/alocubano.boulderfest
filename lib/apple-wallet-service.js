...
export class AppleWalletService {
  ...
  tryExtractFromPKCS12(buffer, password) {
    ...
    try {
      ...
      console.log("[Apple Wallet] Extracted from PKCS#12:");
      console.log(`  - Certificate: ${certificate.length} chars`);
  ...
  decodeCertificate(base64String, certType) {
    ...
    try {
      ...
      console.log(`[Apple Wallet] Decoded buffer length: ${decodedBuffer.length} bytes`);
      ...
      if (isDerFormat) {
        ...
        console.log(`[Apple Wallet] DER validation passed: ${decodedBuffer.length} bytes`);
        ...
        console.log(`[Apple Wallet] PEM length: ${pemContent.length} chars, ${lines.length} lines`);
  ...
  async createPassFile(ticket, serialNumber) {
    ...
    const passProps = {
      ...
      foregroundColor: "rgb(0, 0, 0)",        // Black text
      backgroundColor: "rgb(255, 255, 255)",   // White background
      labelColor: "rgb(206, 17, 38)",         // Cuban red labels
