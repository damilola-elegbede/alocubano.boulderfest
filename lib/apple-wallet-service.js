...
export class AppleWalletService {
  ...
  async createPassFile(ticket, serialNumber) {
    ...
    const passProps = {
      ...
      useAutomaticColor: false,                // iOS 18+ - Disable auto text color detection from background image
    ...
    // CRITICAL: Re-apply colors after setting type (in case type resets passProps colors)
    // This ensures black text displays on white background with background image
    pass.foregroundColor = "rgb(0, 0, 0)";        // Black text for field values
    pass.backgroundColor = "rgb(255, 255, 255)";   // White background
    pass.labelColor = "rgb(206, 17, 38)";          // Cuban red labels
    console.log(`[Apple Wallet] Colors re-applied after type: foreground=black, background=white, label=red`);
    ...
    // Attendee name with black circle (no label)
    const attendeeName = `${ticket.attendee_first_name || ""} ${ticket.attendee_last_name || ""}`.trim() || "Guest";
    pass.auxiliaryFields.push({
      key: "attendee",
      value: `${attendeeName} ⚫`, // Black circle default, no label
    });
