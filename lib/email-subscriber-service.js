...
import { maskEmail } from "./volunteer-helpers.js";
...
class EmailSubscriberService {
  ...
  async _performInitialization() {
    try {
      ...
      await this.db.execute("SELECT 1"); // Simple connectivity test
  ...
  async createSubscriber(subscriberData) {
    ...
    try {
      ...
      // Handle cases where Brevo returns success without contact ID (204 No Content)
      if (!brevoResult || brevoResult === null) {
        console.log(`Newsletter subscription: Brevo returned null for ${maskEmail(email)}, proceeding with local database creation`);
      }
      ...
      const values = [
        ...
        status,
        brevoResult?.id ?? null,
        safeStringify(listIds),
        safeStringify(attributes),
        new Date().toISOString(),
        consentSource,
        optionalField(consentIp),
        optionalField(verificationToken),
      ];
      ...
      const subscriber = {
        ...
        email,
        first_name: optionalField(firstName),
        last_name: optionalField(lastName),
        ...
        consent_ip: optionalField(consentIp),
        verification_token: optionalField(verificationToken),
  ...
  async updateSubscriber(email, updateData) {
    ...
    if (listIds !== undefined) {
      ...
      values.push(safeStringify(listIds));
    }
    if (attributes !== undefined) {
      updates.push(`attributes = ?${paramCount++}`);
      values.push(safeStringify(attributes));
    }
