...
class ValidationService {
  ...
  validateRegistrationSearchParams(params = {}) {
    ...
    // Validate checkedInToday boolean (filter for today's check-ins)
    const checkedInTodayValidation = this.validateEnum(
      params.checkedInToday,
      "checkedInToday",
      "booleanStrings",
    );
    if (!checkedInTodayValidation.isValid) {
      errors.push(checkedInTodayValidation.error);
    } else if (params.checkedInToday === 'true') {
      sanitized.checkedInToday = 'true';
    }

    // Validate walletAccess boolean (filter for wallet-based check-ins)
    const walletAccessValidation = this.validateEnum(
      params.walletAccess,
      "walletAccess",
      "booleanStrings",
    );
    if (!walletAccessValidation.isValid) {
      errors.push(walletAccessValidation.error);
    } else if (params.walletAccess === 'true') {
      sanitized.walletAccess = 'true';
    }
