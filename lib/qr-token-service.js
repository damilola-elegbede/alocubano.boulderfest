...
export class QRTokenService {
  constructor() {
    ...
    // Parse and validate numeric environment variables with NaN protection
    const parsedExpiryDays = Number.parseInt(process.env.QR_CODE_EXPIRY_DAYS ?? "", 10);
    this.expiryDays = Number.isFinite(parsedExpiryDays) && parsedExpiryDays > 0
      ? parsedExpiryDays
      : 90;
  ...
  generateToken(payload) {
    ...
    const tokenPayload = {
      ...
      exp: payload.exp || Math.floor(Date.now() / 1000) + this.expiryDays * 24 * 60 * 60,
    };
  ...
  async getOrCreateToken(ticketId) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // Generate new token using generateToken method
        ...
        await db.execute({
          sql: `UPDATE tickets
                ...
                WHERE ticket_id = ?`,
          args: [token, safeMaxScanCount, ticketId],
        });
