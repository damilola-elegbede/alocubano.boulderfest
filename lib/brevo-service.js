...
class BrevoService {
  constructor() {
    ...
    this.templates = {
      welcome: this.getRequiredNumericEnv('BREVO_WELCOME_TEMPLATE_ID'),
      verification: this.getRequiredNumericEnv('BREVO_VERIFICATION_TEMPLATE_ID'),
      unsubscribe: this.getRequiredNumericEnv('BREVO_UNSUBSCRIBE_TEMPLATE_ID'),
    };
   ...
   */
  getRequiredNumericEnv(envVar) {
    const value = process.env[envVar];

    if (!value) {
      if (this.isTestMode) {
        console.warn(`⚠️ ${envVar} not configured in test mode`);
        return 1; // Default test value
      } else {
  ...
  async makeRequest(endpoint, options = {}) {
    ...
    try {
      ...
      // Check status BEFORE parsing JSON to avoid "Unexpected end of JSON input" errors
      if (!response.ok) {
