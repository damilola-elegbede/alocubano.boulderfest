...
class BrevoService {
  constructor() {
    ...
    this.isTestMode = process.env.NODE_ENV === 'test' || process.env.INTEGRATION_TEST_MODE === 'true';
   ...
   */
  getRequiredNumericEnv(envVar, fallback) {
    const value = process.env[envVar];

    if (!value) {
      if (this.isTestMode) {
        console.warn(`⚠️ ${envVar} not configured - using fallback value ${fallback} in test mode`);
        return fallback;
      } else {
    ...
    if (isNaN(numericValue) || numericValue <= 0) {
      if (this.isTestMode) {
        console.warn(`⚠️ ${envVar} has invalid value "${value}" - using fallback value ${fallback} in test mode`);
        return fallback;
      } else {
