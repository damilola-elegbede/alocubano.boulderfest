...
class BrevoService {
  ...
  async subscribeToNewsletter(subscriberData) {
    ...
    try {
      ...
      if (result === null) {
        if (process.env.NODE_ENV !== 'production') {
          console.log(`Brevo returned 204 No Content for newsletter subscription: ${maskEmail(email)}`);
        }
        const safeResult = {
          email: email,
      ...
      if (error.message.includes("duplicate_parameter")) {
        if (process.env.NODE_ENV !== 'production') {
          console.log(`Contact ${maskEmail(email)} already exists, updating instead`);
        }
        ...
        if (updateResult === null) {
          if (process.env.NODE_ENV !== 'production') {
            console.log(`Brevo returned 204 No Content for update: ${maskEmail(email)}`);
          }
          if (this.isTestMode) {
            return {
              success: true,
  ...
  async addTicketHolder(ticketHolderData) {
    ...
    try {
      ...
      if (result === null) {
        if (process.env.NODE_ENV !== 'production') {
          console.log(`Brevo returned 204 No Content for ticket holder: ${maskEmail(email)}`);
        }
        const safeResult = {
          email: email,
      ...
      if (error.message.includes("duplicate_parameter")) {
        if (process.env.NODE_ENV !== 'production') {
          console.log(`Ticket holder ${maskEmail(email)} already exists, updating instead`);
        }
        const updateResult = await this.updateContact(email, {
          attributes: contactData.attributes,
          listIds: contactData.listIds,
