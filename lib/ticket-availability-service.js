...
export async function reserveTickets(cartItems, sessionId) {
  ...
  // Also filter out test tickets (they're not in database)
  const ticketItems = cartItems.filter(item => {
    if (item.type !== 'ticket') return false;
    // Skip test tickets - they don't need reservations
    const isTestTicket = item.ticketType && item.ticketType.toLowerCase().includes('test');
    return !isTestTicket;
  });
