...
export class GoogleWalletService {
  ...
  async createOrUpdateClass() {
    ...
    const classDefinition = {
      ...
      logo: {
        sourceUri: {
          uri: `${this.baseUrl}/images/logo-circle.png`,
        },
        contentDescription: {
          defaultValue: {
            language: "en",
  ...
  async generatePass(ticketId) {
    ...
    try {
      ...
      try {
        ...
        console.error('[Google Wallet] Failed to get ticket color:', error);
        ticketColor = { name: 'Default', rgb: 'rgb(255, 255, 255)', emoji: '⬤' };
      }

      // Ensure we have valid color data
      const resolvedColor = ticketColor || { name: 'Default', rgb: 'rgb(255, 255, 255)', emoji: '⬤' };
      ...
      const passObject = {
        ...
        barcode: {
          ...
          alternateText: isTest ? `${ticket.ticket_id} (TEST)` : ticket.ticket_id,
        },
        // Ticket type with test mode indicator
        ticketType: {
          defaultValue: {
            language: "en",
            value: isTest ?
              `${this.formatTicketType(ticket.ticket_type).toUpperCase()} (TEST)` :
              this.formatTicketType(ticket.ticket_type).toUpperCase(),
          },
        },
        groupingInfo: {
        ...
        textModulesData: [
          {
            header: "EVENT",
            body: isTest ? "Boulder Fest 2026 (TEST)" : "Boulder Fest 2026",
            id: "event",
          },
          {
            header: "TICKET",
            body: isTest ?
              `${resolvedColor.emoji} ${this.formatTicketType(ticket.ticket_type).toUpperCase()} (TEST)` :
              `${resolvedColor.emoji} ${this.formatTicketType(ticket.ticket_type).toUpperCase()}`,
            id: "ticket-type",
          },
          {
            header: "ATTENDEE",
        ...
        imageModulesData: [
          {
            mainImage: {
              sourceUri: {
                uri: `${this.baseUrl}/api/wallet/circle?rgb=${encodeURIComponent(resolvedColor.rgb)}&size=90`,
              },
              contentDescription: {
                defaultValue: {
                  language: "en",
                  value: `${resolvedColor.name} ticket indicator`,
                },
              },
            },
