...
export class GoogleWalletService {
  ...
  isTestTicket(ticket) {
    ...
    if (!ticket.ticket_id) {
      console.error('[GoogleWalletService] Missing ticket_id field, cannot detect test ticket reliably', { ticket });
    }
  ...
  async getQRToken(ticketId) {
    ...
    return await qrTokenService.getOrCreateToken(String(ticketId));
  }
   ...
   */
  async createOrUpdateClass(ticket) {
    await this.initClient();

    // Use event-specific class ID
    const eventClassId = `event_${ticket.event_id}`;

    const classDefinition = {
      id: `${this.issuerId}.${eventClassId}`,
      ...
      wideLogo: {
        sourceUri: {
          uri: `${this.baseUrl}/wallet/wallet-logo-wide.png`,
        },
        contentDescription: {
          defaultValue: {
            language: "en",
      ...
      classTemplateInfo: {
        cardTemplateOverride: {
          cardRowTemplateInfos: [
            ...
            // Row 2: Date only (venue shows in header)
            {
              oneItem: {
                item: {
                  firstValue: {
    ...
    try {
      ...
      const response = await this.client.request({
        url: `${this.walletApiUrl}/eventTicketClass/${this.issuerId}.${eventClassId}`,
        method: "GET",
      });
      ...
      console.log(`Google Wallet class updated: ${eventClassId}`);
    } catch (error) {
      if (error.response?.status === 404) {
        ...
        console.log(`Google Wallet class created: ${eventClassId}`);
      } else {
        throw error;
      }
    }
  ...
  async generatePass(ticketId) {
    ...
    try {
      ...
      // Get ticket details with validation FIRST (needed for class creation)
      ...
      // Get ticket color for visual indicators with fallback
      const colorService = getTicketColorService();
      let ticketColor;
      try {
        ticketColor = await colorService.getColorForTicketType(ticket.ticket_type);
      } catch (error) {
        console.error("[GoogleWallet] Failed to resolve ticket color:", error);
        ticketColor = { name: "Default", rgb: "rgb(255, 255, 255)", emoji: "⬤" };
      }

      // Create or update event-specific class with ticket data
      const eventClassId = await this.createOrUpdateClass(ticket);
      ...
      const passObject = {
        id: objectId,
        classId: `${this.issuerId}.${eventClassId}`,
        state: "ACTIVE",
        ticketHolderName:
   ...
   * Uses UTC parsing to ensure consistent dates across all timezones
   */
  formatEventDate(startDate, endDate) {
    ...
    // Parse dates as UTC to avoid timezone-dependent behavior
    // '2026-05-30' should always be May 30, regardless of user's timezone
    const start = new Date(startDate + 'T00:00:00Z');
    const end = new Date(endDate + 'T00:00:00Z');
    const monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];

    // Check if same day (year, month, and date all match)
    if (start.getUTCFullYear() === end.getUTCFullYear() &&
        start.getUTCMonth() === end.getUTCMonth() &&
        start.getUTCDate() === end.getUTCDate()) {
      // Same day: display single date
      return `${monthNames[start.getUTCMonth()]} ${start.getUTCDate()}, ${start.getUTCFullYear()}`;
    }
    ...
    return `${monthNames[start.getUTCMonth()]} ${start.getUTCDate()} - ${monthNames[end.getUTCMonth()]} ${end.getUTCDate()}, ${end.getUTCFullYear()}`;
  }
   ...
   *
   * CRITICAL LIMITATION - Daylight Saving Time (DST) Awareness:
   * ============================================================
   * Mountain Time has TWO offsets depending on the season:
   *   - MST (Mountain Standard Time):   UTC-7 (November - March)
   *   - MDT (Mountain Daylight Time):   UTC-6 (March - November)
   *
   * CURRENT BEHAVIOR:
   *   - This method uses a FIXED offset of -07:00 (MST) year-round
   *   - Does NOT account for DST transitions
   *   - May create a 1-hour time discrepancy for events during MDT periods
   *
   * IMPACT:
   *   - Events scheduled during MDT (March - November) will display times
   *     that are 1 hour OFF from the actual event time
   *   - Example: Event at 2:00 PM MDT may show as 3:00 PM or 1:00 PM
   *   - Validation intervals may be incorrectly calculated
   *
   * RECOMMENDED SOLUTIONS:
   *   1. Implement DST-aware timezone handling using:
   *      - date-fns-tz: https://www.npmjs.com/package/date-fns-tz
   *      - Luxon: https://moment.github.io/luxon/
   *   2. Calculate dynamic offset based on event date
   *   3. Use IANA timezone database for accurate America/Denver offset
   *
   * WHY THIS LIMITATION EXISTS:
   *   - Simplicity: Avoiding external dependencies for timezone calculations
   *   - Google Wallet accepts any valid timezone offset
   *   - Trade-off between accuracy and code complexity
   *
   * WHEN TO FIX:
   *   - Before deploying for production events during MDT period
   *   - When time accuracy is critical for event validation
   *   - If user reports indicate confusion about event times
   *
