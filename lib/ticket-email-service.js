...
export class TicketEmailService {
  ...
  async sendTicketConfirmation(transaction) {
    try {
      ...
      // Get tickets from database by transaction ID
      const tickets = await this.getTicketsByTransactionId(transaction.id);
  ...
  async getTicketsByTransactionId(transactionId) {
    ...
    try {
      const db = await import('./database.js').then(m => m.getDatabaseClient());
              ...
              <div class="ticket">
                ...
                <div class="wallet-buttons">
                  ...
                  <div class="wallet-button">
                    <a href="${this.baseUrl}/api/tickets/apple-wallet/${encodeURIComponent(ticket.ticketId)}">
                      <img src="https://developer.apple.com/wallet/add-to-apple-wallet-badge.svg"
                           alt="Add to Apple Wallet" height="40" />
                    </a>
                  </div>
                  <div class="wallet-button">
                    <a href="${this.baseUrl}/api/tickets/google-wallet/${encodeURIComponent(ticket.ticketId)}">
