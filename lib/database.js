...
async function createSQLiteWrapper() {
  try {
    ...
    return function createClient(config) {
      // For in-memory database, use :memory: directly
      const url = config.url.replace('file::memory:?cache=shared', ':memory:');
...
class DatabaseService {
  constructor() {
    ...
    this.connectionMaxAge = 5 * 60 * 1000; // 5 minutes max connection age for Vercel
  ...
  _createTimeoutPromise() {
    // Increase timeout for Vercel serverless environments (cold starts can be slow)
    const isVercel = process.env.VERCEL === "1";
    const defaultTimeout = isVercel ? 30000 : 15000; // 30s for Vercel, 15s for others
  ...
  async _performInitialization() {
    ...
    // Vercel serverless-specific connection settings
    if (isVercel || isVercelProduction || isVercelPreview) {
      ...
      config.fetch = globalThis.fetch; // Use global fetch for edge runtime compatibility

      // Connection pool settings for Vercel
      if (!config.url.startsWith("file:") && config.url !== ":memory:") {
        // These settings apply only to remote databases (Turso)
        config.syncUrl = config.url; // Enable sync for better reliability
        config.syncInterval = 60; // Sync every 60 seconds

        logger.log("✅ Configured Vercel serverless connection settings (intMode: number)");
      }
    }
 ...
 *
 * CRITICAL: In test isolation mode, this returns the worker database
 * to ensure proper test isolation and prevent database conflicts.
 */
export async function getDatabaseClient() {
  ...
  // Standard database client initialization for non-test environments
