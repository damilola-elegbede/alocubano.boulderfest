...
class DatabaseService {
  ...
  async _performInitialization() {
    ...
    if (isIntegrationTest || (isTest && !isVercel)) {
      ...
      // Local development: Use Turso if configured, otherwise fall back to local SQLite
      databaseUrl = cleanDatabaseUrl(process.env.TURSO_DATABASE_URL || process.env.DATABASE_URL);

      if (!databaseUrl || databaseUrl.trim() === "") {
        // Fall back to local SQLite for development
        databaseUrl = "./local.db";
        logger.log(`⚠️  No TURSO_DATABASE_URL configured - using local SQLite: ${databaseUrl}`);
      } else {
...
async function createDirectDatabaseClient() {
  ...
  try {
    ...
    if (process.env.VERCEL === "1") {
      const maskedUrl = databaseUrl.substring(0, 30) + "...";
