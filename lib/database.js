...
class DatabaseService {
  ...
  async _validateConnection() {
    ...
    try {
      ...
      // Use timeout to prevent hanging on slow/unresponsive connections
      const VALIDATION_TIMEOUT_MS = 5000;
      const validationPromise = this.client.execute("SELECT 1");
      const timeoutPromise = new Promise((_, reject) =>
        setTimeout(() => reject(new Error('Connection validation timeout')), VALIDATION_TIMEOUT_MS)
      );

      await Promise.race([validationPromise, timeoutPromise]);
      return true;
    } catch (error) {
      // Connection is likely closed, timed out, or experiencing transient errors
      if (error.message?.includes('CLIENT_CLOSED') ||
          error.message?.includes('closed') ||
          error.code === 'CLIENT_CLOSED' ||
          error.message?.includes('validation timeout') ||
          // Treat 502/503 errors as connection invalid (force reconnection)
          error.message?.includes('502') ||
          error.message?.includes('503') ||
          error.message?.includes('SERVER_ERROR') ||
          error.code === 'SERVER_ERROR') {
        logger.warn("Connection validation failed, will reconnect:", error.message);
        return false;
      }
  ...
  _createTimeoutPromise() {
    ...
    // Optimized timeouts to allow multiple retries within Vercel's 60s function timeout
    // With 3 retries and 8s timeout each: 8s + 8s + 8s = 24s max for connection
    // This leaves ~35s for actual handler logic
    let defaultTimeout;
    if (isTest) {
      defaultTimeout = 5000; // 5s for tests
    } else if (isVercel) {
      defaultTimeout = 8000; // Reduced from 15s to 8s to allow more retries
    } else {
  ...
  async _performInitialization() {
    ...
    if (isVercel || isVercelProduction || isVercelPreview || databaseUrl.startsWith("libsql://")) {
      ...
      if (!config.url.startsWith("file:") && config.url !== ":memory:") {
        ...
        // Connection timeout optimizations - reduced to allow more retries within 60s Vercel limit
        // With 3 retries: 8s + 8s + 8s = 24s max, leaving ~35s for handler logic
        config.connectTimeout = 8000; // 8s connection timeout (reduced from 15s)
        config.requestTimeout = 12000; // 12s request timeout (reduced from 20s)
    ...
    // Retry logic for connection timeouts (ETIMEDOUT errors)
    const maxRetries = 3;
    let lastError = null;
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt > 1) {
          logger.log(
            `✅ Database client initialized successfully on retry ${attempt}/${maxRetries} (${process.env.NODE_ENV || "production"} mode)`,
          );
        } else {
          logger.log(
        ...
        // Check if this is a retryable connection error (timeout, network, or transient server errors)
        const isRetryableError = error.message?.includes('ETIMEDOUT') ||
                               error.message?.includes('connect timeout') ||
                               error.message?.includes('fetch failed') ||
                               error.code === 'ETIMEDOUT' ||
                               error.code === 'ECONNREFUSED' ||
                               // Turso/libsql 502 Bad Gateway errors (transient server issues)
                               error.message?.includes('502') ||
                               ...
                               // Connection reset/abort errors
                               error.code === 'ECONNRESET' ||
                               error.code === 'ECONNABORTED';

        // Retry on transient errors if we haven't exhausted retries
        if (isRetryableError && attempt < maxRetries) {
          const backoffDelay = Math.min(1000 * Math.pow(2, attempt - 1), 5000); // Exponential backoff: 1s, 2s, 4s (max 5s)
          logger.warn(
            `⚠️  Database connection attempt ${attempt}/${maxRetries} failed with transient error, retrying in ${backoffDelay}ms...`,
        ...
        // For non-retryable errors or exhausted retries, break and handle below
        break;
      }
    }
    ...
    if (lastError) {
      ...
      logger.error("❌ Database initialization failed after retries:", errorContext);
      ...
      if (
        ...
        throw new Error(
          `Database connection timeout after ${maxRetries} attempts: ${error.message}`,
        );
      } else {
        throw new Error(
    ...
    try {
      ...
      const fallbackTransaction = {
        execute: async (sql, params = []) => {
          ...
          try {
            // Normalize: convert string+array format to object format for LibSQL compatibility
            // LibSQL requires { sql, args } object format, handle both input styles
            const query = typeof sql === 'string'
              ? { sql, args: params }
              : sql;  // Already an object
  ...
  _wrapTransactionWithTimeout(transaction, timeoutMs) {
    ...
    return {
      execute: async (sql, params = []) => {
        ...
        try {
          // Normalize: convert string+array format to object format for LibSQL compatibility
          // LibSQL native transactions require { sql, args } object format
          const query = typeof sql === 'string'
            ? { sql, args: params }
            : sql;  // Already an object
