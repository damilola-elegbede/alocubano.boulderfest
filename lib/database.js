  ...
  async _performInitialization() {
    ...
    if (isVercel || isVercelProduction || isVercelPreview || databaseUrl.startsWith("libsql://")) {
      ...
      if (!config.url.startsWith("file:") && config.url !== ":memory:") {
        ...
        // Connection timeout optimizations (increased for Turso cold starts and network latency)
        config.connectTimeout = 15000; // 15s connection timeout (handles cold starts and network delays)
        config.requestTimeout = 20000; // 20s request timeout for complex queries and slower connections
        ...
        logger.log("✅ Configured optimized Turso connection settings (intMode: number, timeouts: 15s/20s)");
      }
    }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // Check if this is a retryable connection timeout error
        ...
        // Only retry on timeout errors and if we haven't exhausted retries
        if (isTimeoutError && attempt < maxRetries) {
          const backoffDelay = Math.min(1000 * Math.pow(2, attempt - 1), 5000); // Exponential backoff: 1s, 2s, 4s (max 5s)
          logger.warn(
            `⚠️  Database connection attempt ${attempt}/${maxRetries} failed with timeout, retrying in ${backoffDelay}ms...`,
