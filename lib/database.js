...
async function importLibSQLClient() {
  // Check if this is a unit test with in-memory SQLite
  const isUnitTest = process.env.UNIT_ONLY_MODE === 'true' ||
                     (process.env.NODE_ENV === 'test' &&
                      process.env.DATABASE_URL?.includes('memory'));

  if (isUnitTest) {
    // Use better-sqlite3 wrapper for unit tests (no native binary issues)
    logger.log('Using SQLite wrapper for unit tests');
