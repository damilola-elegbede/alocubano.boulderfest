...
class DatabaseService {
  ...
  async _performInitialization() {
    ...
    // CRITICAL: For integration tests with in-memory databases, use the test isolation manager
    if (isIntegrationTest && process.env.DATABASE_URL === ':memory:') {
      try {
        // Get the shared worker database from the test isolation manager
        ...
        const client = await isolationManager.getScopedDatabaseClient();
        ...
        if (client) {
          ...
          logger.log(`✅ Using worker database from test isolation manager for integration tests`);
          return this.client;
        }
      } catch (error) {
        logger.warn(`⚠️ Could not get test database from isolation manager: ${error.message}`);
