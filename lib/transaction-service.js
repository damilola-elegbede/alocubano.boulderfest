...
import { safeStringify, processDatabaseResult } from "./bigint-serializer.js";
import { optionalField, coalesce } from "./value-utils.js";
...
export class TransactionService {
  ...
  async createFromStripeSession(session, paymentMethodData = null, req = null, inTransaction = false) {
    try {
      ...
      // Use transaction UUID directly (no TEST- prefix)
      const uuid = baseUuid;
      ...
      // 1. Insert transaction with test mode flag, order number, and payment method data
      // Note: event_id belongs on tickets (from Stripe product metadata), not transactions or transaction_items
      batchOperations.push({
        ...
        args: [
          ...
          currency,
          session.id,
          optionalField(session.payment_intent),
          session.payment_method_types?.[0] || "card",
          coalesce(session.customer_details?.email, session.customer_email),
          optionalField(session.customer_details?.name),
          billingAddress,
          "completed",
          new Date().toISOString(),
          isTest, // Add test mode flag
          orderNumber, // Add order number
          optionalField(paymentMethodData?.card_brand),
          optionalField(paymentMethodData?.card_last4),
          optionalField(paymentMethodData?.payment_wallet),
      ...
      for (const item of lineItems) {
        ...
        try {
          ...
          metadata = safeStringify(testModeMetadata);
        } catch (e) {
          metadata = safeStringify({ test_mode: isTest === 1 });
        }
        ...
        batchOperations.push({
          sql: `INSERT INTO transaction_items (
            transaction_id, item_type, item_name, item_description,
            quantity, unit_price_cents, total_price_cents,
            ticket_type, product_metadata, is_test
          ) VALUES ((SELECT id FROM transactions WHERE uuid = ?), ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          args: [
            uuid, // Reference the transaction UUID
            itemType,
            item.description || "Unknown Item",
            optionalField(item.price?.product?.description),
            quantity,
  ...
  async getByUUID(uuid) {
    ...
    // BIGINT FIX: Sanitize database result before returning
    return result.rows[0] ? processDatabaseResult(result.rows[0]) : null;
  }
  ...
  async getByStripeSessionId(sessionId) {
    ...
    // BIGINT FIX: Sanitize database result before mapping to prevent BigInt serialization errors
    // This transaction object is passed to email service where JSON operations occur
    const sanitizedRow = processDatabaseResult(result.rows[0]);

    // Return sanitized object with named properties
    // Note: processDatabaseResult returns an object with the same property names
    return {
      id: sanitizedRow.id,
      transaction_id: sanitizedRow.transaction_id,
      uuid: sanitizedRow.uuid,
      type: sanitizedRow.type,
      order_data: sanitizedRow.order_data,
      amount_cents: sanitizedRow.amount_cents,
      total_amount: sanitizedRow.amount_cents,  // Add alias for email service compatibility
      currency: sanitizedRow.currency,
      stripe_session_id: sanitizedRow.stripe_session_id,
      stripe_payment_intent_id: sanitizedRow.stripe_payment_intent_id,
      paypal_order_id: sanitizedRow.paypal_order_id,
      paypal_capture_id: sanitizedRow.paypal_capture_id,
      payment_method_type: sanitizedRow.payment_method_type,
      customer_email: sanitizedRow.customer_email,
      customer_name: sanitizedRow.customer_name,
      billing_address: sanitizedRow.billing_address,
      status: sanitizedRow.status,
      completed_at: sanitizedRow.completed_at,
      created_at: sanitizedRow.created_at,
      updated_at: sanitizedRow.updated_at,
      registration_token: sanitizedRow.registration_token,
      is_test: sanitizedRow.is_test,
      order_number: sanitizedRow.order_number
    };
  }
  ...
  async updateStatus(uuid, status, metadata = {}) {
    ...
    let eventData;
    try {
      eventData = safeStringify({ metadata });
    } catch (e) {
      eventData = "{}";
    }
    ...
    const batchOperations = [
      ...
      {
        sql: `INSERT INTO payment_events (
          event_id, transaction_id, event_type, source, source_id,
          event_data, previous_status, new_status, processed_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        args: [
          eventId,
   ...
   * Uses indexed lookup on stripe_payment_intent_id for O(log n) performance
   * Critical for Stripe webhook processing (called 4 times per payment event)
   */
  async getByPaymentIntentId(paymentIntentId) {
    const db = await getDatabaseClient();
    const result = await db.execute({
      sql: `SELECT * FROM transactions
            INDEXED BY idx_transactions_stripe_payment_intent
  ...
  async createFromPayPalOrder(paypalOrder, req = null) {
    try {
      ...
      const paypalCaptureId = optionalField(capture?.id);
      const paypalPayerId = optionalField(paypalOrder.payer?.payer_id);
      ...
      try {
        ...
        cartData = safeStringify({ items });
      } catch (e) {
        cartData = safeStringify({ error: "Could not serialize cart data", test_mode: isTest === 1 });
      }
      ...
      batchOperations.push({
        ...
        args: [
          ...
          referenceId,
          'paypal',
          optionalField(paypalOrder.payer?.email_address),
      ...
      for (const unit of purchaseUnits) {
        ...
        for (const item of items) {
          ...
          try {
            const baseMetadata = {
              paypal_item: item,
              sku: optionalField(item.sku),
              category: optionalField(item.category)
            };
            const testModeMetadata = createTestModeMetadata(req, baseMetadata);
            metadata = safeStringify(testModeMetadata);
          } catch (e) {
          ...
          batchOperations.push({
            ...
            args: [
              ...
              optionalField(item.description),
              quantity,
              unitPriceCents, // Keep in cents
              totalPriceCents, // Keep in cents
   ...
   * @param {string} [paymentProcessor] - Optional payment processor (venmo or paypal)
   */
  async updatePayPalCapture(uuid, captureId, status, paymentProcessor = null) {
    const updatedAt = new Date().toISOString();
    ...
    let eventData;
    try {
      eventData = safeStringify({
        metadata: {
          paypal_capture_id: captureId,
          payment_processor: paymentProcessor || transaction.payment_processor
        },
        paypal_capture_id: captureId
      });
    } catch (e) {
      eventData = safeStringify({ paypal_capture_id: captureId });
    }

    // Prepare batch operations for atomic update and logging
    const eventId = `EVT-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;

    // Build UPDATE query with optional payment_processor
    const updateFields = [
      'paypal_capture_id = ?',
      'status = ?',
    ...
    const updateArgs = [captureId, status, status, updatedAt, updatedAt];

    if (paymentProcessor) {
      updateFields.push('payment_processor = ?');
      updateArgs.push(paymentProcessor);
    }

    updateArgs.push(uuid); // WHERE clause parameter
