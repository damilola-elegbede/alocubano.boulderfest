...
export class TransactionService {
   ...
   * Note: DatabaseService handles initialization internally via execute()
   * but this method provides defensive programming and consistent API
   */
  async ensureInitialized() {
    // Ensure we have a database service instance
    if (!this.db) {
      this.db = getDatabase();
    }
    // DatabaseService.execute() handles initialization internally,
    // but we can explicitly ensure it's ready
  ...
  async createFromStripeSession(session, req = null, inTransaction = false) {
    ...
    try {
      ...
      // Rollback on error only if we're managing the transaction
      if (shouldManageTransaction) {
        try {
  ...
  /**
   * Create a new transaction from PayPal order
   */
  async createFromPayPalOrder(paypalOrder, req = null) {
    ...
    try {
      ...
      // Insert transaction with PayPal data
      const result = await this.db.execute({
        sql: `INSERT INTO transactions (
      ...
      // Create transaction items with test mode support
      await this.createPayPalTransactionItems(transaction.id, paypalOrder, isTest, req);
  ...
  /**
   * Create transaction items from PayPal order items
   */
    ...
    await this.db.execute({
      sql: `UPDATE transactions
            SET paypal_capture_id = ?, status = ?,
                ...
                updated_at = ?
            WHERE uuid = ?`,
      args: [captureId, status, updatedAt, updatedAt, uuid],
    });

    // Log the status change
    await this.logStatusChange(uuid, status, { paypal_capture_id: captureId });
  }
}
