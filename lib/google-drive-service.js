...
class GoogleDriveService {
   ...
   * Implements pagination to handle folders with >1000 images
   * @private
   */
  async _performApiFetch(options) {
    ...
    // Step 3: Process each category folder with pagination support
    for (const folder of categoryFolders.data.files) {
      ...
      // Get images from this category folder with pagination
      ...
      do {
        ...
        categoryFiles.push(...files);
        
        pageToken = filesResponse.data.nextPageToken;
        
        if (pageToken) {
          logger.log(`  📄 Found ${files.length} items (fetching more...)`);
        }
      } while (pageToken);

      // Process files to create gallery items
      const galleryItems = categoryFiles.map(file => this._processFile(file, categoryName));

      categories[categoryName].push(...galleryItems.filter(item => item !== null));
      totalFiles += categories[categoryName].length;

      logger.log(`  ✅ Found ${categories[categoryName].length} items total in ${categoryName}`);
    }
  ...
  _processStructuredApiResponse(categories, options, totalFiles) {
    ...
    const displayYear = year ? Number(year) : currentYear; // Ensure year is always a number
    ...
    return {
      ...
      categories,
      hasMore: false, // Pagination now handles all files
