...
import { optionalField, requiredField } from './value-utils.js';
      ...
      try {
        ...
        const auditEntry = {
          ...
          args: [
            ...
            'data_change',
            action || 'UNKNOWN',
            optionalField(targetType),
            optionalField(targetId),
            requiredField(adminUser, 'adminUser'),
            optionalField(sessionId),
            optionalField(ipAddress),
            optionalField(userAgent),
            sanitizedBefore ? safeStringify(sanitizedBefore) : null,
            sanitizedAfter ? safeStringify(sanitizedAfter) : null,
            changedFields ? safeStringify(changedFields) : null,
            sanitizedMetadata ? safeStringify(sanitizedMetadata) : null,
            severity || 'info',
      ...
      try {
        ...
        const auditEntry = {
          ...
          args: [
            ...
            optionalField(ipAddress),
            optionalField(userAgent),
            optionalField(requestMethod),
            optionalField(requestUrl),
            sanitizedBody ? safeStringify(sanitizedBody) : null,
            validResponseStatus,
            responseTimeMs || 0,
            sanitizedMetadata ? safeStringify(sanitizedMetadata) : null,
            optionalField(error),
      ...
      try {
        ...
        const auditEntry = {
          ...
          args: [
            ...
            'data_processing',
            action || 'UNKNOWN',
            optionalField(dataSubjectId),
            optionalField(dataType),
            optionalField(processingPurpose),
            optionalField(legalBasis),
            optionalField(retentionPeriod),
            requiredField(adminUser, 'adminUser'),
            sanitizedMetadata ? safeStringify(sanitizedMetadata) : null,
            severity || 'info',
            new Date().toISOString()
    ...
    return this._executeWithRetry(async () => {
      ...
      const auditEntry = {
        ...
        args: [
          ...
          amountCents || 0,
          currency || 'USD',
          optionalField(transactionReference),
          optionalField(paymentStatus),
          requiredField(adminUser, 'adminUser'),
          sanitizedMetadata ? safeStringify(sanitizedMetadata) : null,
          severity || 'info',
          timestamp
  ...
  async queryAuditLogs(params = {}) {
    ...
    return this._executeWithRetry(async () => {
      ...
      try {
        ...
        // Add ordering and pagination (sanitize orderBy/orderDirection)
        const allowedOrderBy = new Set([
          'created_at', 'event_type', 'action', 'admin_user',
          'target_type', 'target_id', 'severity', 'response_status'
        ]);
        const safeOrderBy = allowedOrderBy.has(orderBy) ? orderBy : 'created_at';
        const safeDirection = orderDirection === 'ASC' ? 'ASC' : 'DESC';
        query += ` ORDER BY ${safeOrderBy} ${safeDirection} LIMIT ? OFFSET ?`;
