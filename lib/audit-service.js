...
export class AuditService {
  ...
  async _performInitialization() {
    try {
      console.log('[AUDIT_INIT] Starting audit service initialization...');
      ...
      if (process.env.INTEGRATION_TEST_MODE === 'true') {
        console.log('[AUDIT_INIT] Integration test mode detected');
        try {
          console.log('[AUDIT_INIT] About to import test-isolation-manager...');
          const { getTestIsolationManager } = await import('./test-isolation-manager.js');
          console.log('[AUDIT_INIT] test-isolation-manager imported');

          console.log('[AUDIT_INIT] Getting isolation manager instance...');
          const isolationManager = getTestIsolationManager();
          console.log('[AUDIT_INIT] Isolation manager obtained');

          console.log('[AUDIT_INIT] Calling getScopedDatabaseClient()...');
          freshDb = await isolationManager.getScopedDatabaseClient();
          console.log('[AUDIT_INIT] Got scoped database client');
          logger.debug('[Audit] Using test isolation database for audit service');
        } catch (error) {
          console.log('[AUDIT_INIT] Test database error, falling back:', error.message);
          logger.warn('[Audit] Failed to get test database, falling back to standard database:', error.message);
          console.log('[AUDIT_INIT] Calling getDatabaseClient() as fallback...');
          freshDb = await getDatabaseClient();
          console.log('[AUDIT_INIT] getDatabaseClient() fallback complete');
        }
      } else {
        console.log('[AUDIT_INIT] Standard mode, calling getDatabaseClient()...');
        // Always get a fresh database client - don't cache it
        // This ensures we get the correct database for the current environment
        freshDb = await getDatabaseClient();
        console.log('[AUDIT_INIT] getDatabaseClient() complete');
      }
