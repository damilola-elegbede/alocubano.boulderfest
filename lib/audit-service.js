...
export class AuditService {
  ...
  /**
   * Ensure audit table exists with specific database connection
   */
  async _ensureAuditTableWithConnection(db) {
    try {
      await db.execute(`
      ...
      if (!error.message.includes('already exists')) {
        logger.debug('[Audit] Table creation note:', error.message);
      }
      return true;
    }
  }
  ...
  async logDataChange(params) {
    ...
    try {
      // Ensure table exists with fresh connection
      await this._ensureAuditTableWithConnection(db);
    ...
    const {
      ...
      if (db && db.close) {
        try {
          ...
          logger.debug('[Audit] Error closing database connection:', closeError.message);
        }
      }
    }
  }
