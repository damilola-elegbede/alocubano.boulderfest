...
export function isTestMode(req = null) {
  ...
  // NOTE: Preview deployments should NOT be treated as test mode
  // We want real emails to send in preview for testing
  const envTestMode =
...
/**
 * Generate test-aware ticket name with appropriate prefix
 *
 * @param {string} baseName - Base ticket name
 * @param {Object} req - Express request object (optional)
 * @returns {string} Ticket name with TEST- prefix if in test mode
 */
export function generateTestAwareTicketName(baseName, req = null) {
  if (isTestMode(req)) {
...
/**
 * Generate test-aware transaction ID with appropriate prefix
 *
 * @param {string} baseId - Base transaction ID
 * @param {Object} req - Express request object (optional)
 * @returns {string} Transaction ID with TEST- prefix if in test mode
 */
export function generateTestAwareTransactionId(baseId, req = null) {
  if (isTestMode(req)) {
    return `TEST-${baseId}`;
  }
  return baseId;
...
/**
 * Create WHERE clause filter for test mode queries with strict data isolation
 *
 * @param {string} tableAlias - Table alias for SQL query (e.g., 't', 'tx')
 * @param {boolean} includeTestData - Whether to include test data (default: auto-detect)
 * @param {Object} req - Express request object (optional)
 * @returns {Object} Object with sql and args for WHERE clause
 */
export function createTestModeFilter(tableAlias = '', includeTestData = null, req = null) {
  const prefix = tableAlias ? `${tableAlias}.` : '';
  const currentTestMode = isTestMode(req);

  // Auto-detect if not specified
  if (includeTestData === null) {
    includeTestData = currentTestMode;
  }

  // SECURITY: Strict data isolation - test environments can only access test data
  if (currentTestMode) {
    // In test mode, ONLY return test data (strict isolation)
    return {
      sql: ` AND ${prefix}is_test = ?`,
      args: [1]
    };
  } else {
...
/**
 * Validate data access permissions based on test mode context
 * Prevents test environments from accessing production data
 *
 * @param {Object} dataRecord - Database record to validate
 * @param {Object} req - Express request object (optional)
 * @throws {Error} If access is denied due to test mode violations
 */
export function validateDataAccess(dataRecord, req = null) {
  const currentTestMode = isTestMode(req);
  const recordIsTest = dataRecord?.is_test === 1 || dataRecord?.is_test === true;

  // SECURITY: Prevent test environments from accessing production data
  if (currentTestMode && !recordIsTest) {
    throw new Error('Access denied: Test environments cannot access production data');
  }

  // SECURITY: Prevent production environments from accidentally accessing test data
  if (!currentTestMode && recordIsTest) {
    throw new Error('Access denied: Production environments cannot access test data');
  }
}
 ...
 * @param {Object} req - Express request object (optional)
 */
export function logTestModeOperation(operation, details = {}, req = null) {
  if (isTestMode(req)) {
    ...
    console.log(`[TEST MODE] ${operation}:`, {
      ...sanitizedDetails,
      timestamp: new Date().toISOString(),
      test_context: createTestModeMetadata(req).test_context
    });
  }
}
