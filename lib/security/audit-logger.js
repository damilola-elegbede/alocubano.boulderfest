...
class AuditLogger {
  ...
  static async log({
    ...
    try {
      ...
      // Asynchronously write log (don't block main thread)
      this._writeLogAsync(logLine);
    ...
    try {
      ...
      const sortedFiles = (await Promise.all(
        logFiles.map(async (file) => {
          const stats = await fs.stat(path.join(CONFIG.LOG_DIR, file));
          return { file, stats };
        })
      ...
      while (sortedFiles.length > CONFIG.MAX_LOG_FILES) {
        ...
        await fs.unlink(path.join(CONFIG.LOG_DIR, oldestFile.file));
      }

      // Remove files older than retention period
      const retentionThreshold = Date.now() - 
        (CONFIG.RETENTION_DAYS * 24 * 60 * 60 * 1000);
      
      for (const fileInfo of sortedFiles) {
        if (fileInfo.stats.birthtimeMs < retentionThreshold) {
          await fs.unlink(path.join(CONFIG.LOG_DIR, fileInfo.file));
        }
      }
    } catch (error) {
  ...
  static async _triggerCriticalAlert(logEntry) {
    try {
      ...
      // - PagerDuty
      // - Custom webhook
      console.warn('CRITICAL SECURITY EVENT:', JSON.stringify(logEntry, null, 2));
    } catch (error) {
  ...
  static async retrieveLogs({
    ...
    try {
      ...
      for (const logFile of logFiles) {
        const filePath = path.join(CONFIG.LOG_DIR, logFile);
        ...
        const filteredEntries = fileEntries.filter(entry => {
          ...
          const matchEventType = eventTypes.length === 0 || 
            eventTypes.includes(entry.eventType);
          
          const matchSeverity = severityLevels.length === 0 || 
            severityLevels.includes(entry.severity);
          
          const matchSuccess = success === undefined || entry.success === success;
