import { getAlertManager, AlertSeverity, AlertCategory } from './alert-manager.js';
import { captureException, captureMessage, addBreadcrumb } from './sentry-config.js';
...
class BusinessMetricsTracker {
  ...
  trackPayment(amount, status, metadata = {}) {
    ...
    if (status === 'success') {
      ...
      this.metricsCollector.incrementCounter('payment.revenue', amount, metadata);
    } else {
      this.paymentMetrics.failures++;
      this.metricsCollector.incrementCounter('payment.failure', 1, metadata);
      
      // Alert on high failure rate
      const failureRate = this.paymentMetrics.failures / this.paymentMetrics.attempts;
  ...
  trackUserActivity(userId, action, metadata = {}) {
    switch (action) {
      case 'registration':
        ...
        this.metricsCollector.incrementCounter('user.registration', 1, metadata);
        break;
      case 'login':
        ...
        this.metricsCollector.setGauge('user.active', this.userMetrics.activeUsers.size);
        break;
      case 'logout':
  ...
  trackTicketOperation(operation, metadata = {}) {
    switch (operation) {
      ...
      case 'transfer':
        ...
        this.metricsCollector.incrementCounter('ticket.transferred', 1, metadata);
        break;
      case 'cancel':
  ...
  getSummary() {
    return {
      payments: {
        ...this.paymentMetrics,
        successRate: this.paymentMetrics.attempts > 0 
...
export class MonitoringService {
  ...
  startPerformanceMonitoring() {
    setInterval(async () => {
      try {
        ...
        // Check thresholds
        if (performanceData.percentiles?.p95 > MONITORING_CONFIG.thresholds.responseTime.p95) {
          await this.alertManager.processAlert({
  ...
  async checkSystemHealth() {
    ...
    if (typeof process !== 'undefined' && process.memoryUsage) {
      ...
      checks.memory = heapUsedPercent < MONITORING_CONFIG.thresholds.memoryUsage ? 'healthy' : 'unhealthy';
      
      this.metricsCollector.setGauge('system.memory.heap_used', memUsage.heapUsed);
      this.metricsCollector.setGauge('system.memory.heap_total', memUsage.heapTotal);
    ...
    const status = unhealthyCount === 0 ? 'healthy' : unhealthyCount > 2 ? 'unhealthy' : 'degraded';
  ...
  collectSystemMetrics() {
    ...
    this.metricsCollector.setGauge('business.revenue', businessSummary.payments.revenue);
    this.metricsCollector.setGauge('business.users.active', businessSummary.users.activeCount);
  }
  ...
  trackError(error, context = {}) {
    ...
    // Check error rate
    const totalErrors = Array.from(this.errorCounts.values()).reduce((a, b) => a + b, 0);
    const errorRate = totalErrors / (Date.now() / 60000); // Errors per minute
  ...
  trackApiRequest(endpoint, method, statusCode, duration, metadata = {}) {
    ...
    if (duration > MONITORING_CONFIG.thresholds.responseTime.p99) {
      captureMessage(`Slow API request: ${method} ${endpoint} took ${duration}ms`, 'warning', {
        endpoint,
        method,
        statusCode,
        duration,
  ...
  formatPrometheus(metrics) {
    ...
    // Business metrics
    lines.push(`alocubano_business_revenue_total ${metrics.business.payments.revenue}`);
    lines.push(`alocubano_business_users_active ${metrics.business.users.activeCount}`);
    lines.push(`alocubano_business_payments_total ${metrics.business.payments.attempts}`);
    lines.push(`alocubano_business_payments_success ${metrics.business.payments.successes}`);
    lines.push(`alocubano_business_payments_failure ${metrics.business.payments.failures}`);
