...
/**
 * Map alert severity to Sentry level
 */
function mapSeverityToSentryLevel(severity) {
  ...
  return levelMapping[severity] || severity;
}
...
export class AlertManager {
  ...
  async processAlert(alertData) {
    try {
      ...
      // Send to Sentry with mapped severity level
      const mappedSeverity = mapSeverityToSentryLevel(severity);
      captureMessage(alert.title, mappedSeverity, alert.details);
...
export function getAlertManager() {
  if (!alertManager) {
    alertManager = new AlertManager({
      ...
      thresholds: {
        payment_failure_rate: parseFloat(process.env.PAYMENT_FAILURE_THRESHOLD) || 0.01,
        database_response_time: parseInt(process.env.DB_RESPONSE_THRESHOLD) || 1000,
        api_response_time: parseInt(process.env.API_RESPONSE_THRESHOLD) || 2000,
        memory_usage_percent: parseInt(process.env.MEMORY_USAGE_THRESHOLD) || 80,
        error_rate: parseFloat(process.env.ERROR_RATE_THRESHOLD) || 0.05
      }
    });
  }
  return alertManager;
