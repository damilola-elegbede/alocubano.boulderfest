...
export class AlertManager {
  ...
  calculateSeverity(alertData) {
    ...
    if (
      ...
      metrics.failure_rate > this.thresholds.payment_failure_rate * 2
    ) {
  ...
  async processAlert(alertData) {
    try {
      ...
      if (this.alertChannels.webhookUrl) {
        const webhookResult = await this.sendWebhookAlert(
          alert,
          this.alertChannels.webhookUrl,
        );
    ...
    try {
      ...
      if (this.alertChannels.escalationWebhookUrl) {
        await this.sendWebhookAlert(
          escalationAlert,
          this.alertChannels.escalationWebhookUrl,
        );
      }
  ...
  getStatistics() {
    ...
    activeAlerts.forEach((alert) => {
      severityCounts[alert.severity] =
        (severityCounts[alert.severity] || 0) + 1;
...
export function getAlertManager() {
  if (!alertManager) {
    alertManager = new AlertManager({
      ...
      thresholds: {
        payment_failure_rate: Number.isFinite(
          parseFloat(process.env.PAYMENT_FAILURE_THRESHOLD),
        )
          ? parseFloat(process.env.PAYMENT_FAILURE_THRESHOLD)
          : 0.01,
        database_response_time: Number.isFinite(
          parseInt(process.env.DB_RESPONSE_THRESHOLD),
        )
          ? parseInt(process.env.DB_RESPONSE_THRESHOLD)
          : 1000,
        api_response_time: Number.isFinite(
          parseInt(process.env.API_RESPONSE_THRESHOLD),
        )
          ? parseInt(process.env.API_RESPONSE_THRESHOLD)
          : 2000,
        memory_usage_percent: Number.isFinite(
          parseInt(process.env.MEMORY_USAGE_THRESHOLD),
        )
          ? parseInt(process.env.MEMORY_USAGE_THRESHOLD)
          : 80,
        error_rate: Number.isFinite(
