...
class BaselineTracker {
  ...
  checkRegression(endpoint, currentStats) {
    ...
    if (
      ...
      baseline.avg_duration * this.deviationThreshold
    ) {
      regressions.push({
        metric: "avg_duration",
        ...
        deviation:
          (currentStats.avg_duration / baseline.avg_duration - 1) * 100,
      });
    }
...
export class PerformanceTracker {
  constructor(options = {}) {
    ...
    // Start periodic checks (skip in test environment)
  ...
  async detectPerformanceRegression(endpoint, currentMetrics) {
    ...
    const regressions = this.baselineTracker.checkRegression(
      endpoint,
      currentMetrics,
    );
  ...
  /**
   * Get performance metrics (alias for exportMetrics)
   */
  getMetrics() {
  ...
  /**
   * Record metric (for compatibility)
   */
  recordMetric(endpoint, duration, statusCode = 200, metadata = {}) {
    try {
      this.metricsStore.addSample(endpoint, duration, statusCode, metadata);
    } catch (error) {
      if (!this.isTestEnvironment()) {
  ...
  exportMetrics() {
    try {
      return {
        ...
        percentiles: this.calculateOverallPercentiles(),
      };
    } catch (error) {
      if (!this.isTestEnvironment()) {
  ...
  /**
   * Calculate overall percentiles across all endpoints
   */
  calculateOverallPercentiles() {
    try {
      const allStats = this.metricsStore.getAllStats();
      const allDurations = [];

      for (const stats of Object.values(allStats)) {
        if (stats && stats.sample_count > 0) {
          // Approximate durations from percentiles for overall calculation
          const samples = Math.min(stats.sample_count, 100);
          for (let i = 0; i < samples; i++) {
            allDurations.push(stats.avg_duration);
          }
        }
      }
