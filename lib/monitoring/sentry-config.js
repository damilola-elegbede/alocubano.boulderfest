 ...
 * Only loads @sentry/node when SENTRY_DSN is configured
 * Saves ~26MB from serverless function bundles when Sentry is not used
 */
let Sentry = null;
 ...
 * Async to support lazy-loading of Sentry module
 */
...
export const captureMessage = async (message, level = "info", context = {}) => {
  ...
  SentryModule.withScope((scope) => {
    ...
    // Capture the message
    SentryModule.captureMessage(sanitizedMessage, sentryLevel);
  });
};
 ...
 */
export const startTransaction = async (name, op = "http.server") => {
...
/**
 * Export Sentry getter for direct access if needed
 * Returns null if Sentry is not loaded
 */
