...
const SENSITIVE_PATTERNS = {
  ...
  phone: /(\+?[0-9]{1,4}[-.\s]?)?((\([0-9]{1,3}\)|[0-9]{1,3})[-.\s]?)?([0-9]{1,4}[-.\s]?[0-9]{1,4}[-.\s]?[0-9]{1,9})/g,
  creditCard: /\b(?:\d[ -]*?){13,19}\b/g,
  apiKey: /(api[_-]?key|apikey|secret|token|password|auth|bearer|jwt)[\s:="']*([a-zA-Z0-9_\-\.]+)/gi,
  stripeToken: /(tok|card|cus|sub|pi|pm|price|prod|sku|tax|src|ch|ba)_[a-zA-Z0-9]{24,}/g,
  dbConnection: /(mongodb|postgres|mysql|redis|sqlite):\/\/[^\s]+/gi,
  ipAddress: /\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b/g,
  ssn: /\b\d{3}-\d{2}-\d{4}\b/g,
  customerId: /(customer_id|customerId|user_id|userId)[\s:="']*([a-zA-Z0-9_\-]+)/gi
};

// Fields that should be completely removed from events
const BLOCKED_FIELDS = [
  'password', 'passwd', 'pwd', 'secret', 'token', 'apiKey', 'api_key',
  'authorization', 'auth', 'bearer', 'creditCard', 'credit_card', 'cc',
  'cvv', 'cvc', 'ssn', 'social_security', 'tax_id', 'bank_account',
  'routing_number', 'iban', 'swift', 'private_key', 'privateKey'
];
...
function sanitizeString(str) {
  ...
  sanitized = sanitized.replace(SENSITIVE_PATTERNS.stripeToken, '[STRIPE_TOKEN]');
  
  // Replace database connection strings
  sanitized = sanitized.replace(SENSITIVE_PATTERNS.dbConnection, '[DATABASE_URL]');
  
  // Replace IP addresses (but keep localhost)
  sanitized = sanitized.replace(SENSITIVE_PATTERNS.ipAddress, (match) => {
    return match === '127.0.0.1' || match === '0.0.0.0' ? match : '[IP_ADDRESS]';
  });
  ...
  try {
    ...
    if (sanitizedEvent.request) {
      if (sanitizedEvent.request.headers) {
        sanitizedEvent.request.headers = sanitizeObject(sanitizedEvent.request.headers);
      }
      if (sanitizedEvent.request.data) {
        sanitizedEvent.request.data = sanitizeObject(sanitizedEvent.request.data);
      }
      if (sanitizedEvent.request.query_string) {
        sanitizedEvent.request.query_string = sanitizeString(sanitizedEvent.request.query_string);
      }
      if (sanitizedEvent.request.cookies) {
        sanitizedEvent.request.cookies = sanitizeObject(sanitizedEvent.request.cookies);
      }
    }
    ...
    if (sanitizedEvent.breadcrumbs && Array.isArray(sanitizedEvent.breadcrumbs)) {
    ...
    if (sanitizedEvent.exception && sanitizedEvent.exception.values) {
      sanitizedEvent.exception.values = sanitizedEvent.exception.values.map(exception => ({
        ...exception,
