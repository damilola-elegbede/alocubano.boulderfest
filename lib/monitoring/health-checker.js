...
class CircuitBreaker {
  constructor(name, options = {}) {
    ...
    this.failureThreshold =
      options.threshold || DEFAULT_CIRCUIT_BREAKER_THRESHOLD;
  ...
  async executeAll() {
    ...
    checks.forEach((name, index) => {
      ...
      if (result.status === "fulfilled") {
        ...
        if (result.value.status === HealthStatus.UNHEALTHY) {
          ...
          result.value.status === HealthStatus.DEGRADED &&
    ...
    return {
      ...
      performance: {
        avg_response_time:
          checks.length > 0
            ? `${Math.round(totalResponseTime / checks.length)}ms`
            : "0ms",
        total_response_time: `${totalResponseTime}ms`,
        error_rate:
          checks.length > 0
            ? `${((errorCount / checks.length) * 100).toFixed(2)}%`
            : "0%",
