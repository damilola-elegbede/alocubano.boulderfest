...
class BootstrapService {
  ...
  async loadBootstrapFile() {
    const filePath = process.env.BOOTSTRAP_FILE_PATH ||
                    path.join(__dirname, '..', 'config', 'bootstrap-tickets.json');
    ...
    try {
      ...
      for (const [eventId, event] of Object.entries(data.events)) {
        statements.push({
          ...
          args: [
            eventId,
            eventId, // Use event ID as slug
            event.name,
            'festival', // Default event type
            event.date,
            event.end_date || null,
            event.venue,
            event.status || 'active',
...
if (process.env.NODE_ENV !== 'test') {
  bootstrapService.initialize().catch(error => {
    logger.error('Failed to auto-initialize bootstrap service:', error);
  });
}