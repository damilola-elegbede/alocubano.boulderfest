...
class BootstrapService {
  ...
  async applyBootstrap(data, checksum) {
    ...
    try {
      ...
      // Note: Use INSERT OR IGNORE to handle corrupted migration state gracefully
      // If migration 041 recorded success but events were deleted, this recreates them
      for (const event of data.events) {
        ...
        eventStatements.push({
          ...
          args: [
            ...
            JSON.stringify({}) // Empty config for now, can be extended later
          ]
        });
      }

      // Execute events first to ensure they exist before ticket_types reference them
      if (eventStatements.length > 0) {
        logger.log(`📝 Inserting ${eventStatements.length} events...`);
        await db.batch(eventStatements);
        logger.log(`✅ Events processed`);
        ...
        logger.log(`🔍 Events in database: ${eventCheck.rows.length}`);
