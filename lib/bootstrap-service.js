...
class BootstrapService {
  ...
  async _performInitialization() {
    try {
      ...
      // OPTIMIZED: Single combined query to check data existence AND checksum match
      // Replaces 3 separate queries with 1 for 2-3x faster bootstrap initialization
      const db = await getDatabaseClient();

      let dataCheck;
      let checksumMatch = false;

      // Calculate expected counts from bootstrap data
      const expectedEvents = bootstrapData.events.length;
      const expectedTickets = bootstrapData.ticket_types.length;

      try {
        // Combined query: Get counts and checksum status in one query
        const result = await db.execute({
          sql: `
            SELECT
              (SELECT COUNT(*) FROM events) as event_count,
              (SELECT COUNT(*) FROM ticket_types) as ticket_type_count,
              (SELECT COUNT(*) FROM bootstrap_versions WHERE checksum = ? AND status = 'success') as checksum_match
          `,
          args: [checksum]
        });
        ...
        const ticketTypeCount = Number(row.ticket_type_count);
        dataCheck = {
          hasData: eventCount > 0 && ticketTypeCount > 0,
          hasAllData: eventCount >= expectedEvents && ticketTypeCount >= expectedTickets,
          eventCount,
          ticketTypeCount,
          expectedEvents,
          expectedTickets
        };
        checksumMatch = Number(row.checksum_match) > 0;

      } catch (error) {
        // If query fails, fall back to individual checks
        logger.log('⚠️  Combined query failed, using fallback...');
        const fallbackCheck = await this.checkDataExists();
        dataCheck = {
          ...fallbackCheck,
          hasAllData: fallbackCheck.eventCount >= expectedEvents && fallbackCheck.ticketTypeCount >= expectedTickets,
          expectedEvents,
          expectedTickets
        };
        checksumMatch = await this.isAlreadyApplied(checksum);
      }
      ...
      // NEW: Check if ALL expected data exists (not just SOME data)
      // This catches cases where production data exists but test data is missing
      if (!dataCheck.hasAllData) {
        logger.log('🔄 Bootstrap data incomplete, applying now...');
        logger.log(`   Expected: ${dataCheck.expectedEvents} events, ${dataCheck.expectedTickets} ticket types`);
        logger.log(`   Actual: ${dataCheck.eventCount} events, ${dataCheck.ticketTypeCount} ticket types`);
        if (checksumMatch) {
          logger.warn('⚠️  Bootstrap checksum exists but data incomplete - possible data loss or migration issue');
        }
        const result = await this.applyBootstrap(bootstrapData, checksum);
        return result;
      }

      // Data exists AND is complete, now check if checksum matches (optimization to skip re-insertion)
      if (checksumMatch && dataCheck.hasAllData) {
      ...
      for (const event of data.events) {
        ...
        eventStatements.push({
          ...
          args: [
            ...
            event.type || 'festival',
            event.status || 'draft',
            optionalField(event.description),
            optionalField(event.venue_name || event.venue), // Support both new and legacy format
            optionalField(event.venue_address),
            event.venue_city || 'Boulder', // Default to Boulder
            event.venue_state || 'CO', // Default to CO
            optionalField(event.venue_zip),
            event.start_date,
            optionalField(event.end_date),
            optionalField(event.max_capacity),
            optionalField(event.early_bird_end_date),
            optionalField(event.regular_price_start_date),
        ...
        for (const color of data.ticket_type_colors) {
          colorStatements.push({
            ...
            args: [
              ...
              color.circle_emoji || '⬤',
              color.display_order,
              optionalField(color.description)
            ]
          });
        }
  ...
  async getStatus() {
    ...
    try {
      // OPTIMIZED: Combined query to get all status info in one query (replaces 4 queries with 1)
      const result = await db.execute({
        sql: `
          SELECT
            (SELECT id FROM bootstrap_versions WHERE status = 'success' ORDER BY applied_at DESC LIMIT 1) as last_id,
            (SELECT version FROM bootstrap_versions WHERE status = 'success' ORDER BY applied_at DESC LIMIT 1) as last_version,
            (SELECT checksum FROM bootstrap_versions WHERE status = 'success' ORDER BY applied_at DESC LIMIT 1) as last_checksum,
            (SELECT applied_at FROM bootstrap_versions WHERE status = 'success' ORDER BY applied_at DESC LIMIT 1) as last_applied_at,
            (SELECT status FROM bootstrap_versions WHERE status = 'success' ORDER BY applied_at DESC LIMIT 1) as last_status,
            (SELECT COUNT(*) FROM events WHERE status != 'test') as event_count,
            (SELECT COUNT(*) FROM ticket_types WHERE status != 'test') as ticket_type_count,
      ...
      // Reconstruct lastBootstrap object from individual fields
      const lastBootstrap = row.last_id ? {
        id: row.last_id,
        version: row.last_version,
        checksum: row.last_checksum,
      ...
      return {
        ...
        ticketTypeCount: Number(row.ticket_type_count) || 0,
        colorPatternCount: Number(row.color_pattern_count) || 0
      };
