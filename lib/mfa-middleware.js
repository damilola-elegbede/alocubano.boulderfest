...
// Disabled for Vercel - setInterval issues
...
export function requireMfa(handler) {
  return async (req, res) => {
    try {
      ...
      const token = getAuthService().getSessionFromRequest(req);
      if (!token) {
        return res.status(401).json({ error: "Authentication required" });
      }

      const session = getAuthService().verifySessionToken(token);
      if (!session.valid) {
...
export async function verifyMfaCode(adminId, code, req) {
  ...
  try {
    ...
    // Rate limiting disabled for Vercel - setInterval issues
    ...
    // Temporary placeholder for rate limit result
    const rateLimitResult = { attemptsRemaining: 5 };
...
async function logMfaAttempt(
  ...
  try {
    ...
    const sessionToken = getAuthService().getSessionFromRequest(req);
