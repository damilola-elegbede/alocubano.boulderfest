...
>
  ...
  <div id="gallery-description" class="sr-only">
    ...
    Use arrow keys to navigate between photos, Enter to open in lightbox, and
    Tab to access gallery controls.
  </div>
      ...
      >
        ...
        <div class="gallery-overlay">
          ...
          <p class="gallery-meta" id="photo-description-0">
            Saturday evening, Main Ballroom. Traditional Cuban salsa performance
            by Havana Dance Company.
          </p>
...
class VirtualGalleryKeyboardNavigation {
  ...
  updateFocus(index) {
    ...
    const previousItem = document.getElementById(
      `photo-item-${this.currentFocusIndex}`,
    );
    if (previousItem) {
    ...
    if (newItem) {
      ...
      this.gallery.viewport.setAttribute(
...
class VirtualGalleryVoiceSupport {
  ...
  addVoiceLabels() {
    ...
    const yearButtons =
      this.gallery.container.querySelectorAll(".year-nav-btn");
  ...
  monitorPerformanceForA11y() {
    ...
    const checkPerformance = (currentTime) => {
      ...
      if (currentTime - lastTime >= 1000) {
        ...
        if (
          fps < 30 &&
          !this.gallery.container.classList.contains(
            "assistive-technology-mode",
          )
        ) {
          this.enableAssistiveTechMode();
...
<div class="gallery-error-state" role="alert" aria-live="assertive">
  ...
  <div class="error-content">
    ...
    <p class="error-message" id="error-description">
      Unable to load gallery images. This may be due to a network connection
      issue.
    </p>

    <div class="error-actions">
      <button class="retry-button primary" aria-describedby="error-description">
        Retry Loading
      </button>
      <button
