...
<div class="responsive-image-container" data-responsive-image>
  <picture class="responsive-picture">
    ...
    <source
      srcset="
        /api/image-proxy/123?w=800&format=avif&q=80   800w,
    ...
    <source
      srcset="
        /api/image-proxy/123?w=400&format=avif&q=85 400w,
    ...
    <source
      srcset="
        /api/image-proxy/123?w=800&format=webp&q=85   800w,
    ...
    <source
      srcset="
        /api/image-proxy/123?w=400&format=webp&q=85 400w,
    ...
    <img
      ...
      srcset="
        /api/image-proxy/123?w=400&format=jpeg&q=85   400w,
...
class AdvancedImageFormatManager {
  ...
  async detectAVIFSupport() {
    return new Promise((resolve) => {
      const avifData =
        "data:image/avif;base64,AAAAIGZ0eXBhdmlmAAAAAGF2aWZtaWYxbWlhZk1BMUEAAADybWVubw==";
      const img = new Image();
  ...
  selectOptimalFormat(imageData = {}) {
    ...
    for (const format of this.options.fallbackChain) {
      if (this.capabilities[format]) {
        ...
        return {
          format,
          quality: this.getQualityForFormat(
            format,
            isMobile,
            isHighDPI,
            isSlowConnection,
  ...
  generateResponsiveSources(imageId, options = {}) {
    ...
    for (const format of this.options.fallbackChain) {
      ...
      const desktopSrcset = breakpoints
        ...
        .map(
          (width) =>
            `/api/image-proxy/${imageId}?w=${width}&format=${format}&q=${quality} ${width}w`,
        )
        .join(", ");
      ...
      const mobileSrcset = breakpoints
        ...
        .map(
          (width) =>
            `/api/image-proxy/${imageId}?w=${width}&format=${format}&q=${quality + 5} ${width}w`,
        )
        .join(", ");
    ...
    const jpegSrcset = breakpoints
      .map(
        (width) =>
          `/api/image-proxy/${imageId}?w=${width}&format=jpeg&q=${jpegQuality} ${width}w`,
      )
      .join(", ");
    ...
    if (formatPerformance.length > 1) {
      ...
      if (slowest.avgLoadTime > fastest.avgLoadTime * 1.5) {
        recommendations.push({
          ...
          message: `${fastest.format.toUpperCase()} is ${Math.round(
            (slowest.avgLoadTime / fastest.avgLoadTime - 1) * 100,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const targetFormat =
      format === "auto" ? detectOptimalFormat(req.headers["accept"]) : format;
