...
async function optimizeImage(inputPath, outputDir, baseName) {
  ...
  try {
    ...
    for (const [variant, config] of Object.entries(OPTIMIZATION_CONFIG)) {
      ...
      await image.clone().webp({ quality: config.quality.webp }).toFile(webpPath);
      ...
      await image.clone().avif({ quality: config.quality.avif }).toFile(avifPath);
      ...
      await image.clone().jpeg({ quality: config.quality.jpeg }).toFile(jpegPath);
    ...
    const reductionPercent = ((originalSize - totalOptimizedSize) / originalSize * 100).toFixed(1);
