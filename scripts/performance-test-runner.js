...
const REGRESSION_THRESHOLDS = {
  response_time_degradation: 0.10,    // 10% slower = regression
...
class K6TestExecutor {
  constructor(testConfig, options = {}) {
    ...
    this.options = {
      baseUrl: options.baseUrl || process.env.LOAD_TEST_BASE_URL || 'http://localhost:3000',
  ...
  async execute() {
    print(`\n🚀 Starting ${this.testConfig.name} performance test...`, 'cyan', true);
    ...
    const k6Command = [
      ...
      'run',
      '--out', `json=${outputFile}`,
      '--env', `LOAD_TEST_BASE_URL=${this.options.baseUrl}`,
      '--env', `TEST_ENV=${this.options.environment}`,
      '--env', `TEST_ID=${this.testId}`,
      this.testFile
    ];
  ...
  extractKPIs(metrics, dataPoints) {
    ...
    const customMetricKeys = Object.keys(metrics).filter(key => 
      !key.startsWith('http_') && !key.startsWith('data_') && 
      !key.startsWith('vus') && !key.startsWith('iteration')
    );
...
class BaselineManager {
  ...
  async compareWithBaseline(testName, currentResults) {
    ...
    print(`🔍 Comparing against baseline from ${new Date(baseline.timestamp).toLocaleDateString()}`, 'blue');
    ...
    for (const [metricName, metricData] of Object.entries(currentResults.summary.custom_metrics || {})) {
  ...
  compareMetric(comparison, name, baselineValue, currentValue, threshold, direction) {
    if (baselineValue == null || currentValue == null) {
      return;
    }

    const percentChange = ((currentValue - baselineValue) / baselineValue);
    const isRegression = direction === 'lower_is_better' ? 
      percentChange > threshold : 
      percentChange < -threshold;

    const isImprovement = direction === 'lower_is_better' ?
      percentChange < -threshold :
      percentChange > threshold;
...
class ReportGenerator {
  ...
  generateExecutiveSummary(testResults, comparisons) {
    ...
    for (const result of testResults) {
      ...
      if (comparison && comparison.hasBaseline && comparison.regressions.length > 0) {
        summary.testsWithRegressions++;
        summary.totalRegressions += comparison.regressions.length;
        
        // Check for critical regressions
        for (const regression of comparison.regressions) {
          if (Math.abs(regression.change) > 0.25) { // >25% degradation
          ...
          } else if (Math.abs(regression.change) > 0.15) { // >15% degradation
            summary.criticalIssues.push({
              test: testName,
              metric: regression.name,
              severity: 'WARNING',
  ...
  generateRecommendations(testResults, comparisons) {
    ...
    for (const [testName, comparison] of Object.entries(comparisons)) {
      if (!comparison.hasBaseline || comparison.regressions.length === 0) continue;
  ...
  async sendAlert(level, message, details = {}) {
    ...
    if (details.regressions) {
      payload.attachments[0].fields.push({
        title: 'Regressions',
        value: details.regressions.map(r => `• ${r.name}: ${r.changeFormatted}`).join('\n'),
        short: false
      });
    }

    try {
      const url = level === 'critical' && this.escalationUrl ? this.escalationUrl : this.webhookUrl;
...
class PerformanceTestOrchestrator {
  ...
  async execute() {
    ...
    try {
      ...
      // Generate reports
      const report = await this.reportGenerator.generateComprehensiveReport(testResults, comparisons);
      
      // Send alerts if needed
      const criticalRegressions = this.extractCriticalRegressions(comparisons);
      await this.alertSystem.notifyResults(report.reportData.summary, criticalRegressions);
      ...
      await this.alertSystem.sendAlert('critical', `Performance test execution failed: ${error.message}`);
      throw error;
    }
  }
    ...
    if (this.options.parallel && testsToExecute.length > 1) {
      ...
      for (let i = 0; i < parallelResults.length; i++) {
        ...
        if (result.status === 'fulfilled') {
          ...
          print(`❌ Test ${testsToExecute[i].name} failed: ${result.reason.message}`, 'red');
          throw result.reason;
        }
      }
    } else {
  ...
  async compareWithBaselines(testResults) {
    ...
    for (const result of testResults) {
      const testName = result.testConfig.name.toLowerCase().replace(/\s+/g, '-');
      const comparison = await this.baselineManager.compareWithBaseline(testName, result);
      comparisons[result.testConfig.name] = comparison;
      
      if (comparison.hasBaseline) {
        if (comparison.regressions.length > 0) {
          print(`  ⚠️  ${result.testConfig.name}: ${comparison.regressions.length} regression(s)`, 'yellow');
        } else if (comparison.improvements.length > 0) {
          print(`  ✅ ${result.testConfig.name}: ${comparison.improvements.length} improvement(s)`, 'green');
        } else {
          print(`  📊 ${result.testConfig.name}: Performance stable`, 'blue');
        }
      } else {
        print(`  📈 ${result.testConfig.name}: No baseline (first run)`, 'blue');
      }
    }
  ...
  printFinalSummary(summary, totalDuration) {
    ...
    print(`🎯 Overall Status: ${summary.overallStatus}`, summary.overallStatus === 'PASS' ? 'green' : summary.overallStatus === 'WARNING' ? 'yellow' : 'red', true);
    
    if (summary.criticalIssues.length > 0) {
      print('\n🚨 Critical Issues:', 'red', true);
      for (const issue of summary.criticalIssues.slice(0, 5)) {
        print(`  • ${issue.test}: ${issue.metric} degraded by ${issue.change}`, 'red');
      }
    }
