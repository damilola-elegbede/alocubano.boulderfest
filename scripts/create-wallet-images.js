...
async function createWalletImages() {
  ...
  // Create @1x versions
  await sharp(path.join(walletDir, "logo@2x.png"))
    .resize(160, 50)
    .toFile(path.join(walletDir, "logo.png"));
  await sharp(path.join(walletDir, "icon@2x.png"))
    .resize(29, 29)
    .toFile(path.join(walletDir, "icon.png"));
  await sharp(path.join(walletDir, "strip@2x.png"))
    .resize(375, 98)
    .toFile(path.join(walletDir, "strip.png"));
  ...
  googleFiles.forEach((file) =>
    console.log(`  - public/images/wallet/${file}`),
  );
}
