...
const config = {
  ...
  patterns: {
    orchestratorImport:
      /import\s*{[^}]*TestInitializationOrchestrator[^}]*}\s*from\s*['"][^'"]*['"];?\s*/g,
    enhancedSetupImport:
      /import\s*{[^}]*}\s*from\s*['"][^'"]*enhanced-test-setup[^'"]*['"];?\s*/g,
    databaseHelpersImport:
      /import\s*{[^}]*}\s*from\s*['"][^'"]*database-test-helpers[^'"]*['"];?\s*/g,
    orchestratorUsage: /(?:testOrchestrator\.|orchestrator\.)/g,
    createTestContext:
      /(?:await\s+)?(?:testOrchestrator\.)?createTestContext\([^)]*\)/g,
    beginTransaction:
      /(?:await\s+)?(?:testOrchestrator\.)?beginTestTransaction\([^)]*\)/g,
    rollbackTransaction:
      /(?:await\s+)?(?:testOrchestrator\.)?rollbackTestTransaction\([^)]*\)/g,
    setupIntegrationTests: /setupIntegrationTests\([^)]*\)/g,
    complexBeforeAll:
      /beforeAll\s*\(\s*async\s*\(\s*\)\s*=>\s*{[^}]*orchestrator[^}]*}\s*\)/g,
    complexAfterAll:
      /afterAll\s*\(\s*async\s*\(\s*\)\s*=>\s*{[^}]*orchestrator[^}]*}\s*\)/g,
  },
...
${existingComment || ""}`;
      ...
      fileFilter: (file) =>
        file.includes("test-initialization-orchestrator") ||
        file.includes("enhanced-test-setup"),
    },
    {
      name: "Replace specific database-test-helpers import",
      pattern:
        /import\s*{\s*dbTestHelpers\s*}\s*from\s*['"][^'"]*database-test-helpers[^'"]*['"];?\s*/g,
      replacement:
...
function processFile(filePath) {
  try {
    ...
    const { content: transformedContent, transformCount } = transformContent(
      content,
      filePath,
    );
    ...
    log(
      `✅ Migrated ${filePath} (${transformCount} transformations)`,
      "success",
    );
  } catch (error) {
...
function findJSFiles(dir, files = []) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (
        ["node_modules", "dist", "build", ".migration-backups"].includes(
          entry.name,
        )
      ) {
        continue;
      ...
      entry.name !== "migrate-orchestrator-tests.js"
    ) {
      files.push(fullPath);
    }
  }
...
async function findFiles() {
  try {
    ...
    const patternRoot =
      userPattern && userPattern !== "tests/**/*.js"
...
function generateReport() {
  ...
  console.log(
    "  4. Remove backup files when confident: rm -rf .migration-backups",
  );
}
