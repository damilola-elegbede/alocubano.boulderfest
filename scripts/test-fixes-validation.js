...
async function testSpecificFiles() {
  ...
  console.log(`${COLORS.BLUE}Testing specific files that were fixed...${COLORS.RESET}\n`);
  ...
  for (const file of testFiles) {
    ...
    if (code === 0) {
      console.log(`${COLORS.GREEN}✓ PASSED${COLORS.RESET} - ${testResults.passed} tests passed\n`);
    } else {
      console.log(`${COLORS.RED}✗ FAILED${COLORS.RESET} - ${testResults.failed} failures\n`);
...
async function validateFixes() {
  ...
  if (allSpecificPassed) {
    console.log(`${COLORS.GREEN}All specific tests passed! Running full suite...${COLORS.RESET}\n`);
    ...
    if (fullResults.passed) {
      ...
      console.log(`${COLORS.RED}✗ FAILURE${COLORS.RESET} Some tests still failing`);
      console.log(`  Total: ${fullResults.total} tests`);
      console.log(`  Passed: ${fullResults.passed}`);
      console.log(`  Failed: ${fullResults.failed}`);
      console.log(`  Skipped: ${fullResults.skipped}`);
    ...
    console.log(`${COLORS.RED}✗ Individual test fixes failed. Please review the failures above.${COLORS.RESET}\n`);
    process.exit(1);
  }
}
