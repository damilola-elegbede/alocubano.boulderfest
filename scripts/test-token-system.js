...
async function testTokenSystem() {
  ...
  try {
    ...
    const accessToken = await tokenService.generateAccessToken(transactionId, email);
    console.log(`   Generated access token: ${accessToken.substring(0, 20)}...`);

    // Generate action token
    const actionToken = await tokenService.generateActionToken('transfer', 'TKT-TEST-123', email);
    console.log(`   Generated action token: ${actionToken.substring(0, 20)}...`);

    // Test 4: Token validation
    console.log('\n4. Testing token validation...');
    
    const accessValidation = await tokenService.validateAccessToken(accessToken);
    console.log(`   Access token valid: ${accessValidation.valid}`);
    console.log(`   Access token email: ${accessValidation.email}`);

    const actionValidation = await tokenService.validateActionToken(actionToken, 'transfer', 'TKT-TEST-123');
    ...
    const qrTokenData = tokenService.generateValidationToken('TKT-TEST-QR-123', 'boulder-fest-2026', email);
    console.log(`   Generated QR data: ${qrTokenData.qr_data.substring(0, 30)}...`);
    ...
    const rateLimitCheck = await tokenService.checkRateLimit(email, 'access', 60, 10);
    ...
    console.log(`   Cleaned up ${cleanupResult.accessTokensDeleted} access tokens`);
    console.log(`   Cleaned up ${cleanupResult.actionTokensDeleted} action tokens`);

    // Test 8: Token statistics
    console.log('\n8. Getting token statistics...');
    
    const stats = await tokenService.getTokenStats();
    console.log(`   Access tokens: ${stats.accessTokens.total} total, ${stats.accessTokens.valid} valid`);
    console.log(`   Action tokens: ${stats.actionTokens.total} total, ${stats.actionTokens.valid} valid`);

    // Show why action token is invalid - it was consumed during validation
    console.log('   Note: Action token shows as invalid because it was consumed during validation (single-use)');
