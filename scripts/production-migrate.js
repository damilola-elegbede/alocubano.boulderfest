...
class ProductionMigrationDeployment {
  ...
  async createBackup() {
    ...
    try {
      ...
      console.log(
        "ℹ️  Manual backup: Use Turso CLI to create replica before deployment",
      );

      return {
        backupName,
        created: false,
        message: "Manual backup recommended",
      };
    } catch (error) {
      console.warn("⚠️  Backup creation failed:", error.message);
      console.log(
        "ℹ️  Continuing without backup - ensure manual backup exists",
      );
  ...
  async deployMigrations(options = {}) {
    ...
    try {
      ...
      console.error(
        "❌ Production migration deployment failed:",
        error.message,
      );
