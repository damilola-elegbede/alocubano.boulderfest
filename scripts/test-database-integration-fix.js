...
try {
  const { integrationTestDatabaseFactory } = await import(
    "../tests/utils/integration-test-database-factory.js"
  );
  ...
  const client =
    await integrationTestDatabaseFactory.createRealDatabaseClient(testContext);
...
try {
  const { databaseClientValidator } = await import(
    "../tests/utils/database-client-validator.js"
  );
...
try {
  ...
  const insertResult = await client.execute(
    "INSERT INTO test_txn (value) VALUES (?)",
    ["test"],
  );
