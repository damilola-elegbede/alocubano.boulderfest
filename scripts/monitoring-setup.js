  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (response.ok && data.status === 'healthy') {
        print(`    ✅ ${endpoint}: HEALTHY (${data.response_time || 'N/A'})`, 'green');
...
async function validateAlertConfig() {
  ...
  for (const threshold of thresholds) {
    ...
    print(`    ${threshold.name}: ${value} (default: ${threshold.default})`, 'green');
  }
...
async function checkExternalServices() {
  ...
  const hasGoogleAuth = checkEnvVar('GOOGLE_SHEETS_API_KEY', false) || 
...
function generateDashboardURLs() {
  ...
  const baseUrl = process.env.VERCEL_URL || 'https://alocubanoboulderfest.vercel.app';
  ...
  if (process.env.SENTRY_DSN) {
    ...
    print(`    https://sentry.io/organizations/${sentryOrg}/projects/${sentryProject}/`, 'green');
  }
...
async function main() {
  ...
  if (process.env.VERCEL_URL || process.argv.includes('--test-health')) {
    ...
    print('\n⏭️  Skipping health check tests (no VERCEL_URL configured)', 'yellow');
  ...
  if (allValid) {
    ...
    if (!results.sentry) {
      print('  - Fix Sentry configuration (optional but recommended)', 'yellow');
    }
