...
class SchemaValidator {
      ...
      for (const file of jsFiles) {
        ...
        for (const pattern of patterns) {
          ...
          for (const match of matches) {
            ...
            if (
              column &&
              (column === "wallet_source" || column === "qr_access_method")
            ) {
        ...
        if (fileReferences.size > 0) {
          ...
          references.found.push(
            ...Array.from(fileReferences).map((col) => ({
              file,
              column: col,
            })),
          );
  ...
  async validateColumnExistence() {
    ...
    for (const [tableName, requiredCols] of Object.entries(
      ...
      const extra = actualColumnNames.filter(
        (col) => !requiredCols.includes(col),
      );
      if (extra.length > 0) {
  ...
  async validateDataTypes() {
    ...
    for (const [tableName, columnTypes] of Object.entries(
      this.columnDataTypes,
    )) {
      const actualColumns = await this.getTableColumns(tableName);
  ...
  async validateIndexes() {
    ...
    for (const [tableName, requiredIdxs] of Object.entries(
      this.requiredIndexes,
    )) {
      ...
      const missing = requiredIdxs.filter(
  ...
  async generateReport() {
    ...
    console.log(
      `  ✅ Found ${codebaseReferences.found.length} column references`,
    );
    ...
    if (performanceResults.status === "FAIL") {
      ...
      performanceResults.status === "WARN" &&
      report.status === "PASS"
    ) {
      report.status = "WARN";
    }
    ...
    report.performance_metrics = {
      avg_query_time:
        Math.round(
          performanceResults.tests.reduce(
    ...
    for (const test of performanceResults.tests) {
      if (test.passed) {
        console.log(
          `  ✅ ${test.name}: ${test.executionTime}ms (max: ${test.maxTime}ms)`,
        );
      } else if (test.error) {
        console.error(`  ❌ ${test.name}: ${test.error}`);
      } else {
        console.warn(
          `  ⚠️  ${test.name}: ${test.executionTime}ms (max: ${test.maxTime}ms)`,
        );
      }
    }
