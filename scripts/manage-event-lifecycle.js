...
const LIFECYCLE_CONFIG_PATH = path.join(
  projectRoot,
  "config",
...
function initializeEvent(
  config,
  eventId,
  initialState = LIFECYCLE_STATES.UPCOMING,
) {
...
function transitionEvent(eventId, newState, options = {}) {
  ...
  if (newState === LIFECYCLE_STATES.CURRENT) {
    const currentEvents = Object.entries(config.events).filter(
      ([id, evt]) => evt.state === LIFECYCLE_STATES.CURRENT && id !== eventId,
    );

    if (currentEvents.length >= config.settings.currentEventLimit) {
      if (options.force) {
        console.log(
          `⚠️  Forcing transition - ${currentEvents.length} current events already exist`,
        );
      } else {
        throw new Error(
          `Cannot set ${eventId} as current - limit of ${config.settings.currentEventLimit} current events reached. Use --force to override.`,
        );
      }
    }
  }
  ...
  if (!options.dryRun) {
    ...
    console.log(
      `🔍 [DRY RUN] Would transition ${eventId} from ${oldState} to ${newState}`,
    );
  }
}
      ...
      events
        ...
        .forEach((event) => {
          const lastTransition =
            event.transitions[event.transitions.length - 1];
...
function autoArchive(options = {}) {
  ...
  cutoffDate.setDate(
    cutoffDate.getDate() - (config.settings.archiveDelayDays || 365),
  );
    ...
    if (event.state === LIFECYCLE_STATES.PAST && event.metadata.completedAt) {
      ...
      if (completedDate < cutoffDate) {
        if (!options.dryRun) {
          ...
          console.log(
            `🔍 [DRY RUN] Would archive ${eventId} (completed ${completedDate.toLocaleDateString()})`,
          );
        }
        archivedCount++;
      }
  ...
  if (archivedCount === 0) {
    ...
    console.log(
      `  ✅ ${options.dryRun ? "Would archive" : "Archived"} ${archivedCount} events`,
    );
  }
}
...
function main() {
  ...
  if (args.length === 0 || args[0] === "--help" || args[0] === "-h") {
    ...
    console.log("Actions:");
    console.log(
      "  list                          List all events and their states",
    );
    console.log(
      "  transition <event-id> <state> Transition event to new state",
    );
    ...
    console.log(
      "  --force                       Force transition (bypass limits)",
    );
    ...
    console.log(
      "  node scripts/manage-event-lifecycle.js promote boulder-fest-2026",
    );
    console.log(
      "  node scripts/manage-event-lifecycle.js transition weekender-2026-09 past",
    );
    console.log(
      "  node scripts/manage-event-lifecycle.js auto-archive --dry-run",
    );
    process.exit(0);
  }
