    ...
    try {
      ...
      if (recent.rows.length > 0) {
        recent.rows.forEach((checkin) => {
          const method =
            checkin.qr_access_method === "wallet" ? " [WALLET]" : "";
          console.log(
            `   ${checkin.ticket_id}: ${checkin.checked_in_at} by ${checkin.checked_in_by}${method}`,
          );
        });
      } else {
      ...
      console.log(
        "1. Database connection not available (this is expected in local testing)",
      );
