...
function getNextFutureHeroNumber() {
  ...
  const existingHeroes = fs.readdirSync(heroDir)
    .filter(file => file.startsWith('future-event-hero') && file.endsWith('.jpg'))
...
function createHeroImages(eventId, futureHeroNumber) {
  ...
  const sourceHero = path.join(heroDir, `future-event-hero${futureHeroNumber}.jpg`);
  ...
  variants.forEach(variant => {
    ...
    if (fs.existsSync(variantDir)) {
      formats.forEach(format => {
        const sourceFile = path.join(variantDir, `future-event-hero${futureHeroNumber}.${format}`);
        ...
        if (fs.existsSync(sourceFile)) {
          ...
          // Try fallback
          const fallbackFile = path.join(variantDir, `future-event-hero1.${format}`);
...
function createEventPage(eventId, pageType, templateEventId) {
  ...
  const templatePath = path.join(projectRoot, 'pages', `${templateEventId}-${pageType}.html`);
  const targetPath = path.join(projectRoot, 'pages', `${eventId}-${pageType}.html`);
  ...
  const eventDisplayName = eventId.split('-').map(word => 
...
function updateGalleryCacheConfig(eventId) {
  ...
  const cacheScriptPath = path.join(projectRoot, 'scripts', 'generate-gallery-cache.js');
  let content = fs.readFileSync(cacheScriptPath, 'utf8');
  
  // Add new event to EVENT_GALLERY_CONFIG
  const configMatch = content.match(/(const EVENT_GALLERY_CONFIG = \{[\s\S]*?)\s*\/\/ Add future events here as needed/);
  if (configMatch) {
    const newConfig = configMatch[1] + `  '${eventId}': null, // To be configured when folder is available\n  // Add future events here as needed`;
    ...
    console.log(`  ⚠️  Could not update gallery cache configuration automatically`);
  }
}
...
function updateHeroPreloadConfig(eventId) {
  ...
  const preloadScriptPath = path.join(projectRoot, 'scripts', 'update-hero-preloads.cjs');
  let content = fs.readFileSync(preloadScriptPath, 'utf8');
  
  // Add new event to EVENT_HERO_MAPPING
  const heroMappingMatch = content.match(/(const EVENT_HERO_MAPPING = \{[\s\S]*?)\s*\/\/ Add more as needed\.\.\./);
  if (heroMappingMatch) {
    const newMapping = heroMappingMatch[1] + `  '${eventId}': '/images/hero/${eventId}-hero.jpg',\n  // Add more as needed...`;
    content = content.replace(heroMappingMatch[0], newMapping);
  }
  
  // Add event pages to EVENT_PAGE_MAPPING
  const pageMappingMatch = content.match(/(\/\/ Future event-specific pages[\s\S]*?)(,\s*};)/);
  if (pageMappingMatch) {
    const eventPages = EVENT_PAGE_TYPES.map(pageType => 
      `  '${eventId}-${pageType}': '${eventId}'`
    ).join(',\n');
    
    const newPageMapping = pageMappingMatch[1] + `,\n${eventPages}` + pageMappingMatch[2];
...
function createNewEvent(eventId, templateEventId = DEFAULT_TEMPLATE) {
  ...
  if (!eventId || !eventId.match(/^[a-z0-9-]+$/)) {
    console.error('❌ Invalid event ID. Use lowercase letters, numbers, and hyphens only.');
    process.exit(1);
  }
  
  // Check if event already exists
  const existingPage = path.join(projectRoot, 'pages', `${eventId}-artists.html`);
  ...
  const templatePage = path.join(projectRoot, 'pages', `${templateEventId}-artists.html`);
  ...
  try {
    ...
    console.log(`   2. Configure Google Drive folder ID in scripts/generate-gallery-cache.js`);
...
function parseArgs() {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log('Usage: node scripts/create-new-event.js <event-id> [--template=event-id]');
    console.log('');
    console.log('Examples:');
    console.log('  node scripts/create-new-event.js salsa-fest-2027');
    console.log('  node scripts/create-new-event.js weekender-2027-03 --template=weekender-2026-09');
    console.log('');
    console.log('Event ID should use lowercase letters, numbers, and hyphens only.');
    process.exit(0);
  }
