...
async function checkDatabase() {
  ...
  // Check transactions
  const transactionResult = await db.execute('SELECT COUNT(*) as count FROM transactions');
  console.log(`Transactions: ${transactionResult.rows[0].count}`);
  
  // Check tickets
  const ticketResult = await db.execute('SELECT COUNT(*) as count FROM tickets');
  console.log(`Tickets: ${ticketResult.rows[0].count}`);
  
  // Check payment events
  const eventResult = await db.execute('SELECT COUNT(*) as count FROM payment_events');
  ...
  if (latestTransactions.rows.length > 0) {
    ...
    latestTransactions.rows.forEach(t => {
      console.log(`  - ${t.transaction_id}: ${t.customer_email} - $${(t.amount_cents / 100).toFixed(2)} (${t.status})`);
    });
  }
  ...
  if (latestTickets.rows.length > 0) {
    ...
    latestTickets.rows.forEach(t => {
      console.log(`  - ${t.ticket_id}: ${t.ticket_type} - ${t.attendee_email} (${t.status})`);
    });
  }
