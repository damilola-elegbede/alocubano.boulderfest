...
function updateHtmlFile(filePath) {
  try {
    ...
    const scriptRegex = /\s*<script src="\/js\/image-cache-manager\.js[^"]*"><\/script>\s*/g;
    ...
    if (scriptRegex.test(content)) {
      ...
      console.log(`ℹ️ No image-cache-manager.js reference found in ${filename}`);
    }
...
function main() {
  ...
  console.log('🔄 Removing image-cache-manager.js references from HTML files...');
