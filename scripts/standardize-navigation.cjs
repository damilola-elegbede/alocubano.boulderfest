...
function updateHtmlFile(filePath) {
  try {
    ...
    if (navRegex.test(content)) {
      ...
      console.log(
        `✅ Updated navigation in ${filename}${activeHref ? ` (active: ${activeHref})` : ""}`,
      );
    } else {
...
function main() {
  ...
  console.log(
    `Navigation links: ${NAVIGATION_LINKS.map((l) => l.text).join(", ")}`,
  );
  ...
  console.log(
    "All pages now have consistent navigation with all main-level links.",
  );
}
