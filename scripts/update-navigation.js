...
function updateNavigationInFile(filePath, dryRun = false) {
  ...
  console.log(
    `📄 Processing ${filename} (${pageInfo.type}${pageInfo.eventId ? `: ${pageInfo.eventId}` : ""})`,
  );
  ...
  try {
    ...
    if (dryRun) {
      ...
      console.log(
        `    Old: ${oldNav.replace(/\s+/g, " ").trim().substring(0, 60)}...`,
      );
      console.log(
        `    New: ${newNav.replace(/\s+/g, " ").trim().substring(0, 60)}...`,
      );
      return true;
    }
...
function updateMobileNavigationInFile(filePath, dryRun = false) {
  ...
  try {
    ...
    const mobileNavRegex =
      /(<ul class="mobile-nav-menu" id="mobile-nav-menu">[\s\S]*?<\/ul>)/;
...
${mobileNavItems}
    ...
    console.log(
      `  ⚠️  Error updating mobile navigation in ${filename}: ${error.message}`,
    );
...
function validateNavigationConsistency() {
    ...
    try {
      ...
      if (pageInfo.type === "event") {
        ...
        if (invalidLinks.length > 0) {
          issues.push(
            `${filename}: Invalid event links found: ${invalidLinks.join(", ")}`,
          );
        }
      }
    } catch (error) {
  ...
  console.log(
    `  📊 Found ${events.size} events: ${Array.from(events).join(", ")}`,
  );
...
function parseArgs() {
  ...
  if (args.includes("--help") || args.includes("-h")) {
    ...
    console.log(
      "  node scripts/update-navigation.js --event=boulder-fest-2026",
    );
    process.exit(0);
  }
