...
async function testAnalyticsEndpoint(token, type, days = 30) {
  console.log(
    `${colors.cyan}📊 Testing analytics type: ${type}${colors.reset}`,
  );

  const response = await fetch(
    `${BASE_URL}/api/admin/analytics?type=${type}&days=${days}`,
    {
      headers: {
  ...
  switch (type) {
    case "summary":
      ...
      console.log(
        `  - Wallet Adoption: ${data.data.wallet?.adoption_rate || 0}%`,
      );
      break;
    case "statistics":
    ...
    case "revenue":
      ...
      data.data.slice(0, 3).forEach((item) => {
        console.log(
          `    • ${item.ticket_type}: $${item.total_revenue} (${item.revenue_percentage}%)`,
        );
      });
      break;
    case "wallet":
      console.log(
        `  - Wallet Users: ${data.data.summary?.total_wallet_users || 0}`,
      );
      console.log(
        `  - Adoption Rate: ${data.data.summary?.overall_adoption_rate || 0}%`,
      );
    ...
    case "customers":
      ...
      console.log(
        `  - Returning Customers: ${data.data?.returning_customers || 0}`,
      );
      break;
    case "checkins":
    ...
    case "funnel":
      ...
      if (data.data?.stages?.length > 0) {
        data.data.stages.slice(0, 3).forEach((stage) => {
          console.log(
            `    • ${stage.name || "Unknown"}: ${stage.count || 0} (${stage.conversion_rate || 0}%)`,
          );
        });
      }
      break;
...
async function testReportGeneration(token, format = "json") {
  console.log(
    `${colors.cyan}📋 Testing report generation (${format})...${colors.reset}`,
  );

  const response = await fetch(
    `${BASE_URL}/api/admin/generate-report?format=${format}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
async function testDashboardAccess(token) {
  console.log(
    `${colors.cyan}🖥️  Testing dashboard page access...${colors.reset}`,
  );
...
async function runTests() {
  console.log(
    `${colors.blue}🚀 Starting Analytics & Reporting Tests${colors.reset}`,
  );
  console.log(
