...
async function testWalletGeneration() {
  ...
  if (!appleWalletService.isConfigured()) {
    ...
    try {
      ...
      console.log(
        `✅ Apple Wallet pass generated (${passBuffer.length} bytes)`,
      );
      ...
      console.log(
        `✅ Serial number saved: ${ticket.rows[0].apple_pass_serial}\n`,
      );
    } catch (error) {
  ...
  console.log("─".repeat(40));
  console.log(
    `✓ Apple Wallet: ${appleWalletService.isConfigured() ? "✅ Configured" : "❌ Not configured"}`,
  );
  console.log(
    `✓ Google Wallet: ${googleWalletService.isConfigured() ? "✅ Configured" : "❌ Not configured"}`,
  );
  ...
  if (
    ...
    console.log(
      "ℹ️  Note: To fully test wallet functionality, you need to configure",
    );
    console.log(
      "   at least one wallet service with the required credentials.\n",
    );
  }
