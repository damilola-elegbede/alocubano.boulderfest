...
const TEST_SUITES = {
  unit_basic: {
    ...
    pattern:
      "tests/unit/gallery-consolidated.test.js tests/unit/accessibility.test.js tests/unit/api-gallery-logic.test.js",
    expected: "low overhead",
  },
  unit_services: {
    name: "Service Unit Tests",
    pattern:
      "tests/unit/brevo-service.test.js tests/unit/email-subscriber-service.test.js",
  ...
  isolation: {
    ...
    pattern:
      "tests/unit/test-singleton-manager.test.js tests/unit/test-mock-manager.test.js",
    expected: "high overhead",
  },
};
...
class RealPerformanceAnalyzer {
  ...
  calculateSuiteStatistics(configName, testSuiteName, results) {
    ...
    return {
      ...
      testMetrics: {
        totalTests: this.mean(
          successfulResults.map((r) => r.testResults.testCount || 0),
        ),
        avgPassedTests: this.mean(
          successfulResults.map((r) => r.testResults.passedTests || 0),
        ),
        avgFailedTests: this.mean(
          successfulResults.map((r) => r.testResults.failedTests || 0),
        ),
        successRate: (successfulResults.length / results.length) * 100,
      },

      memory: {
        avgHeapUsed: this.mean(
          successfulResults.map((r) => r.memoryDelta.heapUsed),
        ),
  ...
  async runCompleteAnalysis() {
    console.log(
      "🚀 Starting Real-World Test Isolation Performance Analysis...\n",
    );
    ...
    for (const [configName, config] of Object.entries(TEST_CONFIGS)) {
      ...
      for (const [testSuiteName, testSuite] of Object.entries(TEST_SUITES)) {
        try {
          ...
          if (result.success) {
            console.log(
              `   ✅ ${testSuite.name}: ${result.timing.mean.toFixed(0)}ms avg`,
            );
          } else {
    ...
    const reportPath = path.join(
      this.projectRoot,
      "reports",
      "real-isolation-performance.json",
    );
  ...
  generateComparisonReport(analysisResults) {
    ...
    for (const [configName, configResults] of Object.entries(analysisResults)) {
      const successfulSuites = Object.values(configResults).filter(
        (r) => r.success,
      );
      ...
      report.summary[configName] = {
        ...
        reliability:
          (successfulSuites.length / Object.keys(configResults).length) * 100,
      };
    }
    ...
    if (baseline && baseline.avgExecutionTime > 0) {
      for (const [configName, summary] of Object.entries(report.summary)) {
        ...
        const overhead =
          ((summary.avgExecutionTime - baseline.avgExecutionTime) /
    ...
    for (const [configName, comparison] of Object.entries(report.comparisons)) {
      if (comparison.overhead > 50) {
        recommendations.push({
          ...
          recommendation:
            "Consider disabling or optimizing isolation components",
        });
        ...
        recommendations.push({
          ...
          recommendation:
            "Implement selective isolation based on test patterns",
        });
  ...
  displayReport(report) {
    ...
    for (const [configName, summary] of Object.entries(report.summary)) {
      ...
      console.log(
        `   Average Execution: ${summary.avgExecutionTime.toFixed(0)}ms`,
      );
      console.log(
        `   Successful Suites: ${summary.successfulSuites}/${summary.totalSuites}`,
      );
    ...
    if (Object.keys(report.comparisons).length > 0) {
      ...
      for (const [configName, comparison] of Object.entries(
        report.comparisons,
      )) {
        ...
        console.log(
          `   Overhead: ${comparison.overhead.toFixed(1)}% (+${comparison.overheadMs.toFixed(0)}ms)`,
        );
