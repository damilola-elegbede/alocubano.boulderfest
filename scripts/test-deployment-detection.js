...
class DeploymentDetector {
  ...
  async detectDeployment(branchName, ref) {
    console.log(`🔍 Detecting deployment for branch: ${branchName}, ref: ${ref}`);
...
async function main() {
  ...
  for (const testCase of testCases) {
    ...
    try {
      const result = await detector.detectDeployment(testCase.branch, testCase.ref);
...
function demonstrateGitHubCLI() {
  ...
  try {
    ...
    const command = 'gh api repos/alocubano/alocubano.boulderfest/deployments --field per_page=5';
