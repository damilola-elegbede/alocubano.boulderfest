...
async function testTickets() {
  ...
  const existingTickets = await ticketService.getTransactionTickets(transaction.id);
  
  if (existingTickets.length > 0) {
    console.log(`Found ${existingTickets.length} existing tickets:`);
    existingTickets.forEach(ticket => {
      console.log(`  - ${ticket.ticket_id}: ${ticket.ticket_type} (${ticket.status})`);
    ...
    if (lineItems.length === 0) {
      ...
      console.log('This may be an older transaction. Skipping ticket creation.');
    } else {
      console.log(`Found ${lineItems.length} line items:`);
      lineItems.forEach(item => {
        console.log(`  - ${item.description}: $${(item.amount_total / 100).toFixed(2)} x ${item.quantity || 1}`);
      });
  ...
  const emailTickets = await ticketService.getTicketsByEmail(transaction.customer_email);
  console.log(`Total tickets for ${transaction.customer_email}: ${emailTickets.length}`);
  
  if (emailTickets.length > 0) {
    console.log('\nTickets by type:');
    const ticketTypes = {};
    emailTickets.forEach(ticket => {
      ticketTypes[ticket.ticket_type] = (ticketTypes[ticket.ticket_type] || 0) + 1;
    });
