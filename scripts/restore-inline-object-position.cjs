...
function updateHtmlFile(filePath) {
  try {
    ...
    const imgRegex = /(<img[^>]*id="hero-splash-image"[^>]*class="hero-splash-img")([^>]*>)/g;
    
    if (imgRegex.test(content)) {
      const newContent = content.replace(imgRegex, '$1 style="object-position: top center !important;"$2');
