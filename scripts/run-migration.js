...
async function runMigration() {
  ...
  try {
    ...
    const isAlreadyApplied = status.appliedMigrations.some(m => m.version === '009');
    if (isAlreadyApplied) {
      ...
      if (schemaIntegrity.valid) {
        ...
        log.warning('Schema validation failed despite migration being marked as applied');
    ...
    const dryRunResult = await migrationRunner.dryRunMigration('009_add_wallet_tracking.sql');
    ...
    if (!migrationResult.success) {
      ...
      if (migrationResult.rollbackSuccess) {
        log.success(`Automatic rollback successful - restored from ${migrationResult.restoredFrom}`);
    ...
    if (!schemaIntegrity.valid) {
      ...
      log.info(`  - Total indexes: ${Object.values(schemaIntegrity.indexes).flat().length}`);
    }
    ...
    performanceTests.tests.forEach(test => {
      ...
      console.log(
        chalk[color](`  ${icon} ${test.name}: ${test.executionTime || 'N/A'}ms (max: ${test.maxTime}ms)`)
      );
    });
    ...
    const endpointTests = [
      ...
      { name: 'Wallet analytics', query: 'SELECT wallet_source, COUNT(*) as count FROM tickets GROUP BY wallet_source' },
      { name: 'QR validation', query: "SELECT * FROM tickets WHERE ticket_id = 'test' LIMIT 1" }
    ];
    ...
    if (!endpointTestsPassed) {
      log.warning('Some endpoint tests failed - review application compatibility');
    }
    ...
    log.info(`  - Endpoint tests: ${endpointTestsPassed ? 'PASSED' : 'PARTIAL'}`);
    ...
    currentBackups.slice(0, 3).forEach(backup => {
      log.info(`  - ${backup.filename} (${backup.description || 'No description'})`);
    });
